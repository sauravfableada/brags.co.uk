...
if(target_elements_instance!=!1){if($payment_element==''){if(typeof selected_subscription!='undefined'&&target_elements_instance_slug=='payment_intents'){let price=(selected_subscription.data('mc_price')!==undefined&&selected_subscription.data('mc_price')!==null)?parseFloat(selected_subscription.data('mc_price')):parseFloat(selected_subscription.data('price'));let sign_up_fee=parseFloat(selected_subscription.data('sign_up_fee'));if(!isNaN(sign_up_fee)&&sign_up_fee>0){price=price+sign_up_fee}
...
return $.ajax({url:pms.ajax_url,type:'POST',data:{action:'pms_validate_sdk_currency',pms_nonce:pms.pms_validate_currency_nonce,subscription_plan_id:subscriptionPlan.val(),pms_mc_currency:subscriptionPlan.data('mc_currency'),subscription_id:subscriptionPlan.closest('form').find('input[name="pms_current_subscription"]').val(),pay_gate:'stripe_connect'},dataType:'json',}).done(function(response){if(response&&response.success&&response.currency)
...
jQuery(function(){try{pms_stripe_maybe_load_gateway(jQuery)}catch(err){console.error('[PMS Stripe] Error during init:',err)}});jQuery(document).on('elementor/popup/show',function(){if(jQuery('.pms-form #pms-stripe-connect',jQuery('.elementor-popup-modal')).length>0){pms_stripe_maybe_load_gateway(jQuery)
