...
jQuery(document).ready(function($) {
    $('.dokan-spmv-add-new-product-search-box-area.dokan-w13.section-closed').removeClass('section-closed');
    $('.dokan-spmv-add-new-product-search-box-area.dokan-w13.section-closed').attr('class', 'dokan-spmv-add-new-product-search-box-area dokan-w13')
});
jQuery(document).ready(function($) {
    $('.wd-product.wd-hover-base .wd-bottom-actions .wd-action-btn.wd-style-icon > a').on('click', function(e) {
        e.preventDefault();
        var button = $(this);
        button.css('background-color', 'var(--btn-accented-bgcolor-hover)')
    })
})
...
jQuery(function ($) {
    ...
    $('.wd-product').off('touchstart');

    $(document).on('touchstart', '.product-image-link, .wd-entities-title a', function (e) {
        window.location = this.href;
    });

});
jQuery(document).on('click', '.product-image-link', function(e){
    console.log('clicked', e.isDefaultPrevented());
});
;