...
function brags_handle_user_registration_emails($user_id, $is_resend = false)
{
    $user = get_userdata($user_id);

    // Send custom email only if user is a CUSTOMER
    // Relaxed check for resends or to allow users with multiple roles (like seller)
    $has_customer_role = in_array('customer', (array) $user->roles);
    $is_allowed = $has_customer_role;

    if (!$is_resend) {
        // Strict check only for initial registration if desired
        $is_allowed = $has_customer_role && !in_array('seller', (array) $user->roles) && !in_array('administrator', (array) $user->roles);
    }
...
function brags_check_user_verification($user, $password) {
    ...
    if (in_array('customer', (array) $user->roles)) {
        ...
        // If status is missing, default to approved for existing users to prevent lockout
        // or just let them log in as per user request "verified should not block to customer"
        if (empty($status)) {
            // update_user_meta($user->ID, 'account_status', 'approved'); // Option: auto-approve old users
            return $user; 
        }

        if ($status !== 'approved') {
            // We still show the notice if they are not approved, but we don't return an error (don't block login)
...
function handle_resend_verification() {
    ...
    if (is_account_page() && isset($_GET['action']) && $_GET['action'] === 'resend_verification' && isset($_GET['user_id'])) {
        ...
        // Verify user exists and is pending verification (or missing status)
        $user = get_userdata($user_id);
        $status = get_user_meta($user_id, 'account_status', true);
        if ($user && ($status === 'pending' || empty($status))) {
