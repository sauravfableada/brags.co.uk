...
<div class="show_if_seller" style="<?php echo esc_attr( $role_style ); ?>">
    ...
    <p class="form-row form-group form-row-wide">
        ...
        <p class="form-row form-group form-row-wide">
            <button type="button" class="button" id="send-otp-btn" style="display:none;"><?php esc_html_e( 'Send OTP', 'dokan-lite' ); ?></button>
        </p>

        <div id="otp-section" style="display:none;" class="form-row form-group form-row-wide">
            <input type="text" class="input-text form-control" id="otp-input" placeholder="Enter OTP" />
            <button type="button" class="button" id="verify-otp-btn"><?php esc_html_e( 'Verify OTP', 'dokan-lite' ); ?></button>
            <div id="otp-message" style="margin-top: 10px; font-weight: bold;"></div>
        </div>
    ...
    <p>
        <?php
        printf(
        '<p style="font-weight: bold; color: #d63638;">%s</p>',
        esc_html__('A Mobile Phone number is required for security purposes. An OTP Code will be sent to this number each time you login to your Seller Account.', 'woocommerce')
    );
    ?>
    </p>
    ...
    do_action( 'dokan_seller_registration_custom', [] );
...
<p class="form-row form-group user-role vendor-customer-registration">
    ...
    <label class="radio">
        ...
        <?php esc_html_e( 'I am a Customer (looking to Buy products on Brags)', 'dokan-lite' ); ?>
    </label>
    <br/>
    <label class="radio">
        <input type="radio" name="role" value="seller"<?php checked( $role, 'seller' ); ?>  class="dokan-role-seller">
        <?php esc_html_e( 'I am a Seller (looking to Sell products on Brags)', 'dokan-lite' ); ?>
    </label>
