            ...
            <div class="dokan-tax-product-options">
                ...
                <div class="dokan-form-group content-half-part">
                    ...
                    <div class="dokan-text-left">
                        ...
                        $vat_number = get_user_meta( $vendor_id, 'dokan_vat_number', true ); // Adjust the meta key if needed
                        ...
                        if ( $vat_number=='') {
                            // If VAT number not set, force to 'None' and disable dropdown
                            echo '<select name="_tax_status" id="_tax_status" class="dokan-form-control" disabled>';
                            echo '<option value="none" selected>' . esc_html( $tax_status_options['none'] ) . '</option>';
                            echo '</select>';
                            echo '<input type="hidden" name="_tax_status" value="none">';
                            echo '<p class="help-block" style="color:red;">' . __( 'If you are required to charge VAT on this product, please make sure to include your VAT Number in your <a href="' . esc_url( dokan_get_navigation_url( 'settings/store' ) ) . '" target="_blank">Store Settings</a>.', 'dokan' ) . '</p>';
                        } else {
                            dokan_post_input_box(
                                $post_id, '_tax_status', array(
                    ...
                    <!-- <p class="form-field">
                        <span class="description" style="color:#555;">
                            <strong>Note:</strong> If you are required to charge VAT on this product, please make sure to include your VAT Number in your <a href="' . esc_url( dokan_get_navigation_url('settings/store') ) . '" target="_blank">Store Settings</a> page here.
                        </span>
                    </p> -->
                </div>
                ...
                $saved_vat_rate = get_post_meta( $post_id, '_seller_vat_rate', true );
                $saved_custom_rate = get_post_meta( $post_id, '_seller_custom_vat_rate', true );
                $confirmed = get_post_meta( $post_id, '_seller_vat_confirmed', true );
                if (  $vat_number!="") { ?>

                    <div class="dokan-form-group content-half-part">
                        <label class="dokan-control-label" for="seller_vat_rate">
                            <strong><?php esc_html_e( 'Select VAT Rate', 'dokan' ); ?> <span class="required">*</span></strong>
                        </label>
                        <select class="dokan-form-control" name="seller_vat_rate" id="seller_vat_rate_select" required>
                            <option value=""><?php esc_html_e( 'Select a VAT Rate', 'dokan' ); ?></option>
                            <option value="20" <?php selected( $saved_vat_rate, '20' ); ?>>UK Standard 20%</option>
                            <option value="5" <?php selected( $saved_vat_rate, '5' ); ?>>Reduced Rate 5%</option>
                            <option value="0" <?php selected( $saved_vat_rate, '0' ); ?>>Zero Rate 0%</option>
                            <option value="custom" <?php selected( $saved_vat_rate, 'custom' ); ?>>Input your own VAT Rate</option>
                        </select>
                    </div>
                
                    <div class="dokan-form-group content-half-part custom-vat-rate-field" style="display: <?php echo ($saved_vat_rate === 'custom') ? 'block' : 'none'; ?>;">
                        <label class="dokan-control-label" for="seller_custom_vat_rate">
                            <?php esc_html_e( 'Your VAT Rate (%)', 'dokan' ); ?> <span class="required">*</span>
                        </label>
                        <input type="number" step="0.01" min="0" name="seller_custom_vat_rate" id="seller_custom_vat_rate" class="dokan-form-control" value="<?php echo esc_attr( $saved_custom_rate ); ?>" />
                    </div>
                
                    <div class="dokan-form-group" style="margin-top:10px;">
                        <small class="dokan-text-note" style="    display: inline-block;">
                            <?php esc_html_e( 'Please note that it is entirely your responsibility to charge the appropriate VAT rate to customers—seek advice from your accountant if you’re unsure. You may also need to provide an Invoice to certain customers, which can be found on the "Orders" page in your Seller Dashboard.', 'dokan' ); ?>
                        </small>
                    </div>
                
                    <div class="dokan-form-group" style="margin-top:10px;">
                        <label>
                            <input type="checkbox" name="seller_vat_confirmed" value="1" <?php checked( $confirmed, '1' ); ?> required />
                            <?php esc_html_e( 'I confirm that I have selected the appropriate VAT rate and understand that I and/or my company are solely responsible for all VAT, accounting, and compliance matters related to our goods. This includes charging the correct VAT rates and providing valid invoices to customers. I acknowledge that if I am uncertain about any finance-related matters, I will seek advice from a qualified accountant or financial advisor.', 'dokan' ); ?>
                        </label>
                    </div>
                    ...
                    <script>
                        (function($){
                            $(document).ready(function(){
                                $('#seller_vat_rate_select').on('change', function(){
                                    if ($(this).val() === 'custom') {
                                        $('.custom-vat-rate-field').slideDown();
                                    } else {
                                        $('.custom-vat-rate-field').slideUp();
                                    }
                                });
                            });
                        })(jQuery);
