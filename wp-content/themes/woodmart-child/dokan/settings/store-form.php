...
<form method="post" id="store-form" action="" class="dokan-form-horizontal test">
    ...
    <div class="dokan-form-group">
        <label class="dokan-w3 dokan-control-label" for="dokan_gravatar"><?php esc_html_e( 'Company Logo', 'dokan-lite' ); ?></label>
    ...
    if ( $tnc_enable === 'on' ) {
        ...
        <div class="dokan-form-group" id="dokan_tnc_text">
            <label class="dokan-w3 dokan-control-label dokan_tnc_text" for="dokan_store_tnc"><?php esc_html_e( 'TOC Details', 'dokan-lite' ); ?></label>
    ...
    <div class="dokan-form-group button-submit">
...
<script type="text/javascript">
  ...
  $('label.dokan-control-label').each(function() {
    var $label = $(this);
    var currentText = $label.text().trim();

    // If "for" is missing, create it from the label text
    if (!$label.attr('for')) {
      var newFor = currentText.toLowerCase().replace(/\s+/g, '-');
      $label.attr('for', newFor);

      // Append (newFor) to the label text if not already present
      if (!currentText.includes(newFor)) {
...
jQuery(document).ready(function($) {
  $('label.dokan-control-label').each(function() {
    ...
    // Add class to the parent .dokan-form-group based on label text
    var generatedClass = currentText.toLowerCase().replace(/\s+/g, '-');
    $label.closest('.dokan-form-group').addClass(generatedClass);
  });
});
