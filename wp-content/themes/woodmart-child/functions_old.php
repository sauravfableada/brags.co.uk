...
?><div class="vender-form">
        ...
        <div id="individual_fields" style="display: none;">
            ...
            <div class="field-row">
                <div class="form-row form-row-wide half-width passport_upload">
                    <label for="passport_upload_individual"><?php esc_html_e('Passport', 'dokan'); ?> <span class="required">(Required)</span></label>
                    <small><?php esc_html_e('Upload Scanned/valid copes of your Passport. We will review all information provided and store privately according to our Privacy Policy.', 'dokan'); ?></small> <br>
                    <p><?php esc_html_e('Files:', 'dokan'); ?></p>
                    <div class="custom-file-upload">
                        <input type="file" name="passport_upload_individual[]" id="passport_upload_individual" accept=".pdf,.doc,.docx,.jpeg,.jpg,.png" multiple hidden>
                        <label for="passport_upload_individual" type="button" id="upload_passport_button_individual" class="dokan-btn dokan-btn-theme passport_upload_button"><i class="fas fa-cloud-arrow-up"></i>&nbsp; <?php esc_html_e('Upload Files', 'dokan'); ?></label>
                    </div>
        ...
        <div id="business_fields" style="display:none;">
            ...
            <p class="form-row form-row-wide half-width">
                <input class="tc_check_box" type="checkbox" id="product_liability_insurance" name="product_liability_insurance">
            ...
            <div class="form-row form-row-wide half-width passport_upload">
                <label for="passport_upload_business"><?php esc_html_e('Passport', 'dokan'); ?> <span class="required">(Required)</span></label>

                <small><?php esc_html_e('Upload Scanned/valid copes of all Owners/Partners/Directors related to your Company. We will review all information provided and store privately according to our Privacy Policy.', 'dokan'); ?></small> <br>
                <p><?php esc_html_e('Files:', 'dokan'); ?></p>
                <div class="custom-file-upload">
                    <input type="file" name="passport_upload_business" id="passport_upload_business" accept=".pdf,.doc,.docx,.jpeg,.jpg,.png" multiple hidden>
                    <button type="button" id="upload_passport_button_business" class="dokan-btn dokan-btn-theme passport_upload_button"><i class="fas fa-cloud-arrow-up"></i>&nbsp; <?php esc_html_e('Upload Files', 'dokan'); ?></button>
                </div>
            </div>
            ...
            <script>
                jQuery(document).ready(function($) {
                    ...
                    $(document).ready(function() {
                        ...
                        handleFileUpload('#passport_upload_individual', 2);
                    });
        ...
        <div id="branded_product_fields" style="display: none;">
            ...
            <p class="form-row form-row-wide brandedproduct_fields" style="display: none;">
                ...
                <input type="file" class="input-text" name="brand_ownership_proof[]" id="brand_ownership_proof" multiple accept=".pdf, .jpeg, .png, .doc, .docx" />
                <span class="error-message" style="color: red; display: none;"></span>
            </p>
            <?php
...
function dokan_custom_new_seller_created($vendor_id, $dokan_settings)
{
    ...
    if (isset($_FILES['passport_upload_individual']) && !empty($_FILES['passport_upload_individual']['name'])) {
        // Get the uploaded file details


        $uploaded_file = $_FILES['passport_upload_individual'];

        // Get the upload directory path using WordPress function
        $upload_dir = wp_upload_dir();
        $target_dir = $upload_dir['basedir'] . '/passport_upload_individual/'; // Folder to store the files
        ...
        $timestamp = time();
        // Ensure that $uploaded_file is an array, and that the 'name' key exists.
        // if (is_array($uploaded_file) && isset($uploaded_file['name'])) {
        //     $file_extension = pathinfo($uploaded_file['name'], PATHINFO_EXTENSION); // Get the file extension
        //     // Now you can use $file_extension safely.
        //     // ... your other code ...
        ...
        //             $passport_upload_individual_registration_proof_url = $upload_dir['baseurl'] . '/passport_upload_individual/' . $new_file_name;

        //             // Save the URL of the uploaded file to user meta
        //             update_user_meta($vendor_id, 'dokan_custom_passport_upload_individual', $passport_upload_individual_registration_proof_url);
        //         } else {
        ...
        //     } else {
        //         // Handle the case where $uploaded_file is not a valid file array.
        //         error_log("Invalid file upload data: " . print_r($uploaded_file, true));
...
function dokan_custom_vendor_dashboard_fields($vendor)
{
    ...
    $business_registration_proof_url = get_user_meta($vendor_id, 'dokan_custom_business_registration_proof', true);
        ...
        <tr>
            ...
            <td><select name="account_type" id="account_type">
                    <option value="individual"
                        <?php echo (esc_attr($account_type) && $account_type == 'individual') ? "selected" : ''; ?>>
                        <?php _e('Individual', 'woocommerce'); ?></option>
                    <option value="business"
                        <?php echo (esc_attr($account_type) && $account_type == 'business') ? "selected" : ''; ?>>
                        <?php _e('Business', 'woocommerce'); ?></option>
                </select></td>
        </tr>
        <tr>
    ...
    ?>
        ...
        <table class="form-table">
            ...
            <tr id="selling_unbranded_products_row">
                ...
                <td>
                    <input type="radio" name="selling_branded_products" value="unbranded_products"
                        id="selling_unbranded_products" style="margin-right: 10px;"
                        <?php checked($selling_branded_products, 'unbranded_products'); ?> />
                    <label
...
?>
    ...
    <div class="dokan-form-group">
        ...
        <p style="font-size: 12px; color: #ff9800; margin-top: 5px;">
            <strong>Note:</strong> If the brand name is not registered in Brags Brand Network, processing times may be longer.
            We strongly recommend ensuring the brand owner has registered their trademark in the Network and granted authorization for your business.
    ...
    <p class="form-row form-group" id="brags-brand-network" style="display: none;">
        ...
        <span>
            <label for="brags_brand_network_intend" style="width: fit-content;">
                <input type="checkbox" name="brags_brand_network[]" id="brags_brand_network_intend" value="Not yet, but I intend to register once I’m approved to sell on Brags.co.uk">
...
?>
    ...
    <table class="form-table">
        <tr>
            ...
            <td>
                <input type="text" name="brags_brand_network" id="brags_brand_network" value="<?php echo esc_attr($brags_brand_network); ?>" class="regular-text" disabled>
            </td>
        </tr>
    </table>
<?php }
...
function bragsy_add_otp_modal_markup() {
    if ( is_account_page() ) {
        ...
        <style>
            ...
            #otp-popup input { width: 90%; padding: 8px; }
            #otp-popup button { margin-top: 10px; padding: 10px 20px; background: #0073aa; color: #fff; border: none; }
        </style>
        <?php
    }
}
...
function bragsy_enqueue_otp_script() {
    if ( is_account_page() ) {
        wp_enqueue_script( 'bragsy-otp-handler', get_stylesheet_directory_uri() . '/assets/js/otp-handler.min.js', ['jquery'], null, true );
