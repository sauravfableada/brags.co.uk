...
if ( ! function_exists( 'woodmart_register_required_plugins' ) ) {
	...
	function woodmart_register_required_plugins() {
		...
		if ( ( ( ! isset( $_GET['page'] ) || ! in_array( $_GET['page'], array( 'tgmpa-install-plugins', 'xts_plugins' ), true ) ) && ( ! isset( $_POST['action'] ) || ! in_array( $_POST['action'], array( 'woodmart_deactivate_plugin', 'woodmart_check_plugins' ), true ) ) ) && ! class_exists( 'RevSliderSlider' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing
			$plugins = array_filter(
				$plugins,
				function ( $plugin ) {
