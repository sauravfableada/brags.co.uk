 ...
 */
if ( get_option( 'wpla_activated_key' ) != '1' ) {
    add_action( 'admin_notices', 'WPLA_Update_API::license_inactive_notice' );
}

class WPLA_Update_API {
	 ...
	 */
	public $text_domain = 'wp-lister-for-amazon';
	...
	public function __clone() {
		_doing_it_wrong( __FUNCTION__, __( 'Cheatin&#8217; huh?', 'wp-lister-for-amazon' ), '1.2' );
	}
	...
	public function __construct() {
		...
		if ( is_admin() ) {
			 ...
			 */
			$this->ame_software_product_id = 'WP-Lister Pro for Amazon';

			/**
			 * Set all data defaults here
			 */
			$this->ame_data_key 				= 'wpla_options';
			$this->ame_api_key 					= 'wpla_api_key';
			$this->ame_activation_email 		= 'wpla_activation_email';
			$this->ame_product_id_key 			= 'wpla_product_id';
			$this->ame_instance_key 			= 'wpla_instance';
			$this->ame_activated_key 			= 'wpla_activated_key';
			// $this->ame_deactivate_checkbox_key 	= 'wpla_deactivate_checkbox';

			/**
			 * Set all admin menu data
			 */
			$this->ame_deactivate_checkbox 			= 'am_deactivate_example_checkbox';
			$this->ame_activation_tab_key 			= 'wpla_dashboard';
			$this->ame_deactivation_tab_key 		= 'wpla_deactivation';
			$this->ame_settings_menu_title 			= 'WP-Lister for Amazon';
			$this->ame_settings_title 				= 'WP-Lister for Amazon';
			$this->ame_menu_tab_activation_title 	= 'License Activation';
			$this->ame_menu_tab_deactivation_title 	= 'License Deactivation';

			/**
			 * Set all software update data here
			 */
			$this->ame_options 				= get_option( $this->ame_data_key );
			$this->ame_plugin_name 			= 'wp-lister-amazon/wp-lister-amazon.php'; // same as plugin slug. if a theme use a theme name like 'twentyeleven'
			...
			$this->license_key 				= wpla_get_license_key(); 		 // API key
			$this->license_email 			= wpla_get_license_email(); // API email
			$this->update_channel 			= get_option( 'wpla_update_channel' ); 
	 ...
	 * Update Check Class.
	 *
	 * @return WPLA_Updater_Update_API_Check
	 */
	public function update_check( $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra = '' ) {
		...
		return WPLA_Updater_Update_API_Check::instance( $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra );
	}

	public function check_for_new_version( $quiet = false ) {

		$this->license_key   = wpla_get_license_key();
		$this->license_email = wpla_get_license_email();
		...
		if ( is_object( $response ) && !empty( $response ) && $response->new_version ) {
			...
			$response->title        = 'WP-Lister for Amazon';
			// $response->upgrade_html = '';
			update_option( 'wpla_update_details', $response, false );

			return $response;
		} else {

			// empty result means no new version
			$update = get_option( 'wpla_update_details', new stdClass() );
			$update->timestamp    = time();
			$update->new_version  = WPLA_VERSION;
			$update->title        = 'WP-Lister for Amazon';
			$update->upgrade_html = '';
			update_option( 'wpla_update_details', $update, false );
	...
	public function on_plugin_activation() {
		...
		$wpla_password_management = new WPLA_Updater_Password_Management();

		// Generate a unique installation $instance id
		$instance = $wpla_password_management->generate_password( 12, false );
		...
		// $curr_ver = get_option( $this->wpla_version_name );

		// // checks if the current plugin version is lower than the version being installed
		// if ( version_compare( $this->version, $curr_ver, '>' ) ) {
		// 	// update the version
		// 	update_option( $this->wpla_version_name, $this->version );
		// }
	...
	public static function license_inactive_notice() { 
		...
		if ( isset( $_GET['page'] ) && 'wpla' != substr($_GET['page'],0,4) ) return;
		if ( isset( $_GET['tab'] )  && 'license' == $_GET['tab'] ) return;
		?>
		<div id="message" class="error">
			<p><?php printf( __( 'The WP-Lister Pro for Amazon license key has not been activated, so you won\'t be able to get updates and support! %sClick here%s to activate the license key and the plugin.', 'wp-lister-for-amazon' ), '<a href="' . esc_url( admin_url( 'admin.php?page=wpla-settings&tab=license' ) ) . '">', '</a>' ); ?></p>
		</div>
		<?php
	}
	...
	public function check_external_blocking() {
		...
		if( defined( 'WP_HTTP_BLOCK_EXTERNAL' ) && WP_HTTP_BLOCK_EXTERNAL === true ) {
			...
			if( ! defined( 'WP_ACCESSIBLE_HOSTS' ) || stristr( WP_ACCESSIBLE_HOSTS, $host ) === false ) {
				?>
				<div class="error">
					<p><?php printf( __( '<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %s updates. Please add %s to %s.', 'wp-lister-for-amazon' ), $this->ame_software_product_id, '<strong>' . $host . '</strong>', '<code>WP_ACCESSIBLE_HOSTS</code>'); ?></p>
				</div>
				<?php
			}
...
function WPLAUP() {
    return WPLA_Update_API::instance();
}
