...
class WPLA_Updater_Update_API_Check {
	...
	/**
	 * Check for updates against the remote server - and return response
	 */
	public function check_for_new_version( $quiet = false ) {
		...
		if ( ! $quiet ) $this->check_response_for_errors( $response );
	...
	public function update_check( $transient ) {
		...
		# This is preventing some sites from getting notified of new versions
		/*if ( empty( $transient->checked ) ) {
			return $transient;
		}*/

		$update_check = apply_filters( 'wpla_run_plugin_update_check', true );
	...
	public function plugin_information( $args ) {
		...
		// there is at least one hoster (pantheon.io) where this does not work via HTTP GET...
		// $target_url = $this->create_upgrade_api_url( $args );
		// $request    = wp_remote_get( $target_url );

		// send request via HTTP POST
		// $api_url = add_query_arg( 'wc-api', 'upgrade-api', WPLAUP()->upgrade_url );
		...
		if ( is_wp_error( $request ) || wp_remote_retrieve_response_code( $request ) != 200 ) {
		    $message = 'wp_remote_post() failed to connect to '.$api_url;

		    if ( is_wp_error( $request ) ) {
		        $message .= '<br>Error: '. $request->get_error_message();
            }
	...
	public function request( $false, $action, $args ) {
		...
		if ( isset( $args->slug ) ) {
			...
			// Check if this plugins API is about this plugin
			if ( $args->slug != dirname( $this->plugin_name ) ) {
				return $false;
			}
	...
	public function expired_license_error_notice( $message ){
		...
		echo sprintf( '<div id="message" class="error"><p>' . __( 'The license key for %s has expired. You can renew your license key from your <a href="%s" target="_blank">account dashboard</a>.', $this->text_domain ) . '</p></div>', $plugin_name, $this->renew_license_url ) ;
