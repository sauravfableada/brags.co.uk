...
<script type="text/javascript">
    ...
    function updateLog() {
        ...
        wp_ajax_url = 'admin-ajax.php?action=wpla_tail_log&ajax=1' + '&_wpnonce=' + '<?php echo wp_create_nonce( 'wpla_tail_log' ) ?>';
        $.getJSON( wp_ajax_url + '&file=' + lastFile + '&lastsize=' + lastSize + '&grep=' + grep + '&invert=' + invert, function(data) {
            lastSize = data.size;
            $("#current").text(data.file + ' (' + Math.round(lastSize/1024) + 'kb)' );
    ...
    <div class="navbar navbar-default navbar-fixed-top" role="navigation">
        <div class="container">
            <div class="navbar-header">
                <a class="navbar-brand">Log Viewer</a>
            </div>
