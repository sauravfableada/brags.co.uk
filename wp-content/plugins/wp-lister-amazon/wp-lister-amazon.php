...
define('WPLA_VERSION', '2.9.2.1' );
define('WPLA_DB_VERSION', 70 );
...
if ( get_option('wpla_php_error_handling') == '9' ) error_reporting( E_ERROR );
...
class WPLA_WPLister extends WPLA_BasePlugin {
    ...
    ## END PRO ##
    public $minmax_wiz;
    public $woo_product_wiz;
    public $woo_product_attr;
    public $woo_shipping;
    
    /** @var \WPLab\Amazon\Helper\FbaStockMonitor */
    public $fba_stock_monitor;

    /* @var WPLA_AmazonAccount[] */
    var $accounts      = array();
	...
	public function initClasses() {
		...
		$this->fba_stock_monitor = new \WPLab\Amazon\Helper\FbaStockMonitor();
...
//		load_plugin_textdomain( 'wp-lister-for-amazon', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
	...
	public function onWpAdminInit() {
        ...
        WPLA_Setup::displayAnnouncements();
		...
		if ( $this->isProductsPage() ) {
			...
			add_action( 'admin_footer', array( &$this, 'modifyProductsBulkActionMenu2' ), 20 );
	...
	public function onWpPrintStyles() {
		...
		if  ( ( isset( $_GET['page'] ) ) && ( substr( $_GET['page'], 0, 4 ) == 'wpla') ) {
			wp_register_style( 'wpla_style', self::$PLUGIN_URL.'css/style.css', array(), WPLA_VERSION );
			wp_enqueue_style( 'wpla_style' );
		}

        $screen = get_current_screen();
		if ( $screen && $screen->id == 'edit-product' ) {
            wp_enqueue_style( 'wpla_flag_icons', self::$PLUGIN_URL .'css/flag-icon.css' );
        }
	}
	...
	public function modifyProductsBulkActionMenu() {
		...
		<script type="text/javascript">
    	    jQuery(document).ready(function() {
        	    jQuery("<option>").val("list_on_amazon").text("<?php echo esc_js( __( 'List on Amazon', 'wp-lister-for-amazon' ) ); ?>").appendTo("select[name='action']");
            	jQuery("<option>").val("list_on_amazon").text("<?php echo esc_js( __( 'List on Amazon', 'wp-lister-for-amazon' ) ); ?>").appendTo("select[name='action2']");
	        });
		</script>
		<?php
	}
	...
	public function modifyProductsBulkActionMenu2() {
		...
		<script type="text/javascript">
    	    jQuery(document).ready(function() {
        	    jQuery("<option>").val("remove_from_amazon").text("<?php echo esc_js( __( 'Remove from Amazon', 'wp-lister-for-amazon' ) ); ?>").appendTo("select[name='action']");
            	jQuery("<option>").val("remove_from_amazon").text("<?php echo esc_js( __( 'Remove from Amazon', 'wp-lister-for-amazon' ) ); ?>").appendTo("select[name='action2']");
	        });
		    ...
		    jQuery(".tablenav .actions input[type='submit'].action").on("click", function() {
				...
				if ( selected_action == "remove_from_amazon" ) {
					var confirmed = confirm("<?php echo esc_js( __( 'Are you sure you want to do this?', 'wp-lister-for-amazon' ) ) . ' ' . esc_js( __('Removing the listing also removes the sales history for the item. If you were to relist these listings later you would then start out with a lower sales rank.', 'wp-lister-for-amazon' ) ); ?>");
	...
	public function printProductsPageStyles() {	
		?>
    	<style type="text/css">
			table.wp-list-table .column-listed_on_amazon { width: 45px; }

            th#listed_on_amazon span.listed_on_amazon_col {
                display: none;
            }
    	</style>
    	<?php
	...
	public function onWpPrintScripts() {
		...
		// PriceMatcher
		wp_register_script( 'wpla_price_matcher', self::$PLUGIN_URL.'js/classes/PriceMatcher.js', array( 'jquery' ), WPLA_VERSION );
	...
	public function printProductsPageScripts() {
		...
		// ProductMatcher
		wp_register_script( 'wpla_product_matcher', self::$PLUGIN_URL.'js/classes/ProductMatcher.js', array( 'jquery' ), WPLA_VERSION );
...
// Run DB migrations on every request (including cron) so the schema is always current
// immediately after a plugin update, without requiring an admin page visit first.
add_action( 'plugins_loaded', function() {
    if ( (int) get_option( 'wpla_db_version', 0 ) < WPLA_DB_VERSION ) {
        WPLA_UpgradeHelper::upgradeDB();
    }
}, 1 );

// Declare support for WCs Custom Order Tables because this plugin does not work with WC order data
