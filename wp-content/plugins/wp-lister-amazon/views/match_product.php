    ...
    <!-- <h3>Matching products for <i><?php #echo $wpl_query ?></i> -->
        ...
        <form id="wpla_matcher_query_form" action="#">
            ...
            <select id="wpla_matcher_query_select" name="query_select" class="select">
                ...
                <option value="ean"   data-value="<?php echo htmlspecialchars( $wpl_ean_value ); ?>" <?php selected( 'ean', $wpl_query_select ); ?> ><?php echo __( 'EAN', 'wp-lister-for-amazon' );   ?></option>
                <?php
                foreach ( apply_filters( 'wpla_product_matches_query_fields', $wpl_query_product_attributes ) as $attribute_label => $attribute_value) :
                    if (is_array( $attribute_value )) continue;
                ?>
