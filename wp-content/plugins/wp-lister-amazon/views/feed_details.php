...
<body class="wp-core-ui">
    ...
    <div class="tab-content wpla_tab_feed_content_wrapper">
        ...
        <h2 style="margin-top:0;"><?php echo esc_html( $wpl_feed->getRecordTypeName( $wpl_feed->FeedType ) ); ?> - Batch ID <?php echo esc_html( $wpl_feed->FeedSubmissionId ); ?> <!--(<?php echo esc_html( $wpl_feed->id ); ?>)--></h2>
        ...
        <table>
            ...
            <tr><td>
                ...
                : &nbsp; <?php echo esc_html( $wpl_feed->template_name ); ?>
                <?php if ( $wpl_feed_version ) : ?>
                    - version <?php echo esc_html( $wpl_feed_version ); ?>
                <?php endif; ?>
                <?php if ( $wpl_feed_category ) : ?>
                    (<?php echo esc_html( $wpl_feed_category ); ?>)
                <?php endif; ?>
                <?php if ( 'fpt' == substr( $wpl_feed->template_name, 0, 3 ) && $wpl_feed_signature == '' ) : ?>
                    <span style="color:darkred;">Template signature is missing. Please update your feed templates!</span>
                <?php endif; ?>
            </td></tr>
            <?php endif; ?>
            <tr><td>
                ...
                : &nbsp; <?php echo esc_html( $wpl_feed->FeedType ); ?>
            </td></tr>
            <?php if ( $wpl_feed->SubmittedDate ) : ?>
            <tr><td>
                Submitted at
            </td><td>
                : &nbsp; <?php echo esc_html( $wpl_feed->SubmittedDate ); ?>
                ( <?php echo esc_html( human_time_diff( strtotime($wpl_feed->SubmittedDate.' UTC') ) ); ?> ago )
            </td></tr>
            <?php endif; ?>
        </table>
        <br>
        ...
        <!-- <h3>CSV Data</h3> -->
        <?php if ( $wpl_feed->FeedType == 'UPLOAD_VAT_INVOICE' ): ?>
        <pre>PDF Invoice. No data to display</pre>
        <?php elseif ( is_array($wpl_rows) && ( sizeof($wpl_rows)>0 ) ) : ?>
        <table id="wpla_feed_data_table" class="csv-table">
            <thead>
            <tr>
                <?php foreach ($wpl_rows[0] as $key => $value) : ?>
                    <th class="wpla_csv-<?php echo esc_attr( $key ); ?>"><?php echo esc_html( $key ); ?></th>
                <?php endforeach; ?>
            </tr>
            </thead>
            <tbody>
            ...
            <tr>
                <?php foreach ($row as $key => $value) : ?>
                    ...
                    <td class="wpla_csv-<?php echo esc_attr( $key ); ?>"
        ...
        <a href="<?php echo esc_url( $feed_permalink ); ?>" class="button">Permalink</a> &nbsp;
        <a href="<?php echo esc_url( admin_url('admin.php?page=wpla-feeds&action=view_amazon_feed_details_raw&amazon_feed=' . $wpl_feed->id . '&_wpnonce=' . wp_create_nonce( 'wpla_view_feed_details_raw' )) ); ?>" class="button" target="_blank">View raw feed</a> &nbsp;
        ...
        <?php if ( is_array($wpl_result_rows) && ( sizeof($wpl_result_rows)>0 ) ) : ?>
            ...
            <?php if ( $is_localized_result ) : ?>
                <div id="message" class="error">
                    <p>
                        <b><?php echo __( 'This report seems to use localized column headers and can not be processed.', 'wp-lister-for-amazon' ) ?></b>
                    </p>
                    <p>
    ...
    <div class="tab-content wpla_tab_feed_support_wrapper" style="display:none;">
        ...
        <div id="support_request_wrap" style="">
            <form method="post" action="<?php echo esc_url( $_SERVER['REQUEST_URI'] ); ?>" >
                ...
                <input type="hidden" name="feed_id" value="<?php echo esc_attr( $wpl_feed->id ); ?>" />
