...
<body>
	...
	<?php if ( ( ! isset($_REQUEST['send_to_support']) ) && ( ! isset($_REQUEST['new_tab']) ) ) : ?>
		<div id="support_request_wrap" style="">
			<form method="post" action="<?php echo esc_url( $_SERVER['REQUEST_URI'] ); ?>" target="_blank" >
                <?php wp_nonce_field( 'wpla_send_to_support' ); ?>
				<input type="hidden" name="log_id" value="<?php echo esc_attr( $wpl_row->id ); ?>" />
				...
				<input type="text" name="user_email" value="<?php echo esc_attr( get_bloginfo( 'admin_email' ) ); ?>" class="text-input"/>
		...
		<div style="float:right;margin-top:10px;">
			...
			<a href="<?php echo esc_url( $_SERVER['REQUEST_URI'] . '&new_tab=yes' ); ?>" target="_blank" class="button-secondary">Open in new tab</a>
		</div>
	<?php endif; ?>

    <h2>Call: <?php echo esc_html( $wpl_row->callname ); ?> (#<?php echo esc_html( $wpl_row->id ); ?>)</h2>

    <!-- <h3>Request URL</h3> -->
    <!-- <pre><?php echo $url ?></pre> -->

    <h3>Request</h3>
    <pre><?php echo esc_html( str_replace( '.wplab.com','', $req ) ); ?></pre>
    ...
    <h3>Debug Info</h3>
    <pre>
    	WP-Lister: <?php echo esc_html( $wpl_version ); ?> 
        WC       : <?php echo esc_html( WC_VERSION ); ?>        
    	DB       : <?php echo esc_html( get_option('wpla_db_version') ); ?>
    	...
    	Charset  : <?php echo esc_html( get_bloginfo( 'charset' ) ); ?>

    	Site URL : <?php echo esc_url( get_bloginfo( 'wpurl' ) ); ?>
    	
    	Admin    : <?php echo esc_html( get_bloginfo( 'admin_email' ) ); ?>

    	Email    : <?php echo esc_html( get_option('wpla_license_email') ); ?>
    </pre>
