<?php
	...
	$builtin_shortcodes = array(
		...
		'[product_brand]'          => 'WooCommerce Brand',
	...
	foreach (WPLA()->getShortcodes() as $key => $custom_shortcode) {
		$builtin_shortcodes[ "[$key]" ] = $custom_shortcode['title'];
	}
	...
	foreach ( $variation_meta_fields as $key => $varmeta ) {
		$key                            = 'meta_'.$key;
		$builtin_shortcodes[ "[$key]" ] = $varmeta['label'];
	}
		...
		<table style="width:100%;">
			...
			<tr>
				<td style="width:50%; vertical-align:top;">
					<?php foreach ( $builtin_shortcodes as $shortcode => $title ) : ?>
