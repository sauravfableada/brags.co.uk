...
<div class="wrap wpla-page">
	...
	<h2><?php echo __( 'Edit Profile', 'wp-lister-for-amazon' ) ?></h2>
	<?php else: ?>
	<h2><?php echo __( 'New Profile', 'wp-lister-for-amazon' ) ?></h2>
	<?php endif; ?>
			...
			<div id="postbox-container-2" class="postbox-container">
				<div class="meta-box-sortables ui-sortable">
					...
					<div class="postbox" id="GeneralSettingsBox">
						<h3 class="hndle"><span><?php echo __( 'General', 'wp-lister-for-amazon' ); ?></span></h3>
						<div class="inside">

							<div id="titlediv" style="margin-bottom:5px;">
								<div id="titlewrap">
									<label class="text_label"><?php echo __( 'Profile name', 'wp-lister-for-amazon' ); ?> *</label>
									<input type="text" name="wpla_profile_name" size="30" value="<?php echo esc_attr($wpl_profile->profile_name); ?>" id="title" autocomplete="off" style="width:65%;">
								</div>
							</div>

							<label for="wpl-text-profile_description" class="text_label">
								<?php echo __( 'Profile description', 'wp-lister-for-amazon' ); ?>
                                <?php wpla_tooltip('A profile description is optional and only used within WP-Lister.') ?>
							</label>
							<input type="text" name="wpla_profile_description" id="wpl-text-profile_description" value="<?php echo esc_attr( $wpl_profile->profile_description ); ?>" class="text_input" />
							<br class="clear" />
					...
					<div class="submit" style="padding-top: 0; float: right; display:none;">
						<input type="submit" value="<?php echo __( 'Save profile', 'wp-lister-for-amazon' ); ?>" name="submit" class="button-primary">
					</div>
	...
	<script type="text/javascript">
		...
		jQuery( document ).ready(
			function () {
				...
				jQuery('.wpla-page form').on('submit', function() {
					...
					// prevent submission if feed attributes are still loading
					if (typeof window.wpla_is_loading === 'function' && window.wpla_is_loading()) {
						alert('Please wait for feed attributes to finish loading before saving the profile.');
						return false;
					}
					...
					if (missing_required_fields) {
						// disabled because there are too many fields incorrectly tagged as required, like lens_type in Health UK
				...
				// Amazon categories tree - lazy initialization for BTG selection
				window.wpla_init_categories_tree = function() {
					// Only initialize if not already initialized
					if (jQuery('#amazon_categories_tree_container').hasClass('wpla-tree-initialized')) {
						return;
					}

					jQuery('#amazon_categories_tree_container').addClass('wpla-tree-initialized');
