...
class WPLA_ListingQueryHelper {
    ...
    // Only process online listings - repricing should not apply to prepared/changed listings
    ...
    static function getAllListingIDFromPostIDOrParentID( $post_id ) {
        global $wpdb;
        $table = $wpdb->prefix . self::TABLENAME;
        ...
        return $wpdb->get_col( $wpdb->prepare("
			SELECT id
			FROM $table
			WHERE ( post_id = %s OR parent_id = %s )
            AND status <> 'trash'
			ORDER BY id DESC
		", $post_id, $post_id ) );
    }
