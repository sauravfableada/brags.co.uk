...
//        }
	...
	public function parseVariationChildNodes( $product_node, $parent_listing, $account ) {
		...
		foreach ( $relationships->getChildAsins() as $child_asin ) {
			...
			$variation_theme = $relationships->getVariationTheme();
			$variation_theme_attributes = $variation_theme ? $variation_theme->getAttributes() : [];
			foreach ( $variation_theme_attributes as $attribute ) {
				$attribute_value = '';
				if ( ! empty( $listing_attributes[ $attribute ] ) ) {
					$raw_value = $listing_attributes[ $attribute ];
					// Ensure the value is a string for use in implode() later
					if ( is_scalar( $raw_value ) ) {
						$attribute_value = (string) $raw_value;
					} elseif ( is_array( $raw_value ) ) {
						$attribute_value = implode( ', ', array_filter( $raw_value, 'is_scalar' ) );
					} elseif ( is_object( $raw_value ) ) {
						if ( isset( $raw_value->value ) ) {
							$attribute_value = (string) $raw_value->value;
						} elseif ( isset( $raw_value->name ) ) {
							$attribute_value = (string) $raw_value->name;
						}
					}
				}
				...
				$newvar->vtheme[] = $variation_theme->getTheme();
				$newvar->attribute_values[] = $attribute_value;
			}
			$newvar->vtheme = implode( '-', $newvar->vtheme );
	...
	public function insertVariationListing( $var, $product_node, $parent_listing, $account ) {
		...
		$attribute_values = isset( $var->attribute_values ) && is_array( $var->attribute_values ) ? $var->attribute_values : array();
		$suffix = implode( ', ', $attribute_values );
