...
class WPLA_ImportHelper {
	...
	public static function ajax_processReportPage( $job, $task, $single_sku_mode = false ) {
        ...
        switch ( $report->ReportType ) {
            ...
            case 'GET_FBA_FULFILLMENT_INVENTORY_HEALTH_DATA':
        ...
        WPLA()->logger->info( 'Unknown report type: '. $report->ReportType );
	...
	public static function processInventoryReportPage( $report, $rows, $job, $task ) {
		...
		// Adding for #46851
		do_action( 'wpla_after_processing_inventory_report', $report, $rows, $response );
		...
		foreach ($rows as $row) {
			...
			if ( $listing_item = $listingsModel->getItemBySKU( $sku ) ) {
				...
				if ( $fba_enable_fallback && $listing_item->fba_fcid != $fba_default_fcid ) {
					...
					// Also queue for proper FBA - FBM conversion via offer deletion/recreation
					if ( $fba_quantity == 0 && $listing_item->quantity > 0 ) {
						do_action('wpla_fba_fallback_detected', $listing_item, $sku, $account_id);
					}
				}
    ...
    public static function processManageFBAReportPage( $report, $rows, $job, $task ) {
        ...
        // Log FBA processing settings
        WPLA()->logger->info("Processing FBA Manage Inventory Report - Settings: fallback_enabled=$fba_enable_fallback, only_mode=$fba_only_mode, stock_sync=$fba_stock_sync, is_first_page=$is_first_page");

        if ( $fba_enable_fallback && $is_first_page ) {
            WPLA()->logger->info("FBA fallback enabled - clearing FBA data for all items in account $account_id");
            // reset FBA info for all items using this account
            $update_data = array(
                'fba_quantity' => null,
                'fba_fcid'     => null,
        ...
        if ( is_array($rows) )
            foreach ($rows as $row) {
                ...
                // check for the afn-fulfillable-quantity-local and remote data #54953
                if ( isset( $row['afn-fulfillable-quantity-local'] ) ) {
                    $fba_quantity  = $row['afn-fulfillable-quantity-local'];
                    WPLA()->logger->info("SKU $sku: Using local FBA quantity: {$row['afn-fulfillable-quantity-local']}");

                    if ( isset( $row['afn-fulfillable-quantity-remote'] ) ) {
                        $fba_quantity += intval( $row['afn-fulfillable-quantity-remote'] );
                        WPLA()->logger->info("SKU $sku: Adding remote FBA quantity: {$row['afn-fulfillable-quantity-remote']}, total: $fba_quantity");
                    }
                } else {
                    $fba_quantity  = $row['afn-fulfillable-quantity'];
                    WPLA()->logger->info("SKU $sku: Using Fulfillable FBA quantity field: $fba_quantity");
                }
                ...
                if ( ( empty( $row['afn-listing-exists'] ) || strtolower( $row['afn-listing-exists'] ) != 'yes' ) && $fba_quantity == 0 ) {
                    WPLA()->logger->info("SKU $sku: Skipping non-FBA listing (afn-listing-exists: {$row['afn-listing-exists']}, FBA quantity: $fba_quantity)");
                    continue;
                }
                ...
                if ( $listing_item = $listingsModel->getItemBySKU( $sku ) ) {
                    ...
                    if ( $fba_quantity == 0 && $fba_enable_fallback && !$fba_only_mode && !$fba_stock_sync ) {
                        WPLA()->logger->info("SKU $sku: FBA fallback triggered - zero FBA quantity (prev_fba: $prev_fba_quantity, woo_qty: {$listing_item->quantity}, current_fcid: '{$listing_item->fba_fcid}', default_fcid: '$fba_default_fcid')");
                        if ( !is_null( $prev_fba_quantity ) && $prev_fba_quantity > 0 && $listing_item->quantity == 0 ) {
                            ...
                            WPLA()->logger->info("SKU $sku: FBA ran out of stock - updating WooCommerce product to out of stock");
                            $wc_product = wc_get_product( $listing_item->post_id );

                            if ( $wc_product ) {
                                $wc_product->set_stock_quantity( 0 );
                                $wc_product->save();
                                WPLA()->logger->info("SKU $sku: WooCommerce stock updated to 0");
                            }
                        }
                        
                        // Check if this should trigger FBA-to-FBM conversion
                        if ( $listing_item->quantity > 0 ) {
                            WPLA()->logger->info("SKU $sku: Zero FBA quantity with WC stock - triggering conversion action");
                            do_action('wpla_fba_fallback_detected', $listing_item, $sku, $account_id);
                        }
                        continue;
                    }
                    ...
                    // Log FCID values for debugging
                    WPLA()->logger->info("SKU $sku: FCID comparison - current: '{$listing_item->fba_fcid}', default: '$fba_default_fcid', fallback_enabled: $fba_enable_fallback");
                    
                    // mark item as changed - if FBA fallback is enabled and Fulfillment Center ID has been changed
                    if ( $fba_enable_fallback && $listing_item->fba_fcid != $fba_default_fcid ) {
                        WPLA()->logger->info("SKU $sku: FBA fulfillment center changed - marking as changed (old: {$listing_item->fba_fcid}, new: $fba_default_fcid)");
                        $update_data['status'] = 'changed';
                        
                        // Also queue for proper FBA - FBM conversion via offer deletion/recreation
                        if ( $fba_quantity == 0 && $listing_item->quantity > 0 ) {
                            WPLA()->logger->info("SKU $sku: FBA-to-FBM conversion detected - triggering fallback action (FBA qty: 0, WooCommerce qty: {$listing_item->quantity})");
                            do_action('wpla_fba_fallback_detected', $listing_item, $sku, $account_id);
                        } else {
                            WPLA()->logger->info("SKU $sku: FCID changed but conversion conditions not met (FBA qty: $fba_quantity, WooCommerce qty: {$listing_item->quantity})");
                        }
                    } else {
                        WPLA()->logger->info("SKU $sku: No FCID change detected - no conversion action triggered");
                    }
                    ...
                    do_action( 'wpla_process_fba_report_row', $listing_item, $row );
    ...
    public static function processOrderDataReportPage( $report, $rows, $job, $task ) {

        // process rows
        foreach ($rows as $row) {
            if ( $row['is-business-order'] ) {
                $tax_id = $row['buyer-tax-registration-id'];
                $order = WPLA_OrdersModel::getWhere( 'order_id', $row['order-id'] );

                if ( $tax_id && $order ) {
                    if ( $order->post_id ) {
                        update_post_meta( $order->post_id, 'billing_vat', $tax_id );
                    }
                }
            }
