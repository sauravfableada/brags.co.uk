...
class AmazonSchemaFormGenerator {
	...
	private bool $inventory_loader_mode = false;
	private bool $offer_only_mode = false;
	...
	/**
	 * Set inventory loader mode for filtering fields
	 * @param bool $mode
	 */
	public function setInventoryLoaderMode( $mode ) {
		$this->inventory_loader_mode = $mode;
	}

	/**
	 * Set offer only mode for filtering fields
	 * @param bool $mode
	 */
	public function setOfferOnlyMode( $mode ) {
		$this->offer_only_mode = $mode;
	}
		...
		foreach ( (array)$properties as $prop_key => $property ) {
			...
			if ( $this->isRenderable( $property['type'], $property ) ) {
				if ( $details['type'] == 'array' ) {
					// Check for maxItems (standard JSON Schema) or maxUniqueItems (Amazon custom)
					// This handles quantity_discount_plan (maxItems) and bullet_point (maxUniqueItems)
					$maxItems = 1; // default
					if ( isset( $details['maxItems'] ) ) {
						$maxItems = $details['maxItems'];
					} elseif ( isset( $details['maxUniqueItems'] ) ) {
						$maxItems = $details['maxUniqueItems'];
					}
	...
	private function renderElement( $name, $property, $type, $value = '', $required = false, $custom_label = null ) {
		...
		// Filter for Inventory Loader mode - only show fields that are in getInventoryLoaderFields()
		if ( $this->inventory_loader_mode ) {
			$inventory_fields = array_keys( (new \WPLab\Amazon\Helper\JsonFeedDataBuilder())->getInventoryLoaderFields() );

			// Only render fields that exactly match inventory loader field names
			if ( !in_array( $name, $inventory_fields ) ) {
				return '';
			}
		}

		// Filter for Offer Only mode - only show fields that are in getOfferOnlyFields()
		if ( $this->offer_only_mode ) {
			$offer_fields = array_keys( (new \WPLab\Amazon\Helper\JsonFeedDataBuilder())->getOfferOnlyFields() );

			// Only render fields that exactly match offer only field names
			if ( !in_array( $name, $offer_fields ) ) {
				return '';
			}
		}
		...
		switch ( $type ) {
			...
			case 'array':
				...
				if ( ! empty( $enumOptions ) ) {
					...
					// Check if maxItems is 1, then render as single-select
					$maxItems = $property['maxItems'] ?? $property['items']['maxItems'] ?? null;
					$isMultiple = $maxItems !== 1;

					$html = $this->renderSelectbox($name, $property, $value, $isMultiple);
				}
				break;
		}
	...
	private function renderSelectbox( $name, $property, $value = '', $multiple = false ) {
		...
		if ( ! empty( $property['_enumOptions'] ) ) {
			// _enumOptions uses [label => enum_value] format; pass enum values as $keys
			$allowed_values = $property['_enumOptions'];
			$keys = array_values( $allowed_values );
	...
	private function buildOptionGroups( $allowed_values, $selected = '', $keys = [] ) {
		...
		// For new-format options, track whether a plain (non-composite) enum value has already been matched
		// to prevent duplicate selections when loading old stored values.
		$selected_is_composite = is_string( $selected ) && strpos( $selected, '|||' ) !== false;
		$old_enum_matched      = false;

        foreach ( $allowed_values as $key => $value ) {
			$selected_attr = '';
			if ( $key != "" && in_array( $key, $keys ) ) {
				// Old format: key = enum_value, value = label
			...
			} elseif ( $key != "" && in_array( $value, $keys ) ) {
				// New format: key = label, value = enum_value (from extractEnumOptionsFromSchema)
				// Use a composite "enum_value|||sanitized_label" as the option value so that options
				// with duplicate enum values remain uniquely selectable.
				$composite = $value . '|||' . sanitize_title( $key );
				if ( is_array( $selected ) ) {
					if ( in_array( $composite, $selected ) || ( ! $selected_is_composite && in_array( $value, $selected ) ) ) {
						$selected_attr = 'selected="selected"';
					}
				} elseif ( $composite === $selected ) {
					$selected_attr = 'selected="selected"';
				} elseif ( ! $selected_is_composite && ! $old_enum_matched && $value === $selected ) {
					// Backward compat: old stored value is a plain enum slug — select first match only.
					$selected_attr  = 'selected="selected"';
					$old_enum_matched = true;
				}
				$html .= '<option value="'. esc_attr( $composite ) .'" '. $selected_attr .'>'. esc_html( $key ) .'</option>';
			} else {
				$key = $value;
        ...
        if ( !empty( $selected ) && !is_array( $selected ) ) {
            ...
            // Handle composite enum values (enum_value|||sanitized_label) stored by new-format options.
            if ( $is_custom_value && strpos( $selected, '|||' ) !== false ) {
                $enum_part = explode( '|||', $selected, 2 )[0];
                if ( in_array( $enum_part, $allowed_values ) ) {
                    $is_custom_value = false;
                }
            }
	...
	/**
	 * Strips the sanitized label suffix appended by buildOptionGroups to make composite option values
	 * unique (format: "enum_value|||sanitized_label"). Returns the plain enum value.
	 * Non-composite values are returned unchanged.
	 *
	 * @param mixed $value
	 * @return mixed
	 */
	public static function stripEnumComposite( $value ) {
	 ...
	 * @return array    [label => value, …] or empty array if none found.
	 */
	 ...
	 *
	 * Returns [label => enum_value] pairs. Labels are unique category paths so they serve as
	 * safe array keys, and this format preserves all entries even when the same enum value
	 * appears multiple times (e.g. "polo-shirts" valid across Men/Women/Boys/Girls categories).
	 *
	 * @param array $schema The field schema
	 * @return array Array of [label => enum_value] or empty array if none found
	 */
	public static function extractEnumOptionsFromSchema( array $schema ) {
		...
		if ( isset( $schema['enum'] ) ) {
			...
			$options = [];
			foreach ( $schema['enum'] as $idx => $val ) {
				$options[ $labels[ $idx ] ?? $val ] = $val;
			}
			return $options;
		}
		...
		if ( isset( $schema['items']['enum'] ) ) {
			...
			$options = [];
			foreach ( $enum_values as $idx => $val ) {
				$options[ $enum_labels[ $idx ] ?? $val ] = $val;
			}
			return $options;
		}
