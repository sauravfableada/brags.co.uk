...
class ProfileProductTypeConverter {
	...
	public function convertFromArray( $fields ) {
		...
		// Fix corrupted fields that have nested arrays instead of flat field names
		$fields = $this->fixCorruptedFields( $fields );
	 ...
	 *
	 * @param string|array $value The display value from old feed template (string) or structured array from new format
	 * @return string|array The corresponding value for the new system, preserving input data type
	 */
	private function convertSupplierDeclaredDangerousGoods( $value ) {
		...
		// Handle array values (new JSON feed format)
		if ( is_array( $value ) ) {
			$converted_array = [];
			foreach ( $value as $key => $item ) {
				if ( is_array( $item ) && isset( $item['value'] ) ) {
					// Convert the value within the array structure
					$converted_array[$key] = $item;
					$converted_array[$key]['value'] = strtolower( str_replace( ' ', '_', $item['value'] ) );
				} else {
					// Handle simple array elements
					$converted_array[$key] = is_string( $item ) ? strtolower( str_replace( ' ', '_', $item ) ) : $item;
				}
			}
			return $converted_array;
		}

		// Handle string values (old format) - ensure we have a string
		if ( is_string( $value ) ) {
			return strtolower( str_replace( ' ', '_', $value ) );
		}

		// Return unchanged if not string or array
		return $value;
	}
	...
	private function convertShippingGroupValue( $display_value ) {
		...
		// extractEnumOptionsFromSchema returns [label => enum_value]
		foreach ( $allowed_values as $display_name => $id ) {
	...
	private function convertCountryOfOriginValue( $country_value ) {
		...
		// Try exact match against display names
		// extractEnumOptionsFromSchema returns [label => enum_value]
		foreach ( $allowed_values as $country_name => $code ) {
	...
	/**
	 * Fix corrupted profile fields that have nested arrays instead of flat field names
	 * This can happen when $_REQUEST fallback is used in getFlatInput
	 *
	 * @param array $fields The fields array to check and fix
	 * @return array The fixed fields array with flattened keys
	 */
	private function fixCorruptedFields( $fields ) {
		if ( !is_array( $fields ) ) {
			return $fields;
		}
		...
		foreach ( $fields as $key => $value ) {
			if ( is_array( $value ) && !empty( $value ) ) {
				// Check if this looks like corrupted nested array format
				$first_item = reset( $value );
				if ( is_array( $first_item ) && ( isset( $first_item['marketplace_id'] ) || isset( $first_item['value'] ) || isset( $first_item['unit'] ) ) ) {
					// This is corrupted format - flatten it
					$needs_fix = true;
					$flattened = $this->flattenNestedArrayToFormFields( [ $key => $value ] );
					$fixed = array_merge( $fixed, $flattened );
					continue;
				}
			}
			$fixed[ $key ] = $value;
		...
		if ( $needs_fix ) {
			WPLA()->logger->info( 'ProfileProductTypeConverter: Fixed corrupted profile fields with nested arrays' );
		}
