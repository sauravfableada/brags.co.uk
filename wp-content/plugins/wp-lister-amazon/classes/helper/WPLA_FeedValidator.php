...
class WPLA_FeedValidator {
	...
	static function returnHTML( $problems ) {
        ...
        $id = substr( md5(serialize($problems)), 0, 6 );
		$msg = '<p>' . sprintf( __( 'Warning: There are %s problem(s) with your products', 'wp-lister-for-amazon' ), count($problems) ) . ' <a href="#" onclick="jQuery(\'#wpla_issues_container_'. $id.'\').slideToggle(); return false;"  class="button">'. __('View Problems', 'wp-lister-for-amazon') .'</a></p>';
		$msg .= '<div id="wpla_issues_container_'. $id .'" style="display:none;"><br><br>';
	...
	static function formatAmazonFeedError( $feed_error ) {
        ...
        // Note: Do not esc_html here - convert_links handles escaping to avoid double-encoding URLs
				...
				if ( preg_match('/\((.*)\)\./', $error_message, $matches ) ) {
					...
					foreach ($rows as $row) {
						...
						if ( preg_match('/"(.*)" Merchant: "(.*)" \/ Amazon: "(.*)"/', $row, $matches ) ) {
							$extra_msg .= '<b>Field: ' . esc_html( $matches[1] ) . '</b><br>';
							$extra_msg .= 'You: ' . esc_html( $matches[2] ) . '<br>';
							$extra_msg .= 'Amz: ' . esc_html( $matches[3] ) . '<hr>';
						}
	 ...
	 *
	 * Handles URLs with spaces in query parameters (common in Amazon error messages)
	 * by URL-encoding spaces in the href attribute. Also escapes non-URL text for XSS prevention.
	 *
	 * @param string $text The text containing URLs (unescaped).
	 * @return string The text with URLs converted to HTML links and text escaped.
	 */
	static function convert_links( $text ) {
		// Check if there's a URL with query string at the end that might have spaces
		// Amazon error URLs often have unencoded spaces in param values
		if ( preg_match( '@(https?://[^\s<]+\?[^\s<]+)(\s+[^<]+)$@', $text, $matches ) ) {
			$base_url  = $matches[1];
			$remainder = $matches[2];

			// Check if remainder looks like continuation of URL params (contains &param=)
			if ( preg_match( '/&[a-zA-Z_][a-zA-Z0-9_]*=/', $remainder ) ) {
				$full_url     = $base_url . $remainder;
				$trimmed_url  = rtrim( $full_url, '.,;:!?' );
				$trailing     = substr( $full_url, strlen( $trimmed_url ) );
				$href_url     = str_replace( ' ', '%20', $trimmed_url );
				$link         = '<a href="' . esc_attr( $href_url ) . '" target="_blank">' . esc_html( $trimmed_url ) . '</a>' . esc_html( $trailing );
				$prefix       = substr( $text, 0, -strlen( $matches[0] ) );
				return esc_html( $prefix ) . $link;
			}
		}

		// Standard URL conversion (no spaces) - process text in segments
		$result   = '';
		$last_pos = 0;
		...
		if ( preg_match_all( '@(https?://[^\s<]+)@', $text, $matches, PREG_OFFSET_CAPTURE ) ) {
			foreach ( $matches[0] as $match ) {
				...
				$result .= esc_html( substr( $text, $last_pos, $offset - $last_pos ) );

				// Process the URL - trim trailing punctuation but preserve it
				$trimmed_url = rtrim( $url, '.,;:!?' );
				$trailing    = substr( $url, strlen( $trimmed_url ) );
				$href_url    = str_replace( ' ', '%20', $trimmed_url );
				$result     .= '<a href="' . esc_attr( $href_url ) . '" target="_blank">' . esc_html( $trimmed_url ) . '</a>' . esc_html( $trailing );

				$last_pos = $offset + strlen( $url );
			}
		}

		// Escape remaining text after the last URL
		$result .= esc_html( substr( $text, $last_pos ) );
