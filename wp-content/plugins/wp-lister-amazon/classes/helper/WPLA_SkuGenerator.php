...
class WPLA_SkuGenerator {
    ...
    function getPageItems( $current_page, $per_page ) {
        ...
        $orderby  = (!empty($_REQUEST['orderby'])) ? esc_sql( wpla_clean($_REQUEST['orderby']) ) : 'post_title';
        ...
        // filter sku_status
        $sku_status = isset($_REQUEST['sku_status']) ? esc_sql( wpla_clean($_REQUEST['sku_status']) ) : '';
    ...
    public static function getAllExistingSKUs() {
        global $wpdb;
    ...
    public static function getProductSkuData() {
        global $wpdb;
    ...
    /**
     * Resolve duplicate SKU by appending a number suffix
     *
     * @param string $base_sku The base SKU to make unique
     * @param array $existing_skus Array of existing SKUs to check against
     * @return string Unique SKU with number suffix if needed
     */
    public static function resolveDuplicateSKU( $base_sku, $existing_skus ) {

        // If base SKU doesn't exist, return it as-is
        if ( ! in_array( $base_sku, $existing_skus ) ) {
            return $base_sku;
        }

        // Start with suffix 2 and increment until we find a unique SKU
        $counter = 2;
        do {
            $new_sku = $base_sku . '-' . $counter;
            $counter++;
        } while ( in_array( $new_sku, $existing_skus ) );
