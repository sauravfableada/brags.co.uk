...
class WPLA_FeedDataBuilder {
	...
	static function buildNewProductsFeedData( $items, $account_id, $profile, $append_feed = false ) {
		...
		// BATCH LOADING: Extract all product IDs first
		$product_ids = [];
		...
		// BATCH LOADING: Load all products in advance
		$products_batch = self::load_products_batch( $product_ids );
		...
		foreach ( $items as $item ) {
			...
			$product = self::get_batch_product( $products_batch, $product_id );
			if ( ! $product ) continue;
		...
		switch ( $column ) {
            ...
            case 'standard_price':
				...
				if ( $value ) {
                    ...
                    $has_repricing = $item['min_price'] > 0 && $item['max_price'] > 0;
                    if ( ( $apply_shipping_to_all || $has_repricing ) && $shipping_fee = get_option( 'wpla_repricing_shipping', false ) ) {
                        $value -= $shipping_fee;
                ...
                if ( !$product_value && isset( $profile_fields[$column] ) && ! empty( $profile_fields[$column] ) ) {
...
//					    WPLA()->logger->info( 'Value from profile: '. $value );
			...
			case 'generic_keywords5':
				$value = get_post_meta( $product_id, '_amazon_'. $column, true );
...
//                    WPLA()->logger->info( 'found '. $value .' for '. $product_id );
				...
				$allow_variation_override = apply_filters( 'wpla_allow_custom_gallery_variation_override', false, $post_id, $product_id );
...
//                WPLA()->logger->info( 'image_index: '. $image_index );
			...
			case 'product-id':
				// Use listing-specific ASIN first (most accurate for current marketplace)
				$value = !empty($item['asin']) ? $item['asin'] : '';

				// Fallback to product meta ASIN if listing has no ASIN
				if ( empty( $value ) ) {
			...
			case 'product-id-type':
				// Use listing-specific ASIN first (most accurate for current marketplace)
				$matched_asin = !empty($item['asin']) ? $item['asin'] : '';

				// Fallback to product meta ASIN if listing has no ASIN
				if ( empty( $matched_asin ) ) {
...
//                WPLA()->logger->info( 'Found size map for column' );
	...
	static public function parseProfileShortcode( $original_value, $placeholder, $item, $product, $post_id, $profile ) {
		...
		switch ( $placeholder ) {
			...
			case '[product_price]':
                ...
                $apply_shipping_to_all = apply_filters( 'wpla_repricing_shipping_apply_to_all', false );
                $has_repricing = $item['min_price'] > 0 && $item['max_price'] > 0;
                if ( !empty( $shipping_fee ) && ( $apply_shipping_to_all || $has_repricing ) ) {
			...
			case '[product_brand]':
				$value = WPLA_ProductWrapper::getBrand( $post_id );
				break;
			...
			default:
				...
				$value = self::doTranslate( $value, $profile ? $profile->account_id : null );
				break;
		}
	...
	static function buildPriceAndQuantityFeedData( $items, $account_id ) {
		...
		// Cache profiles to avoid loading the same profile multiple times (90% query reduction)
		$profiles_cache = array();
		...
		foreach ( $items as $item ) {
			...
			// load profile fields (with caching to avoid duplicate DB queries)
			$profile_id = $item['profile_id'];
			if ( ! isset( $profiles_cache[ $profile_id ] ) ) {
				$profiles_cache[ $profile_id ] = new WPLA_AmazonProfile( $profile_id );
			}
	...
	static function parseOrderColumn( $column, $post_id ) {
		...
		switch ( $column ) {
			...
			case 'carrier-code':
                ...
                if ( strtolower( $value ) == 'hermes' || strtolower( $value ) == 'evri' ) {
                    // Hermes/Evri needs to be defined in the carrier-name instead of the carrier-code
                    $value = 'Other';
                }
				break;
			...
			case 'carrier-name':
				...
				$original_provider = $order->get_meta( '_wpla_tracking_provider', true );

				if ( $carrier != 'Other' ) {
				    // Always leave this empty if carrier has a value that's not "Other" #49684
				    $value = '';
                } else {
                    // If provider was Hermes/Evri, use that as carrier-name
                    if ( empty( $value ) && in_array( strtolower( $original_provider ), array( 'hermes', 'evri' ) ) ) {
                    ...
                    // Fallback to metabox filter (allows prefilling from external sources)
                    if ( empty( $value ) ) {
                        $value = apply_filters( 'wpla_set_tracking_service_for_order', '', $post_id );
                    }
			...
			case 'tracking-number':
				...
				// Final fallback to metabox filter (allows prefilling from external sources)
				if ( empty( $value ) ) {
					$value = apply_filters( 'wpla_set_tracking_number_for_order', '', $post_id );
				}
				break;
            ...
            case 'ship-method':
                ...
                // Hermes/Evri requires ship-method to be "Other"
                if ( empty( $value ) && in_array( strtolower( $original_provider ), array( 'hermes', 'evri' ) ) ) {
                    $value = 'Other';
                }
                ...
                // Fallback to metabox filter (allows prefilling from external sources)
                if ( empty( $value ) ) {
                    $value = apply_filters( 'wpla_set_tracking_ship_method_for_order', '', $post_id );
                }
                break;
	...
	/**
	 * Batch load WooCommerce products using WPLA_ProductWrapper
	 * Uses WooCommerce's internal caching for efficiency
	 *
	...
	protected static function load_products_batch( $product_ids ) {
		...
		// Use WPLA_ProductWrapper for consistency with existing code
