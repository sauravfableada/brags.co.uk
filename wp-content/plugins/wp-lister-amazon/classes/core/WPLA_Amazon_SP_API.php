...
use WPLab\Amazon\SellingPartnerApi\Model\ListingsV20210801 as Listings;
...
class WPLA_Amazon_SP_API {
    ...
    public function initAPI( $section = false ) {
        ...
        if ( ! $account_id ) {
            WPLA()->logger->error( 'initAPI() called without valid account_id' );
            return false;
        }
    ...
    /**
     * Extract a meaningful error message from an ApiException.
     *
     * The SP-API library uses a generic "Error connecting to the API" message for all
     * non-2xx responses. This method extracts the actual error from the response body.
     *
     * @param \WPLab\Amazon\SellingPartnerApi\ApiException $ex The API exception
     * @return array Array with 'message', 'code', and optionally 'details' keys
     */
    protected function extractApiErrorDetails( \WPLab\Amazon\SellingPartnerApi\ApiException $ex ) {
        $result = [
            'message' => $ex->getMessage(),
            'code'    => $ex->getCode(),
        ];

        // Try to get error from the deserialized response object first
        ...
        if ( $response_object && method_exists( $response_object, 'getErrors' ) ) {
            ...
            if ( !empty( $amazon_errors ) ) {
                ...
                $result['message'] = $first_error->getMessage();
                $result['code'] = $first_error->getCode();
                if ( method_exists( $first_error, 'getDetails' ) && $first_error->getDetails() ) {
                    $result['details'] = $first_error->getDetails();
                }
                return $result;
            }
        }
        ...
        $response_body = $ex->getResponseBody();
        if ( $response_body ) {
            ...
            if ( is_object( $response_body ) && isset( $response_body->errors ) && is_array( $response_body->errors ) && !empty( $response_body->errors ) ) {
                ...
                if ( isset( $first_error->code ) ) {
                    $result['code'] = $first_error->code;
                }
                if ( isset( $first_error->details ) ) {
                    $result['details'] = $first_error->details;
                }
            }
        }
    ...
    public function getFulfillmentPreview( $products, $shipping_address ) {
        ...
        try {
            ...
            $error_details = $this->extractApiErrorDetails( $e );
            WPLA()->logger->error( 'SP-API Exception: '. $error_details['message'] );

            $error = new stdClass();
            $error->ErrorMessage = $error_details['message'];
            $error->ErrorCode    = $error_details['code'];
            return $error;
        }
    ...
    public function listMarketplaceParticipations() {
        ...
        try {
            ...
            $error_details = $this->extractApiErrorDetails( $ex );
            $error = new stdClass();
            $error->ErrorMessage = $error_details['message'];
            $error->ErrorCode    = $error_details['code'];
            return $error;
	...
	public function putListingsItem( $listing, $profile ) {
		...
		if ( $this->initAPI( 'Listings' ) === false ) {
			...
			if ( get_option('wpla_log_to_db') == '1' ) {
				$this->logPutListingItemRequest( $listing, null, 'Failed to initialize SP-API configuration' );
			}
		...
		try {
			...
			$body_data = [
				'productType'  => ($product_type === 'Inventory Loader' || $product_type === 'LISTING_OFFER_ONLY') ? 'PRODUCT' : $product_type,
				'requirements' => ($product_type === 'PRODUCT' || $product_type === 'Inventory Loader' || $product_type === 'LISTING_OFFER_ONLY') ? 'LISTING_OFFER_ONLY' : 'LISTING',
				'attributes'   => $builder->getAttributes( $listing, $profile )
			];

			// Format the request data for logging
			$formatted_request = json_encode( $body_data, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE );
	...
	/**
	 * Log patchListingItem API calls to amazon_log table with formatted JSON request
	 */
	private function logPatchListingItemRequest( $sku, $patch_operations, $result ) {
		// Determine success status based on result
		$success = 'Processing';
		...
		// Format the patch operations for logging
		$formatted_request = json_encode( $patch_operations, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES );

		try {
			$logger = new WPLA_AmazonLogger();
			$logger->updateLog( array(
				'callname'    => 'patchListingItem',
				'request_url' => 'SP-API listings patch endpoint',
				'request'     => $formatted_request,
				'response'    => maybe_serialize( $result ),
				'success'     => $success,
				'account_id'  => $this->account_id,
			...
			// Log error if logging fails
			WPLA()->logger->error( 'Failed to log patchListingItem: ' . $e->getMessage() );
		}
	}
    ...
    public function getListingsItem( $sku ) {
        ...
        try {
            ...
            // Extract detailed error information for ApiExceptions
            if ($ex instanceof \WPLab\Amazon\SellingPartnerApi\ApiException) {
                $error_details = $this->extractApiErrorDetails( $ex );
                $error->ErrorMessage = $error_details['message'];
                $error->ErrorCode = $error_details['code'];
    ...
    public function getCompetitivePricing( $product_ids ) {
        ...
        try {
            ...
            $error->success      = false;
            WPLA()->logger->error( $error_details['message'] );
            return $error;
        } catch ( Exception $ex ) {
            $error = new stdClass();
            $error->ErrorMessage = $ex->getMessage();
    ...
    public static function feedOptionsToArray( $feed_options_str ) {
        ...
        foreach ( $pcs as $pc ) {
            $parts = explode( '=', $pc );
            // Skip malformed entries that don't have both key and value
    ...
    /**
     * Patch listings item using Amazon SP-API PatchListingsItem endpoint
     * 
     * @param string $sku SKU of the listing to patch
     * @param string $product_type Product type (e.g., 'PRODUCT')  
     * @param array $patch_operations Array of patch operations
     * @param array|null $marketplace_ids Marketplace IDs (defaults to account marketplace)
     * @param int $max_retries Maximum number of retry attempts (default: 3)
     * @return object Result object with success flag and response data
     */
    public function patchListingsItem( $sku, $product_type, $patch_operations, $marketplace_ids = null, $max_retries = 3 ) {
        $this->initAPI();
        ...
        WPLA()->logger->info( sprintf( 
            'PatchListingsItem request for SKU: %s, Product Type: %s, Marketplaces: %s', 
            $sku, 
            $product_type, 
            implode( ', ', $marketplace_ids )
        ) );
        WPLA()->logger->info( 'Patch operations: ' . json_encode( $patch_operations, JSON_PRETTY_PRINT ) );

        // Log the exact API call details that would be made
        WPLA()->logger->info( sprintf(
            'API Call Details - Merchant ID: %s, SKU: %s, Marketplace IDs: [%s]',
            $this->account->merchant_id,
            $sku,
            implode( ', ', $marketplace_ids )
        ) );
...
$api = new ListingsApi( $this->config, $this->client );
        ...
        // Convert patch operations to proper model objects
        $patch_objects = [];
        foreach ( $patch_operations as $operation ) {
            $patch_objects[] = new Listings\PatchOperation([
                'op' => $operation['op'],
                'path' => $operation['path'],
        ...
        $patch_request = new Listings\ListingsItemPatchRequest([
        ...
        while ( $attempt <= $max_retries ) {
            try {
                $response = $api->patchListingsItem(
                    $this->account->merchant_id,
                    $sku,
                    $marketplace_ids,
                    $patch_request
                ...
                WPLA()->logger->info( sprintf( 
                    'PatchListingsItem successful for SKU: %s (attempt %d/%d)', 
                    $sku, 
                    $attempt + 1, 
                    $max_retries + 1 
                ) );
                ...
                $result->patches_applied = count( $patch_operations );
                $result->response = $response;
                
                // Log the API call if database logging is enabled
                if ( get_option('wpla_log_to_db') == '1' ) {
                    $this->logPatchListingItemRequest( $sku, $patch_operations, $result );
                }
                ...
                // Log the attempt
                WPLA()->logger->error( sprintf( 
                    'PatchListingsItem attempt %d/%d failed for SKU: %s - Code: %s, Message: %s', 
                    $attempt, 
                    $max_retries + 1, 
                    $sku, 
                    $error_code, 
                ...
                $should_retry = $this->shouldRetryApiCall( $error_code, $attempt, $max_retries );
                
                if ( $should_retry ) {
                    $delay = $this->calculateRetryDelay( $attempt );
                    WPLA()->logger->info( sprintf( 
                        'Retrying PatchListingsItem for SKU: %s after %d seconds', 
                        $sku, 
                        $delay 
                    ) );
                    sleep( $delay );
                ...
                break; // Don't retry for non-retriable errors
            }
        }
        ...
        $error->success = false;
        $error->sku = $sku;
        $error->ErrorMessage = $last_error->getMessage();
        $error->ErrorCode = $last_error->getCode();
        $error->attempts_made = $attempt;
        
        WPLA()->logger->error( sprintf( 
            'PatchListingsItem failed for SKU: %s after %d attempts - Final error: %s', 
            $sku, 
            $attempt, 
            $error->ErrorMessage 
        ) );

        // Log the failed API call if database logging is enabled
        if ( get_option('wpla_log_to_db') == '1' ) {
            $this->logPatchListingItemRequest( $sku, $patch_operations, $error );
        }
    ...
    /**
     * Determine if an API call should be retried based on error code
     * 
     * @param int $error_code HTTP error code
     * @param int $current_attempt Current attempt number
     * @param int $max_retries Maximum retries allowed
     * @return bool Whether to retry the call
     */
    private function shouldRetryApiCall( $error_code, $current_attempt, $max_retries ) {
        ...
        return in_array( $error_code, $retriable_codes );
    }
    ...
    private function calculateRetryDelay( $attempt ) {
        ...
        return min( pow( 2, $attempt - 1 ), 30 ); // Cap at 30 seconds
    }
