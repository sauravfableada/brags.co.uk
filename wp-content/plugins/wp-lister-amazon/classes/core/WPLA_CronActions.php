...
class WPLA_CronActions {
	...
	public function __construct() {
		...
		add_action('wpla_process_pending_feeds', 				array( &$this, 'action_process_pending_feeds' ) );
	...
	public function cron_update_schedule() {
		...
		// This runs orders in a separate request so they don't compete with feed building for execution time.
		if ( get_option( 'wpla_dedicated_orders_cron', 0 ) ) {
            // Only queue if no order task is already pending - prevents duplicate tasks piling up
            if ( ! as_next_scheduled_action( 'wpla_update_orders', [], 'WPLA' ) ) {
	...
	public function action_autosubmit_fba_orders() {
        ...
        WPLA()->logger->info( 'found '. $num_orders .' recent orders');
		...
		foreach ($accounts as $account ) {
			...
			foreach ($feeds as $feed) {
				...
				$autosubmit_feeds = array(
					...
					// JSON
					'JSON_LISTINGS_FEED',
					'OFFER_CLEARING',	// Offer clearing (sale price removal) feed
				);
			...
			while ( ! empty( $queue ) ) {
				...
				foreach ( $chunk as $listing_id ) {
					...
					if ( in_array( $listing['status'] ?? '', $already_published_statuses ) ) {
						WPLA()->logger->info( "Listing {$listing_id} already published (status: " . ($listing['status'] ?? 'unknown') . "), removing from queue" );
						WPLA_ListingsModel::removeListingFromPublishingQueue( $listing_id );
						continue;
					}

					$result = $mdl->submitSingleListing( $listing_id, [
						'allow_status' => [WPLA_ListingsModel::STATUS_PREPARED, WPLA_ListingsModel::STATUS_MATCHED],
						'remove_from_queue' => true,
						'retry_on_throttle' => true
					]);

					// Handle throttling - add to retry queue
					if ( $result->should_retry ) {
						WPLA()->logger->info( "Throttled on {$listing_id}, will retry next run" );
						$retry_queue[] = $listing_id;
					} elseif ( !$result->success ) {
						// Log non-throttling errors
						WPLA()->logger->info( "Publish failed for {$listing_id}: {$result->error_message}" );
					}
				}
    ...
    public function set_inventory_check_cron_schedule() {
		...
		$frequency_changed = get_option( 'wpla_inventory_check_frequency_changed', false );
        ...
        if ( get_option( 'wpla_run_background_inventory_check', 1) ) {
			if ( $frequency_changed ) {
				...
				as_unschedule_all_actions('wpla_bg_inventory_check_run', [], 'WPLA');
			}

            // Turn it on
            if ( ! as_next_scheduled_action( 'wpla_bg_inventory_check' ) || $frequency_changed ) {
				...
				delete_option( 'wpla_inventory_check_frequency_changed' );
            }
        } else {
        ...
        // Schedule pending feeds processing - respect update interval setting
        $cron_schedule = get_option('wpla_option_cron_schedule', 'fifteen_min');
        if ( $cron_schedule != 'manually' && ! as_next_scheduled_action( 'wpla_process_pending_feeds' ) ) {
            // Use same interval as main cron but ensure minimum 5 minutes
            $interval_map = array(
                ...
                'hourly'       => 3600,  // 1 hour
                'twicedaily'   => 3600, // 12 hours (but limit to 1 hour max)
                'daily'        => 3600   // 1 day (but limit to 1 hour max)
            );
            
            $default_interval = apply_filters('wpla_feed_processing_interval', 180); // 3 minutes default
            $interval = isset($interval_map[$cron_schedule]) ? $interval_map[$cron_schedule] : $default_interval;
            $interval = min($interval, 3600); // Maximum 1 hour interval for feeds
            
            as_schedule_recurring_action( time(), $interval, 'wpla_process_pending_feeds', [], 'WPLA' );
            WPLA()->logger->info("Scheduled wpla_process_pending_feeds to run every {$interval} seconds (based on {$cron_schedule} setting)");
        }
    }
    ...
    public function cron_bg_inventory_check( $check = false ) {
        ...
        foreach ($accounts as $account ) {
            ...
            $report_id = $api->createReport( 'GET_MERCHANT_LISTINGS_DATA' );
            ...
            if ( is_object($report_id) && isset( $report_id->ErrorMessage ) ) {
	            ...
	            if ( !empty( $report->errors ) && isset( $report->errors[0]->message ) ) {
		            ...
		            as_schedule_recurring_action( time() + 120, 120, 'wpla_update_reports', array('inventory_sync' => 1), 'WPLA' );
	            }
	...
	public function handle_reduce_order_stock( $order ) {
			...
			if ( (@$item['id']>0) || (@$item['product_id']>0) ) {
				...
				if ( $_product && $_product->exists() && $_product->managing_stock() ) {
					...
					if ( ! $variation_id ) {
						...
						// mark as modified
						$listingsModel->markItemAsModified( $post_id, true );
		...
		if ( count($post_IDs) > 0 ) {
			...
			$order->add_order_note( sprintf( __( 'Scheduled inventory update for %s item(s) on Amazon...', 'wp-lister-for-amazon' ), count($post_IDs) ) .' '. join(', ',$post_IDs) );
	...
	public function action_clean_tables() {
		...
		if ( $days_to_keep ) {
			...
			$rows = $wpdb->query('DELETE FROM '.$wpdb->prefix.'amazon_fulfillment_feed_items WHERE date_added < DATE_SUB(NOW(), INTERVAL '.$days_to_keep.' DAY )');
			WPLA()->logger->info('Cleaned table amazon_fulfillment_feed_items - affected rows: ' . $rows);
		}
	...
	/**
	 * Process pending feeds in background - called by ActionScheduler
	 */
	public function action_process_pending_feeds() {
		WPLA()->logger->info("do_action: wpla_process_pending_feeds (background task)");

		// Check if staging site - skip processing if configured
		if ( WPLA_Setup::isStagingSite() ) {
			WPLA()->logger->info("Skipping feed processing on staging site");
			return;
		}

		// Set execution time limit and memory limit for safety
		$start_time = time();
		$max_execution_time = 45; // Stay under 45 seconds to avoid ActionScheduler timeout
		set_time_limit(60); // Set PHP limit to 60s as backup
		ini_set('memory_limit', '512M');

		try {
			WPLA()->logger->info("Starting background feed processing with {$max_execution_time}s timeout");
			
			// Process feeds with timeout monitoring
			$this->processAccountFeeds($start_time, $max_execution_time);
			
		} catch (Exception $e) {
			WPLA()->logger->error("Error in background feed processing: " . $e->getMessage());
			
			// Schedule continuation if the error might be timeout-related
			if (time() - $start_time >= $max_execution_time - 5) {
				WPLA()->logger->info("Scheduling continuation due to timeout");
				as_enqueue_async_action('wpla_process_pending_feeds', [], 'WPLA');
			}
		}
	}

	/**
	 * Process feeds for accounts with timeout monitoring
	 */
	private function processAccountFeeds($start_time, $max_execution_time) {
		$accounts = WPLA_AmazonAccount::getAll();
		...
		foreach ($accounts as $account) {
			...
			if (time() - $start_time >= $max_execution_time - 10) {
				WPLA()->logger->info("Approaching timeout - processed {$processed_accounts}/" . count($accounts) . " accounts");
				
				// Schedule continuation if more accounts to process
				if ($processed_accounts < count($accounts)) {
					as_enqueue_async_action('wpla_process_pending_feeds', [], 'WPLA');
					WPLA()->logger->info("Scheduled continuation for remaining accounts");
				}
				break;
			}

			try {
				WPLA()->logger->info("Processing pending feeds for account: " . $account->title);
				WPLA_AmazonFeed::updatePendingFeedForAccount($account);
				$processed_accounts++;
				
			} catch (Exception $e) {
				WPLA()->logger->error("Error processing account {$account->title}: " . $e->getMessage());
				$processed_accounts++; // Continue with next account
			}
		}

		// Process inventory feed auto-submission if enabled
		if (get_option('wpla_autosubmit_inventory_feeds', 0) && time() - $start_time < $max_execution_time - 5) {
			foreach ($accounts as $account) {
				if (time() - $start_time >= $max_execution_time - 5) break;
				
				try {
					WPLA_AmazonFeed::submitInventoryFeedsForAccount($account);
				} catch (Exception $e) {
					WPLA()->logger->error("Error submitting inventory feeds for {$account->title}: " . $e->getMessage());
				}
			}
		}

		WPLA()->logger->info("Background feed processing completed. Processed {$processed_accounts} accounts in " . (time() - $start_time) . " seconds");
	}
