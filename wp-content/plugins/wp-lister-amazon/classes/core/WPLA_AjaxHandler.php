		...
		if ( isset( $_REQUEST['id'] ) ) {
			...
			if ( $product ) {
			    ...
			    switch ($query_type) {
				    ...
				    case 'ean':
			    		# Try WPLA's amazon_product_id first, fallback to WooCommerce's global_unique_id
						$query = get_post_meta( wpla_clean($_REQUEST['id']), '_amazon_product_id', true );
						if ( empty( $query ) ) {
							$query = get_post_meta( wpla_clean($_REQUEST['id']), '_global_unique_id', true );
						}
			    		break;
				...
				if ( empty( $products ) ){
					...
					if ( $query_type == 'sku' || $query_type == 'ean' ) {
						$result = $api->searchCatalogItems( '', $query, strtoupper( $query_type ), $account->merchant_id );
					} else {
				...
				//if ( is_array( $products ) )  {
					...
					// Get EAN value with fallback from _amazon_product_id to _global_unique_id
					$ean_value = get_post_meta( wpla_clean($_REQUEST['id']), '_amazon_product_id', true );
					if ( empty( $ean_value ) ) {
						$ean_value = get_post_meta( wpla_clean($_REQUEST['id']), '_global_unique_id', true );
					}
...
//				}
	...
	public function ajax_get_product_type_attributes_html() {
		...
		try {
			...
			if ( $product_type === 'Inventory Loader' ) {
				// Try to get existing PRODUCT schema from database first
				$type_obj = new AmazonProductTypesModel();
				$product_type_data = $type_obj->getProductType( 'PRODUCT', $marketplace );

				// If not found in database, try to fetch from API
				if ( !$product_type_data ) {
					$product_type_data = $type_obj->getDefinitionsProductType( 'PRODUCT', $marketplace, false, $account_id );
				}

				// Check if we got an error loading the PRODUCT schema
				if ( is_wp_error( $product_type_data ) || !$product_type_data ) {
					$error_msg = is_wp_error( $product_type_data ) ? $product_type_data->get_error_message() : 'PRODUCT type not found for this marketplace';
					echo "Error loading PRODUCT schema for Inventory Loader: " . $error_msg;
					exit();
				}

				// Mark this as inventory loader for filtering
				$product_type_data->inventory_loader_mode = true;
				...
				// build settings form using PRODUCT schema but filtered for Inventory Loader
				$data = [
				...
				$data = apply_filters( 'wpla_profile_product_type_data', $data, $profile, $product_type_data );

				@WPLA_Page::do_display( 'profile/product_type_attributes_form', $data );
				exit();
			}

			// Handle Offer Only as special case
			if ( $product_type === 'LISTING_OFFER_ONLY' ) {
				// Try to get existing PRODUCT schema from database first
				$type_obj = new AmazonProductTypesModel();
				$product_type_data = $type_obj->getProductType( 'PRODUCT', $marketplace );
				...
				if ( is_wp_error( $product_type_data ) || !$product_type_data ) {
					$error_msg = is_wp_error( $product_type_data ) ? $product_type_data->get_error_message() : 'PRODUCT type not found for this marketplace';
					echo "Error loading PRODUCT schema for Offer Only: " . $error_msg;
					exit();
				}

				// Mark this as offer only for filtering
				$product_type_data->offer_only_mode = true;
				...
				// build settings form using PRODUCT schema but filtered for Offer Only
				$data = [
					'profile_field_data'    => maybe_unserialize( $profile->fields ),
					'product_attributes'    => WPLA_ProductWrapper::getAttributeTaxonomies(),
					'product_type'          => $product_type_data,
			...
			// Handle regular product types
			$type_obj       = new AmazonProductTypesModel();
			$product_type   = $type_obj->getDefinitionsProductType( $product_type, $marketplace, false, $account_id );

			// Check if we got an error loading the product type
			if ( is_wp_error( $product_type ) || !$product_type ) {
				$error_msg = is_wp_error( $product_type ) ? $product_type->get_error_message() : 'Product type could not be loaded.';
				echo "Error loading product type: " . $error_msg;
				exit();
			}
	...
	public function ajax_wpla_tail_log() {
		...
		$tail = new PHPTail( WPLA()->logger->file, 3000, 524288 );

		// handle ajax call
		if(isset($_GET['ajax']))  {
			echo $tail->getNewLines(0, wpla_clean(@$_GET['lastsize']), wpla_clean($_GET['grep']), wpla_clean($_GET['invert']) );
			die();
		}
