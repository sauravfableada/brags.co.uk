...
use WPLab\Amazon\GuzzleHttp\Exception\GuzzleException;
...
class WPLA_Amazon_SP_API_Authentication extends \WPLab\Amazon\SellingPartnerApi\Authentication {
    ...
    private function newToken(): void
    {
        try {
            ...
            WPLA()->logger->error( 'Error fetching an LWA token: '. $e->getMessage() );
            if ( !is_ajax() && is_admin() ) {
                wpla_show_message( 'There was an error connecting to the authentication server. Please try again later.', 'error' );
            }
            return;
        }
