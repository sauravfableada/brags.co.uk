...
class WPLA_FeedsTable extends WP_List_Table {
    ...
    function column_details($item){
        ...
        $actions = array(
            'view_amazon_feed_details' => sprintf( '<a href="admin-ajax.php?action=%s&id=%s&sig=%s&_wpnonce=%s" target="_blank">%s</a>', 'wpla_feed_details', $item['id'], $signature, wp_create_nonce( 'wpla_view_feed_details' ), __( 'Details', 'wp-lister-for-amazon' ) ),
            'view_amazon_feed_results' => sprintf( '<a href="?page=%s&action=%s&amazon_feed=%s&_wpnonce=%s&width=600&height=470" class="thickbox">%s</a>', $page, 'view_amazon_feed_results', $item['id'], wp_create_nonce( 'wpla_view_feed_results' ), __( 'View Results', 'wp-lister-for-amazon' ) ),
            'submit_feed_to_amazon'    => sprintf( '<a href="?page=%s&action=%s&amazon_feed=%s&_wpnonce=%s">%s</a>', $page, 'submit_feed_to_amazon', $item['id'], wp_create_nonce( 'wpla_submit_feed' ), __( 'Submit', 'wp-lister-for-amazon' ) ),
            'process_amazon_feed_results'      => sprintf( '<a href="?page=%s&action=%s&amazon_feed=%s&_wpnonce=%s">%s</a>', $page, 'process_amazon_feed_results', $item['id'], wp_create_nonce( 'wpla_process_feed_results' ), __( 'Process Results', 'wp-lister-for-amazon' ) ),
            'process_amazon_feed_again'        => sprintf( '<a href="?page=%s&action=%s&amazon_feed=%s&_wpnonce=%s">%s</a>', $page, 'process_amazon_feed_results', $item['id'], wp_create_nonce( 'wpla_process_feed_results' ), __( 'Process again', 'wp-lister-for-amazon' ) ),
        );

        // item title
        if ( $item['FeedType'] == 'JSON_LISTINGS_FEED' ) {
	        // Display user-friendly names for special feeds
	        if ( $item['template_name'] === 'FBA_TO_FBM_CONVERSION' ) {
		        $title = 'Fulfillment Channel Conversion <i style="color:silver">JSON Feed</i>';
	        } elseif ( $item['template_name'] === 'B2B_PRICE_DELETION (PATCH)' ) {
		        $title = 'B2B Price Removal <i style="color:silver">JSON Feed</i>';
	        } elseif ( $item['template_name'] === 'LISTING_OFFER_ONLY' ) {
		        $title = 'Offer Only <i style="color:silver">JSON Feed</i>';
	        } else {
		        $title = $item['template_name'].' <i style="color:silver">JSON Feed</i>';
	        }

	        // Add SKU information for PATCH feeds
	        if ( strpos( $item['template_name'], '(PATCH)' ) !== false ) {
		        if ( isset( $item['display_reference'] ) ) {
			        $title .= ' <span style="color: #666; font-size: 0.9em;">(' . esc_html( $item['display_reference'] ) . ')</span>';
		        } else {
			        // Extract SKU information from feedOptions
			        $feed_options = maybe_unserialize( $item['feedOptions'] ?? '' );
			        if ( is_array( $feed_options ) && isset( $feed_options['patch_skus'] ) ) {
				        $sku_display = WPLA_AmazonFeed::formatSkuArray( $feed_options['patch_skus'] );
				        if ( $sku_display ) {
					        $title .= ' <span style="color: #666; font-size: 0.9em;">(' . esc_html( $sku_display ) . ')</span>';
				        }
			        }
		        }
	        }
    ...
    function display_unspecific_feed_errors( $item ) {
        ...
        // clean up the results by removing the ASIN Creation Policy notice
        $item['results'] = preg_replace( "/\n\nPlease refer to the ASIN Creation Policy([^\n]+)\n/", "\n", $item['results'] );
        ...
        foreach ($result_rows as $row) {
            ...
            if ( empty( $row['error-code'] ) && empty( $row['error-type'] ) && empty( trim($row['error-message']) ) ) continue;
    ...
    function column_SubmittedDate($item){
        ...
        return sprintf('%1$s <br><span style="color:silver">%2$s</span>',
            /*$1%s*/ date( get_option( 'date_format' ) .' H:i', strtotime( $item['SubmittedDate'] ) ),
            /*$2%s*/ sprintf( __( '%s ago', 'wp-lister-for-amazon' ), human_time_diff( strtotime( $item['SubmittedDate'] ) ) )
        );
    }
    ...
    function column_CompletedProcessingDate($item){
        ...
        return sprintf('%1$s <br><span style="color:silver">%2$s</span>',
            /*$1%s*/ date( get_option( 'date_format' ) .' H:i', strtotime( $item['CompletedProcessingDate'] ) ),
    ...
    function column_status($item){
        ...
        switch( $item['FeedProcessingStatus'] ){
            ...
            case '_IN_PROGRESS_':
                $color = 'darkorange';
                $value = __( 'In progress', 'wp-lister-for-amazon' );
                break;
            case '_DONE_':
                $color = 'green';
                $value = __( 'Done', 'wp-lister-for-amazon' );
                break;
            case '_CANCELLED_':
                $color = 'gray';
                $value = __( 'Cancelled', 'wp-lister-for-amazon' );
                break;
            case '_DONE_NO_DATA_':
                $color = 'gray';
                $value = __( 'No results', 'wp-lister-for-amazon' );
                break;
            case 'pending':
                $color = 'purple';
                $value = __( 'Pending', 'wp-lister-for-amazon' );
				break;
            default:
                $color = 'black';
    ...
    function get_columns(){
        $columns = array(
            ...
            'FeedSubmissionId'   => __( 'Batch ID', 'wp-lister-for-amazon' ),
            // 'FeedType'        => __( 'FeedType', 'wp-lister-for-amazon' ),
            'details'           => __( 'Type', 'wp-lister-for-amazon' ),
            'SubmittedDate'     => __( 'Submitted at', 'wp-lister-for-amazon' ),
            'CompletedProcessingDate'	    => __( 'Completed at', 'wp-lister-for-amazon' ),
            'status'            => __( 'Status', 'wp-lister-for-amazon' ),
            'account_id'        => __( 'Account', 'wp-lister-for-amazon' )
        );
        return $columns;
    }
    ...
    function get_bulk_actions() {
        $actions = array(
            'update_amazon_feed' => __( 'Update feed status from Amazon', 'wp-lister-for-amazon' ),
            'cancel_amazon_feed' => __( 'Cancel submitted feeds', 'wp-lister-for-amazon' ),
            'delete_amazon_feed' => __( 'Delete selected feeds', 'wp-lister-for-amazon' )
        );
        return $actions;
    }
    ...
    function get_views(){
       $views    = array();
       $current  = ( !empty($_REQUEST['feed_status']) ? wpla_clean($_REQUEST['feed_status']) : 'all');
       ...
       $views['processed'] = "<a href='{$processed_url}' {$class} >".__( 'Processed', 'wp-lister-for-amazon' )."</a>";
       ...
       $views['pending'] = "<a href='{$pending_url}' {$class} >".__( 'Pending', 'wp-lister-for-amazon' )."</a>";
       ...
       // new submissions link - count from API logs (putListingItem calls)
       global $wpdb;
       $feeds_table = $wpdb->prefix . 'amazon_feeds';
       $log_table = $wpdb->prefix . 'amazon_log';
       $new_submissions_count = 0;
       
       // Only show if database logging is enabled
       if ( get_option('wpla_log_to_db') == '1' ) {
           $new_submissions_count = $wpdb->get_var("
               SELECT COUNT(*)
               FROM $log_table
               WHERE callname = 'putListingItem'
           ");
       }
       
       if ( $new_submissions_count > 0 ) {
           $new_submissions_url = add_query_arg( 'feed_status', 'new_submissions', $base_url );
           $class = ($current == 'new_submissions' ? ' class="current"' :'');
           $views['new_submissions'] = "<a href='{$new_submissions_url}' {$class}>".__( 'New Submissions', 'wp-lister-for-amazon' )."</a>";
           $views['new_submissions'] .= '<span class="count">('.$new_submissions_count.')</span>';
       }

       // blocked legacy feeds link - count actual legacy feeds in database
       $legacy_count = $wpdb->get_var("
           SELECT COUNT(*)
           FROM $feeds_table
           WHERE (template_name IN ('InventoryLoader', 'ListingLoader', 'LiLo') 
       ...
       if ( $legacy_count > 0 ) {
           $blocked_url = add_query_arg( 'feed_status', 'blocked_legacy', $base_url );
           $class = ($current == 'blocked_legacy' ? ' class="current"' :'');
           $views['blocked_legacy'] = "<a href='{$blocked_url}' {$class} style='color: #d63638;'>".__( 'Blocked Legacy Feeds', 'wp-lister-for-amazon' )."</a>";
           $views['blocked_legacy'] .= '<span class="count">('.$legacy_count.')</span>';
       }
