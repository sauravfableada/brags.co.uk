...
class WPLA_ProductWrapper {
    ...
    /**
     * Get the product's price. Returns the sale price if it is set and disable_sale_price setting is off
     * @param $post_id
     * @return mixed|string
     */
	static function getPrice( $post_id ) {
	    ...
	    if ( !$product ) {
	        // fallback to pulling from the postmeta
            return get_post_meta( $post_id, '_price', true);
        }

	    // consider the disable_sale_price setting #50473
	...
	static function getStock( $post_id ) {
	    ...
	    if ( $post_id && wpla_is_json( $post_id ) ) {
	        ...
	        $post_id = $data['id'];
            WPLA()->logger->info( 'Found ID: '. $post_id);
        }
	    WPLA()->logger->info( 'WPLA getStock for #'. $post_id );
        ...
        if ( method_exists( $product, 'get_stock_managed_by_id' ) ) {
            // Support for variations with parent-level stock management (from Yatin @ multidots) #56456
            $product_id_with_stock = $product->get_stock_managed_by_id();
            $product               = $product_id_with_stock !== $product->get_id() ? wc_get_product( $product_id_with_stock ) : $product;
        }

        // Adding support for ATUM Multi Inventory #40149 so it returns the total number of stocks across all sources
        add_filter( 'atum/multi_inventory/bypass_mi_get_stock_quantity', '__return_false', PHP_INT_MAX );

        if ( $product->is_type( 'bundle' ) && is_callable( array( $product, 'get_bundle_stock_quantity' ) ) ) {
            // Product Bundles plugin support
            $stock = $product->get_bundle_stock_quantity( 'edit' );
        } else {
        ...
        WPLA()->logger->info( 'get_stock_quantity: '. $stock );

        if ( !$product->managing_stock() && get_option( 'wpla_fallback_to_stock_status', 0 ) ) {
            if ( $product->is_in_stock() ) {
                $stock = intval( apply_filters( 'wpla_fallback_stock_quantity', 1 ) );
                WPLA()->logger->info( 'fallback_stock_quantity: '. $stock );
            } else {
                $stock = 0;
            }
        }

        WPLA()->logger->info( 'getStock value: '. $stock );

        // Use WC_Product::get_id() instead of passing the $post_id in the filter
        // since $post_id can be an instance of WC_Product at times
        //return apply_filters( 'wpla_get_stock', $stock, $post_id );
        return apply_filters( 'wpla_get_stock', $stock, $product->get_id() );
	}
	 ...
	 *
	 * Applies the `wpla_handle_stock_decrease` filter to allow third-party plugins
	 * (e.g., multi-warehouse inventory) to handle stock reduction.
	 *
	 * @param int              $post_id  Product or variation ID.
	 * @param int              $by       Quantity to decrease.
	 * @param string|int|false $order_id Amazon order ID or false.
	 * @return int|false New stock quantity or false on failure.
	 */
	static function decreaseStockBy( $post_id, $by, $order_id = false ) {
		...
		// Allow third-party plugins (e.g., multi-warehouse inventory) to handle stock reduction
		$handled = apply_filters( 'wpla_handle_stock_decrease', false, $post_id, $by, $order_id );
		if ( $handled !== false ) {
			return $handled;
		}
		...
		if ( $product->managing_stock() ) {
            ...
            if ( version_compare( WC_VERSION, '3.0', '<' ) ) {
                ...
                $stock = wc_update_product_stock( $post_id, $by, 'decrease' );
                //$current_stock = WPLA_ProductWrapper::getStock( $post_id );
	 ...
	 *
	 * Applies the `wpla_handle_stock_increase` filter to allow third-party plugins
	 * (e.g., multi-warehouse inventory) to handle stock restoration.
	 *
	 * @param int              $post_id  Product or variation ID.
	 * @param int              $by       Quantity to increase.
	 * @param string|int|false $order_id Amazon order ID or false.
	 * @return int|void New stock quantity or void if not managing stock.
	 */
	static function increaseStockBy( $post_id, $by, $order_id = false ) {
		$product = self::getProduct( $post_id );
		if ( ! $product ) return false;

		// Allow third-party plugins (e.g., multi-warehouse inventory) to handle stock increase
		$handled = apply_filters( 'wpla_handle_stock_increase', false, $post_id, $by, $order_id );
		if ( $handled !== false ) {
			return $handled;
		}
	...
	static function getImageURL( $post_id ) {
		...
		return is_array( $large_image_url ) ? $large_image_url[0] : '';
	}
	...
	static function getGalleryAttachmentIDs( $_product ) {
        ...
        if ( $_product->is_type('variation') ) {
            ...
            if ( class_exists( 'Iconic_WooThumbs' ) ) {
                ...
                if ( !$additional_images ) {
                    ...
                    $additional_images = $variation ? $variation->get_gallery_image_ids() : array();
                }
            ...
            // WooCommerce Additional Variation Images Gallery #30474
            if ( class_exists( 'Woo_Variation_Gallery' ) ) {
                $additional_images = get_post_meta( $product_id, 'woo_variation_gallery_images', true );

                if ( $additional_images ) {
                    $gallery_images = maybe_unserialize( $additional_images );
                    return $gallery_images;
                }
            }
        }
		...
		if ( ! empty($gallery_images) ) {
		    WPLA()->logger->info( 'gallery_images from WC_Product::get_gallery_image_ids: '. print_r( $gallery_images, 1 ) );
		    return $gallery_images;
        }
        ...
        if ( $_product->is_type( 'variation' ) && version_compare( WC_VERSION, '3.0', '>=' ) ) {
            ...
            if ( $parent_id ) {
                ...
                WPLA()->logger->debug( 'parent '. $parent_id .' gallery_images: '. print_r( $gallery_images, 1 ) );
                if ( ! empty($gallery_images) ) return $gallery_images;
            }
        }

		// if gallery is empty and fallback disabled, return empty array
		if ( get_option( 'wpla_product_gallery_fallback', 'none' ) == 'none' ) {
		    WPLA()->logger->debug( 'fallback is disabled. Returning an empty array' );
		    return array();
        }
	...
	static function getVariations( $post_id ) {
		global $product, $post; // make $product globally available for some badly coded themes...

		$product = self::getProduct( $post_id );
		$post = get_post( $post_id );

		if ( ! $product || !$product->is_type( array( 'variable', 'variable-product-part' ) ) ) return array();
    ...
    public static function maybeUpdateParentBundle( $product_id ) {
        ...
        WPLA()->logger->info( 'maybeUpdateParentBundle #'. $product_id );

        $bundles = WC_PB_DB::query_bundled_items( array(
            'return'        => 'id=>bundle_id',
            'product_id'    => $product_id
        ) );
        WPLA()->logger->debug( 'Bundles: '. print_r( $bundles, 1 ) );
        if ( $bundles ) {
            foreach ( $bundles as $bundle_id ) {
                do_action( 'wpla_product_has_changed', $bundle_id );
            }
        }
	...
	public static function getBrand( $product_id ) {
		if ( WPLA_ProductWrapper::isSingleVariation( $product_id ) ) {
			$product_id = WPLA_ProductWrapper::getVariationParent( $product_id );
		}
