...
class WPLA_Product_MetaBox {
	...
	function meta_box_basic( $post ) {
		...
		// woocommerce_wp_radio( array(
			...
			'desc_tip'		=>  true,
			'value'			=> strtolower(get_post_meta( $post->ID, '_amazon_id_type', true ))
		) );
    ...
    function get_search_term_value( $post_id ) {
	    // Only pull combine the keywords if keyword_fields_type is set to separate #47194
        if ( 'single' == get_option( 'wpla_keyword_fields_type', 'separate' ) ) {
				...
				jQuery('form#post').on('submit', function() {
					...
					if ( jQuery('#product-type')[0].value == 'variable' ) {
						...
						if ( jQuery('#_regular_price')[0].value == '' ) {
							missing_fields.push('<?php echo esc_js( __( 'Price', 'wp-lister-for-amazon' ) ); ?>');
						}

						// Quantity
						if ( jQuery('#_stock')[0].value == '' ) {
							missing_fields.push('<?php echo esc_js( __( 'Quantity', 'wp-lister-for-amazon' ) ); ?>');
						}
					...
					if ( missing_fields.length > 0 ) {
						var CRLF = "\n";
						var msg  = '<?php echo esc_js( __( 'This product is missing the following fields required to be listed on Amazon:', 'wp-lister-for-amazon' ) ); ?>' + CRLF + CRLF + '- ' + missing_fields.join(CRLF+'- ');
						alert(msg);
					}
	...
	function save_meta_box( $post_id, $post ) {
		...
		// Handle B2B price changes and trigger PATCH feed if needed
		$old_b2b_price = get_post_meta( $post_id, '_amazon_b2b_price', true );
		update_post_meta( $post_id, '_amazon_b2b_price', $_amazon_b2b_price );
		
		// If B2B price was cleared (had value, now empty), trigger PATCH feed to remove B2B pricing
		if ( ! empty( $old_b2b_price ) && empty( $_amazon_b2b_price ) ) {
			WPLA()->logger->info('B2B price cleared for product #' . $post_id . ' (was: "' . $old_b2b_price . '", now: "' . $_amazon_b2b_price . '")');
			$this->triggerB2BPriceDeletionFeed( $post_id );
		}
		...
		update_post_meta( $post_id, '_amazon_id_type', 				strtolower(wpla_clean($_POST['wpl_amazon_id_type']  ?? '')) );
        ...
        if ( is_null( $profile_id ) ) {
            $profile_id = (!empty( $_POST['wpla_current_profile_id'] ) ) ? intval( $_POST['wpla_current_profile_id'] ) : null;
        }

        // if no profile ID is set, use the default from the settings
        if ( is_null( $profile_id ) ) {
    ...
    function auto_match_ASIN( $post_id, $profile_id = null ) {
		...
		$listing_id = $lm->insertMatchedProduct( $post_id, $asin, $account_id, $profile_id );
		...
		foreach ( $variation_meta_fields as $key => $varmeta ) :
            ...
            <div>
                <p class="form-row form-row-full">
                    ...
                    <input type="text" name="variable_wpla_<?php echo esc_attr( $key ); ?>[<?php echo esc_attr( $loop ); ?>]" class="" value="<?php echo esc_attr( $current_value ); ?>" placeholder="" />
                </p>
            </div>
    ...
    function woocommerce_variation_options( $loop, $variation_data, $variation ) {
		...
		$_amazon_id_type        = strtolower(get_post_meta( $variation_post_id, '_amazon_id_type'  		, true ));
        ...
        ?>
            ...
            <div>
                ...
                <p class="form-row form-row-first">
                    ...
                    <input type="text" name="variable_amazon_product_id[<?php echo $loop; ?>]" class="" value="<?php echo esc_attr( $_amazon_product_id ); ?>" placeholder="UPC / EAN" />
                </p>
            ...
            <div>
                <p class="form-row form-row-first">
                    ...
                    <input type="text" id="variable_amazon_asin_<?php echo $loop; ?>" name="variable_amazon_asin[<?php echo $loop; ?>]" class="" value="<?php echo esc_attr( $_amazon_asin ); ?>" />
                </p>
                <p class="form-row form-row-last">
                	...
                	<?php
                		...
                		$onclick  = 'window.wpla_matching_asin_field_id = "variable_amazon_asin_'.esc_js($loop).'";';
                		$onclick .= 'tb_show("Match variation #'.$variation_post_id.' on Amazon", "'.$tb_url.'");';
                		$onclick .= 'return false;';
                	?>
                    <a href="#" onclick='<?php echo esc_js( $onclick ); ?>' class="button">
            ...
            <div>
                <p class="form-row form-row-first">
                    ...
                    <input type="text" name="variable_amazon_price[<?php echo $loop; ?>]" class="wc_input_price" value="<?php echo esc_attr( $_amazon_price ); ?>" />
                </p>
                <?php if ( get_option('wpla_load_b2b_templates',0) ): ?>
                    <p class="form-row form-row-last">
                        ...
                        <input type="text" name="variable_amazon_b2b_price[<?php echo $loop; ?>]" value="<?php echo esc_attr( $_amazon_b2b_price ); ?>" placeholder="B2B Price or shortcode" />
                    </p>
                <?php endif; ?>
            </div>
            <?php endif; ?>
            ...
            <div>
                <p class="form-row form-row-first">
                    ...
                    <input type="text" name="variable_amazon_minimum_price[<?php echo $loop; ?>]" class="wc_input_price" value="<?php echo esc_attr( $_amazon_minimum_price ); ?>" />
                </p>
                <p class="form-row form-row-last">
                    <label>
                    ...
                    <input type="text" name="variable_amazon_maximum_price[<?php echo $loop; ?>]" class="wc_input_price" value="<?php echo esc_attr( $_amazon_maximum_price ); ?>" />
                </p>
            </div>
            <?php endif; ?>
            ...
            <div>
                ...
                <p class="form-row form-row-last">
                    ...
                    <input type="text" name="variable_amazon_condition_note[<?php echo $loop; ?>]" class="" value="<?php echo esc_attr( $_amazon_condition_note ); ?>" maxlength="1000" />
                </p>
            </div>
            <?php endif; ?>
            ...
            <div>
                <p class="form-row form-row-first">
                    ...
                    <input type="text" name="variable_amazon_handling_time[<?php echo $loop; ?>]" class="" value="<?php echo esc_attr( $_amazon_handling_time ); ?>" />
                </p>
            ...
            <p class="form-row form-row-first">
                ...
                <input type="text" name="variable_amazon_restock_date[<?php echo $loop; ?>]" class="" value="<?php echo esc_attr( $_amazon_restock_date ); ?>" placeholder="MM/DD/YYYY" />
            </p>
            </div>
    ...
    public function process_product_meta_variable( $post_id ) {
        ...
        for ( $i=0; $i <= $max_loop; $i++ ) {
            ...
            $variation_id = (int) $variable_post_id[$i];
            $current_variable_amazon_product_id = $variable_amazon_product_id[$i] ?? '';
            ...
            update_post_meta( $variation_id, '_amazon_id_type', 		            strtolower($current_variable_amazon_id_type) );
            ...
            // Handle B2B price changes for variations and trigger PATCH feed if needed
            $old_variation_b2b_price = get_post_meta( $variation_id, '_amazon_b2b_price', true );
            update_post_meta( $variation_id, '_amazon_b2b_price',                  $variation_b2b_price );
            
            // If B2B price was cleared (had value, now empty), trigger PATCH feed to remove B2B pricing
            if ( ! empty( $old_variation_b2b_price ) && empty( $variation_b2b_price ) ) {
                WPLA()->logger->info('B2B price cleared for variation #' . $variation_id . ' (was: "' . $old_variation_b2b_price . '", now: "' . $variation_b2b_price . '")');
                $this->triggerB2BPriceDeletionFeed( $variation_id );
            }
            ...
            if ( $current_variable_amazon_asin ) {
                $all_variations_with_ASIN[ $variation_id ] = trim( $current_variable_amazon_asin );
            }
	...
	/**
	 * Trigger a PATCH feed to remove B2B pricing from a product listing
	 *
	 * @param int $product_id Product ID that had B2B pricing removed
	 * @return void
	 */
	private function triggerB2BPriceDeletionFeed( $product_id ) {
		try {
			WPLA()->logger->info('Triggering B2B price deletion PATCH feed for product #' . $product_id);

			// Get the listing for this product
			$all_listings = WPLA_ListingQueryHelper::getWhere( 'post_id', $product_id );
			...
			if ( empty( $listings ) ) {
				WPLA()->logger->info('No updatable listings found for product #' . $product_id . ' - skipping B2B deletion PATCH');
				return;
			}

			// Create the PATCH feed to remove B2B pricing
			// Note: removeB2BPricing() handles account selection internally via groupItemsByProfileAccount()
			...
			if ( empty( $accounts ) ) {
				WPLA()->logger->error('No Amazon accounts found for B2B deletion PATCH');
				return;
			}

			$fallback_account = reset( $accounts );
			$builder->removeB2BPricing( $listings, $fallback_account, 'PRODUCT' );

			WPLA()->logger->info('B2B price deletion PATCH feed triggered successfully for product #' . $product_id);

		} catch ( Exception $e ) {
			WPLA()->logger->error('Failed to trigger B2B price deletion PATCH feed for product #' . $product_id . ': ' . $e->getMessage());
		}
	}
