...
class WPLA_WooBackendIntegration extends WPLA_Core {
	...
	function __construct() {
        ...
        // Handle price changes via WC object property updates
        add_action( 'woocommerce_product_object_updated_props', array( $this, 'handle_product_price_change' ), 10, 2 );
        // Handle WooCommerce scheduled sale expiration
        add_action( 'woocommerce_scheduled_sales', array( $this, 'handle_scheduled_sale_expiration' ), 10 );
        // Look for complete product import
        add_action( 'parse_query', array($this, 'check_for_completed_product_import') );

        // Detect WooCommerce CSV import start - works for ALL import modes (AJAX, background, direct)
        // These hooks fire before/during import regardless of processing mode #72613
        add_action( 'woocommerce_product_import_before_import', array( $this, 'start_wc_import_mode' ) );
        add_action( 'woocommerce_product_importer_before_set_parsed_data', array( $this, 'start_wc_import_mode' ) );
        ...
        if ( is_admin() ) {
            ...
            if ( class_exists( 'Automattic\WooCommerce\Utilities\OrderUtil' ) && \Automattic\WooCommerce\Utilities\OrderUtil::custom_orders_table_usage_is_enabled() ) {
                ...
                add_filter( 'woocommerce_hpos_generate_where_for_search_filter', array( $this, 'add_hpos_amazon_meta_search' ), 10, 4 );
            } else {
	...
	function wpla_on_woocommerce_product_quick_edit_save( $post_id, $post ) {
        ...
        // Skip feed generation during WooCommerce CSV import #65915
        // The import detection check is done here because this save_post handler
        // fires during imports and bypasses the woocommerce_update_product handler
        if ( self::is_wc_import_in_progress() ) {
            $lm = new WPLA_ListingsModel();
            $lm->markItemAsModified( $post_id, true ); // skip_updating_feeds = true
            return;
        }
...
//                WPLA()->logger->info( 'REST account: '. print_r( $account, 1 ) );
    ...
    function handle_product_update( $product_id ) {
        ...
        // Check for WooCommerce CSV import (works for ALL modes: AJAX, background, direct) #72613
        if ( self::is_wc_import_in_progress() ) {
            do_action( 'wpla_product_has_changed', $product_id, true ); // skip_feeds = true
            return;
        }

        // Check for other bulk edit tools
    ...
    public function handle_product_stock_update( $product ) {
        ...
        // Check for WooCommerce Bulk Stock Management plugin - skip feeds during bulk updates #73305
        if ( did_action( 'wc_bulk_stock_before_process_qty' ) ) {
            $skip_feeds = true;
        }
    ...
    /**
     * Handle price changes detected via woocommerce_product_object_updated_props hook.
     * This hook is fired when WC_Product object properties are updated, allowing us to detect
     * specific price changes and set pnq_status to queue the changes for Amazon.
     * 
     * Enhanced to detect sale price removal and trigger offer clearing when needed.
     * 
     * @param WC_Product $product The product object that was updated
     * @param array $updated_props Array of property names that were updated
     */
    public function handle_product_price_change( $product, $updated_props ) {
        // Prevent fatal error if invalid product object
        if ( !is_object( $product ) ) {
            WPLA()->logger->info( 'handle_product_price_change() - Invalid product object received' );
            return;
        }

        // Check if price-related properties were updated
        $price_props = array( 'regular_price', 'sale_price' );
        $price_changed = false;
        ...
        // If no price properties were updated, skip processing
        if ( ! $price_changed ) {
            return;
        }
        ...
        // Handle variations - get parent ID if this is a variation
        if ( $product->is_type( 'variation' ) ) {
            $product_id = method_exists( $product, 'get_parent_id' ) ? $product->get_parent_id() : $product->get_parent();
        }

        // Skip if certain actions have been triggered to prevent unnecessary updates
        if (
            apply_filters( 'wpla_force_product_update_action', false ) === false && (
                //did_action( 'woocommerce_process_product_meta' ) ||
                did_action( 'wp_ajax_woocommerce_save_variations' ) ||
            ...
            WPLA()->logger->info( 'handle_product_price_change() - Skipping price change processing due to ongoing action for product #'. $product_id );
            return;
        }

        // Skip feed generation during bulk imports (WP All-Import and WooCommerce CSV Import)
        $skip_feeds = false;
        if ( did_action( 'wp_all_import_before_make_product_simple' ) || did_action( 'wp_all_import_before_variable_product_import' ) ) {
            $skip_feeds = true;
        }
        ...
        if ( self::is_wc_import_in_progress() ) {
            $skip_feeds = true;
            WPLA()->logger->info( 'handle_product_price_change() - Skipping feed generation during WooCommerce CSV import for product #'. $product_id );
        }

        WPLA()->logger->info( 'handle_product_price_change() - Price changed for product #'. $product_id . ', updated props: ' . implode( ', ', $updated_props ) );

        // ===== ENHANCED SALE PRICE REMOVAL DETECTION =====
        // Check if sale_price was specifically updated and handle offer clearing logic
        // Skip sale price clearing during bulk imports to avoid performance issues
        if ( in_array( 'sale_price', $updated_props ) && ! $skip_feeds ) {
            $this->handle_sale_price_change( $product, $original_product_id, $product_id );
        }

        // Trigger the standard product change action to update listings
        do_action( 'wpla_product_has_changed', $product_id, $skip_feeds );
    }

    /**
     * Handle sale price changes with comprehensive offer clearing logic
     *
     * @param WC_Product $product The product object that was updated
     * @param int $original_product_id The original product ID (variation ID if applicable)
     * @param int $product_id The effective product ID (parent ID for variations)
     * @return void
     */
    private function handle_sale_price_change( $product, $original_product_id, $product_id ) {
        // Get current sale price
        $current_sale_price = $product->get_sale_price( 'edit' );
        
        // Get stored previous sale price for comparison
        $previous_sale_price = get_post_meta( $product_id, '_wpla_previous_sale_price', true );
        
        // Detect different scenarios of sale price changes
        $sale_price_removed = false;
        $sale_price_scenario = '';
        
        // Scenario 1: Sale price was explicitly cleared
        if ( ! empty( $previous_sale_price ) && empty( $current_sale_price ) ) {
            $sale_price_removed = true;
            $sale_price_scenario = 'explicit_removal';
        }
        
        // Scenario 2: Check for scheduled sale expiration
        elseif ( ! empty( $previous_sale_price ) ) {
            $sale_price_removed = $this->check_scheduled_sale_expiration( $original_product_id, $current_sale_price );
            if ( $sale_price_removed ) {
                $sale_price_scenario = 'scheduled_expiration';
            }
        }
        
        // Scenario 3: Bulk edit detection - check if this might be part of bulk operation
        $is_bulk_operation = $this->detect_bulk_operation();
        
        // Get detailed debug information for logging
        $debug_info = $this->get_price_change_debug_info( $product, $original_product_id );
        
        // Log the sale price change details
        WPLA()->logger->info( sprintf(
            'handle_sale_price_change() - Product #%d: previous_sale_price="%s", current_sale_price="%s", scenario="%s", bulk_operation=%s, debug_info=%s',
            $product_id,
            $previous_sale_price,
            $current_sale_price,
            $sale_price_scenario,
            $is_bulk_operation ? 'true' : 'false',
            wp_json_encode( $debug_info )
        ) );

        // Trigger offer clearing if sale price was removed
        if ( $sale_price_removed ) {
            // Directly trigger the sale price clearing PATCH feed
            $result = $this->triggerSalePriceClearingFeed( $product_id );

            if ( $result ) {
                WPLA()->logger->info( sprintf(
                    'handle_sale_price_change() - Sale price removed for product #%d via %s, PATCH feed triggered',
                    $product_id,
                    $sale_price_scenario
                ) );
            } else {
                WPLA()->logger->error( sprintf(
                    'handle_sale_price_change() - Failed to trigger PATCH feed for product #%d via %s',
                    $product_id,
                    $sale_price_scenario
                ) );
            }

            // Add action hook for extensibility
            do_action( 'wpla_sale_price_removed', $product_id, $sale_price_scenario, array(
        ...
        // Update stored previous sale price for next comparison
    ...
    /**
     * Check if a scheduled sale has expired
     *
     * @param int $product_id Product ID to check
     * @param string $current_sale_price Current sale price from product object
     * @return bool True if scheduled sale expired
     */
    private function check_scheduled_sale_expiration( $product_id, $current_sale_price ) {
        // Get sale end date
        $sale_end_date = get_post_meta( $product_id, '_sale_price_dates_to', true );
        
        // If no end date is set, it's not a scheduled sale
        if ( empty( $sale_end_date ) ) {
            return false;
        }
        
        // Check if sale end date has passed and sale price was cleared
        $current_time = current_time( 'timestamp' );
        $sale_expired = ( $current_time > $sale_end_date );
        
        // Additional check: if sale expired and current sale price is empty, it's expiration
        if ( $sale_expired && empty( $current_sale_price ) ) {
            return true;
        }
    ...
    /**
     * Detect if current operation is part of a bulk edit
     *
     * @return bool True if bulk operation detected
     */
    private function detect_bulk_operation() {
        // Check for various bulk operation indicators
        $bulk_indicators = array(
            // WordPress bulk edit
            did_action( 'wp_ajax_inline_save' ),
            did_action( 'bulk_edit_posts' ),
            
            // WooCommerce bulk edit
            did_action( 'woocommerce_product_bulk_edit_save' ),
            did_action( 'woocommerce_bulk_edit_variations' ),
            
            // Quick edit
            did_action( 'wp_ajax_inline_save_tax' ),
            
            // Import operations
            did_action( 'wp_all_import_before_make_product_simple' ),
            did_action( 'wp_all_import_before_variable_product_import' ),
            
            // REST API bulk operations
            did_action( 'woocommerce_rest_pre_insert_product_object' ),
        );
        
        foreach ( $bulk_indicators as $indicator ) {
            if ( $indicator ) {
                return true;
            }
        }
        
        // Check $_POST for bulk edit indicators
        if ( isset( $_POST['bulk_edit'] ) || isset( $_POST['_inline_edit'] ) ) {
            return true;
        }
        
        // Check for batch processing patterns in request
	...
	/**
     * WooCommerce redirects the browser to the URL edit.php?post_type=product&page=product_importer&step=done after
     * it finishes importing a CSV of products. WPLA needs to build a feed to update all products affected by the import.
     *
	 * @return void
	 */
    public function check_for_completed_product_import() {
        $post_type = $_REQUEST['post_type'] ?? '';
        $page = $_REQUEST['page'] ?? '';
        $step = $_REQUEST['step'] ?? '';
        $feeds_updated = get_transient( 'wpla_product_importer_complete' );

        if ( !$feeds_updated && $post_type == 'product' && $page == 'product_importer' && $step == 'done' ) {
            // Clear import mode flag first
            delete_transient( 'wpla_wc_import_in_progress' );
            WPLA()->logger->info( 'WooCommerce CSV import completed - clearing import mode and updating feeds' );

	        // update pending feed
	        WPLA_AmazonFeed::updatePendingFeeds();
            set_transient( 'wpla_product_importer_complete', true, 60 );
        }

    }

    /**
     * Set flag when WooCommerce CSV import starts.
     * This works for ALL import modes: AJAX batched, background processing, and direct.
     * The flag is cleared when check_for_completed_product_import() detects import completion.
     *
     * @since 2.9-beta13
     * @see #72613 - Tim's CSV import performance issue
     */
    public function start_wc_import_mode() {
        // Set transient for 2 hours - should be plenty for most imports
        // Will be cleared when import completes via check_for_completed_product_import()
        if ( ! get_transient( 'wpla_wc_import_in_progress' ) ) {
            set_transient( 'wpla_wc_import_in_progress', true, 2 * HOUR_IN_SECONDS );
            WPLA()->logger->info( 'WooCommerce CSV import detected - enabling import mode to defer feed generation' );
        }
    }

    /**
     * Check if a WooCommerce CSV import is currently in progress.
     * Returns true if import mode is active (either via transient or $_REQUEST detection).
     *
     * @since 2.9-beta13
     * @return bool
     */
    public static function is_wc_import_in_progress() {
        // Check transient first (works for background processing)
        if ( get_transient( 'wpla_wc_import_in_progress' ) ) {
            return true;
        }

        // Also check $_REQUEST for AJAX mode (belt and suspenders)
		...
		if ( $typenow == 'product' && !empty( $_GET['is_on_amazon'] ) ) {
			if ( $_GET['is_on_amazon'] == 'yes' ) {
				$where .= " AND (
							    ...
							    AND {$wpdb->prefix}amazon_listings.post_id IS NOT NULL
							    )
							    OR {$wpdb->posts}.ID IN (
				...
				$where .= " AND {$wpdb->posts}.ID NOT IN (
							...
							AND {$wpdb->posts}.ID NOT IN (
                                SELECT {$wpdb->prefix}amazon_listings.parent_id
                                FROM {$wpdb->prefix}amazon_listings
                                WHERE {$wpdb->prefix}amazon_listings.parent_id IS NOT NULL
                            )";
	...
	function wpla_product_submitbox_misc_actions() {
		...
		$current_profile = ( $post->ID ) ? WPLA_AmazonProfile::getProfileForProduct( $post->ID ) : '';
		?>
        <input type="hidden" name="wpla_current_profile_id" value="<?php echo $current_profile ?>" />
        ...
        <script type="text/javascript">
			jQuery( document ).ready( function () {
				...
				jQuery('#woocommerce-product-data').on('woocommerce_variations_loaded', function(event) {
					jQuery('#variable_product_options_inner .woocommerce_variable_attributes p[class*="variable_sku"] input').each(function() {
	...
	function wpla_add_woocommerce_order_views( $views ) {
		...
		$query_string = esc_url_raw( remove_query_arg(array( 'is_from_amazon', 'is_from_ebay' )) );
	...
	/**
	 * Add Amazon order ID meta search to HPOS WHERE clauses
	 *
	 * This hooks into the HPOS search system to ensure Amazon order IDs are searchable
	 ...
	 */
	public function add_hpos_amazon_meta_search( $where, $search_term, $search_filter, $query ) {
		global $wpdb;
		...
		// Build meta search for Amazon order IDs
		$meta_where = $wpdb->prepare(
			"$orders_table.id IN (
				SELECT meta.order_id
				FROM $meta_table as meta
				WHERE meta.meta_key = '_wpla_amazon_order_id'
    ...
    /**
     * Handle WooCommerce scheduled sale expiration cron
     * This method is triggered when WooCommerce runs its scheduled sales cron job
     * and processes products that need offer clearing due to sale expiration
     *
     * @return void
     */
    public function handle_scheduled_sale_expiration() {
        WPLA()->logger->info( 'handle_scheduled_sale_expiration() - Processing scheduled sale expiration' );

        // Get products with scheduled sale end dates that have passed
        global $wpdb;
        
        // Security: Ensure current_time returns a valid integer timestamp
        $current_time = absint( current_time( 'timestamp' ) );
        if ( ! $current_time ) {
            WPLA()->logger->error( 'handle_scheduled_sale_expiration() - Invalid current_time value' );
            return;
        }
        
        // Query for products with expired sales
        // Security: Using $wpdb->prepare() with %d placeholder to prevent SQL injection
        // The $current_time parameter is properly sanitized through prepare()
        $expired_sale_products = $wpdb->get_results( $wpdb->prepare("
            SELECT p.ID 
            FROM {$wpdb->posts} p
            INNER JOIN {$wpdb->postmeta} pm1 ON p.ID = pm1.post_id AND pm1.meta_key = '_sale_price_dates_to'
        ...
        if ( empty( $expired_sale_products ) ) {
            WPLA()->logger->info( 'handle_scheduled_sale_expiration() - No expired sale products found' );
            return;
        }

        $processed_count = 0;
        foreach ( $expired_sale_products as $product_data ) {
            // Validate and sanitize product ID from database results
            $product_id = absint( $product_data->ID );
            if ( ! $product_id ) {
                WPLA()->logger->error( 'handle_scheduled_sale_expiration() - Invalid product ID from database results: ' . var_export( $product_data->ID, true ) );
                continue;
            }
            
            // Get the product object to validate
            $product = wc_get_product( $product_id );
            if ( ! $product ) {
                continue;
            }

            // Check if this product has WPLA listings
            $lm = new WPLA_ListingsModel();
            $original_product_id = $product_id;
            
            // For variations, work with parent product ID for WPLA
            if ( $product->is_type( 'variation' ) ) {
                $product_id = method_exists( $product, 'get_parent_id' ) ? $product->get_parent_id() : $product->get_parent();
            }
            ...
            if ( empty( $listings ) ) {
                continue; // Skip products without WPLA listings
            }

            WPLA()->logger->info( sprintf(
                'handle_scheduled_sale_expiration() - Processing expired sale for product %d (original: %d)',
                $product_id,
                $original_product_id
            ) );
            ...
            if ( $result ) {
                // Add action hook for extensibility
                do_action( 'wpla_sale_price_expired', $product_id, array(
                    'original_product_id' => $original_product_id,
                    'expiration_method' => 'woocommerce_cron'
                ) );

                $processed_count++;
            } else {
                WPLA()->logger->error( sprintf(
                    'handle_scheduled_sale_expiration() - Failed to mark product %d (original: %d) for offer clearing',
                    $product_id,
                    $original_product_id
                ) );
            }
        ...
        WPLA()->logger->info( sprintf(
            'handle_scheduled_sale_expiration() - Processed %d products for sale expiration offer clearing',
            $processed_count
        ) );
    }

    /**
     * Get detailed information about price changes for debugging
     *
     * @param WC_Product $product Product object
     * @param int $product_id Product ID
     * @return array Debug information array
     */
    private function get_price_change_debug_info( $product, $product_id ) {
        return array(
            ...
            'sale_end_date' => get_post_meta( $product_id, '_sale_price_dates_to', true ),
            'previous_sale_price' => get_post_meta( $product_id, '_wpla_previous_sale_price', true ),
            'offer_clearing_flag' => get_post_meta( $product_id, '_wpla_needs_offer_clearing', true ),
            'current_time' => current_time( 'timestamp' ),
            'actions_fired' => array(
                'woocommerce_process_product_meta' => did_action( 'woocommerce_process_product_meta' ),
                'wp_ajax_woocommerce_save_variations' => did_action( 'wp_ajax_woocommerce_save_variations' ),
                'woocommerce_scheduled_sales' => did_action( 'woocommerce_scheduled_sales' ),
            )
        );
    }

    /**
     * Trigger a PATCH feed to clear sale price/offers from a product listing
     *
     * @param int $product_id Product ID that had sale price removed
     * @return bool True on success, false on failure
     */
    private function triggerSalePriceClearingFeed( $product_id ) {
        try {
            WPLA()->logger->info('Triggering sale price clearing PATCH feed for product #' . $product_id);

            // Get the listing for this product (including variations via parent_id)
            $lm = new WPLA_ListingsModel();
            $all_listings = $lm->getAllItemsByPostOrParentID( $product_id );

            // Filter for listings that can be updated (online, changed, or matched status)
            $active_listings = array_filter( $all_listings, function( $listing ) {
                return in_array( $listing->status, ['online', 'changed', 'matched'] );
            });
            ...
            if ( empty( $listings ) ) {
                WPLA()->logger->info('No updatable listings found for product #' . $product_id . ' - skipping sale price clearing PATCH');
                return false;
            }

            // Create the PATCH feed to clear sale price/offers
            // Note: clearOffers() handles account selection internally via groupItemsByProfileAccount()
            $builder = new \WPLab\Amazon\Helper\JsonFeedDataBuilder();

            // Get a fallback account (first available account)
            $accounts = WPLA_AmazonAccount::getAll();
            if ( empty( $accounts ) ) {
                WPLA()->logger->error('No Amazon accounts found for sale price clearing PATCH');
                return false;
            }

            $fallback_account = reset( $accounts );
            $builder->clearOffers( $listings, $fallback_account, 'PRODUCT' );

            WPLA()->logger->info('Sale price clearing PATCH feed triggered successfully for product #' . $product_id);
            return true;

        } catch ( Exception $e ) {
            WPLA()->logger->error('Failed to trigger sale price clearing PATCH feed for product #' . $product_id . ': ' . $e->getMessage());
            return false;
        }
    }
