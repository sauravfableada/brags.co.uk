...
class WPLA_Shipping_Method extends WC_Shipping_Method {
    ...
    protected $discount_mode;
    protected $show_estimated_arrival;
    protected $standard_fixed_fee;
    protected $standard_percent_fee;
    protected $expedited_fixed_fee;
    protected $expedited_percent_fee;
    protected $priority_fixed_fee;
    protected $priority_percent_fee;
    ...
    public function calculate_shipping( $package = array() ) {
	    // Check if FBA Shipping Options is enabled
        $fba_enabled = get_option( 'wpla_fba_enabled' );
	    $fba_shipping_enabled = get_option('wpla_fba_wc_shipping_options');

	    if ( !$fba_enabled || ($fba_shipping_enabled != '1' && $fba_shipping_enabled != 1) ) {
		    WPLA()->logger->info("WPLASO: calculate_shipping() - FBA or FBA Shipping Options is disabled, skipping calculation");
		    return;
	    }
        ...
        foreach ( WPLA()->accounts as $account ) {
	        ...
	        if ( isset( $result->success ) && $result->success ) {
		        foreach ( $result->previews as $preview ) {
			        ...
			        $shipping_speed = $preview->ShippingSpeedCategory->value;

			        // build label (added filter #14710)
			        $method_label = apply_filters( 'wpla_shipping_method_label', $shipping_speed, $preview, $this, $package );
			        ...
			        if ( $this->discount_mode == 'free_standard' ) {
				        if ( 'Standard' == $shipping_speed ) {
					        $shipping_discount = $preview->TotalShippingFee;
				        }
			        }

			        $shipping_fee = $this->apply_addon_fee( $preview->TotalShippingFee, $shipping_speed );
