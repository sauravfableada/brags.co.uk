...
//        $lock_row_sql  = apply_filters( 'wpla_lock_feeds_table', false ) ? ' FOR UPDATE ' : '';
        ...
        if ( $item ) {
            // Only rebuild from items table if that feature is enabled
            if ( $item->FeedType == 'POST_FLAT_FILE_FULFILLMENT_DATA' && get_option( 'wpla_feed_items_table', 0 ) ) {
		...
		$items = $wpdb->get_results( $wpdb->prepare("
			...
			WHERE account_id = %s
			  AND ( FeedProcessingStatus = '_SUBMITTED_'
			  	 ...
			  	 OR status = %s )
			  AND FeedProcessingStatus NOT IN ('FATAL', 'CANCELLED', 'DONE')
	...
	/**
	 * Check if feedOptions contains PHP serialized data (internal metadata)
	 * rather than Amazon API format (key=value pairs separated by semicolons)
	 *
	 * @since 2.9
	 * @param string $feed_options Feed options string to check
	 * @return bool True if serialized data detected, false otherwise
	 */
	function isFeedOptionsSerializedData( $feed_options ) {
		if ( empty( $feed_options ) ) {
			return false;
		}

		// Use WordPress core function to detect serialized data
		// This handles all PHP serialization formats: a: (array), s: (string), O: (object),
		// b: (boolean), i: (integer), d: (float), N: (null)
		return is_serialized( $feed_options );
	}
	...
	function submit() {
        ...
        // Reload fulfillment feed data fresh from DB to pick up any items appended
        // after this object was constructed (e.g. a shipment webhook that fired while
        // the cron was processing a previous feed in the same batch). Only needed for
        // fulfillment feeds since those are the only type that accumulate items
        // incrementally via webhooks.
        if ( $this->FeedType === 'POST_FLAT_FILE_FULFILLMENT_DATA' ) {
            $fresh = self::getFeed( $this->id );
            if ( $fresh ) {
                // Bail if another process already submitted this feed between our
                // initial status check and now (prevents double-submission to Amazon).
                if ( $fresh->status !== self::STATUS_PENDING ) {
                    WPLA()->logger->info( sprintf( 'submit() - feed #%d already submitted by another process', $this->id ) );

                    // Move any items from the submitted feed into a new pending feed
                    // so they get a chance to be submitted on the next cron run.
                    if ( get_option( 'wpla_feed_items_table', 0 ) ) {
                        global $wpdb;

                        $new_feed                      = new WPLA_AmazonFeed();
                        $new_feed->FeedType            = 'POST_FLAT_FILE_FULFILLMENT_DATA';
                        $new_feed->status              = self::STATUS_PENDING;
                        $new_feed->account_id          = $this->account_id;
                        $new_feed->date_created        = gmdate( 'Y-m-d H:i:s' );
                        $new_feed->data                = '';
                        $new_feed->line_count          = 0;
                        $new_feed->FeedProcessingStatus = 'pending';
                        $new_feed->add();

                        if ( $new_feed->id ) {
                            $wpdb->update(
                                $wpdb->prefix . 'amazon_fulfillment_feed_items',
                                [ 'feed_id' => $new_feed->id ],
                                [ 'feed_id' => $this->id ],
                                [ '%d' ],
                                [ '%d' ]
                            ...
                            $items_count = $wpdb->get_var( $wpdb->prepare(
                                "SELECT COUNT(*) FROM {$wpdb->prefix}amazon_fulfillment_feed_items WHERE feed_id = %d",
                                $new_feed->id
                            ) );
                            $new_feed->line_count = $items_count;
                            $new_feed->update();

                            WPLA()->logger->info( sprintf(
                                'submit() - moved %d items to new pending feed #%d',
                                $items_count,
                                $new_feed->id
                            ) );
                        }
                ...
                // Always sync line_count from the reload so $submitted_count (captured
                // just before submitFeed) accurately reflects the items table at reload time.
                $this->line_count = $fresh->line_count;
            }
        }
        ...
        // but only if the FeedType is not UPLOAD_VAT_INVOICE (PDFs need UTF8) or JSON_LISTINGS_FEED (JSON requires UTF8)
        if ( get_option( 'wpla_feed_encoding' ) != 'UTF-8' && $this->FeedType != 'UPLOAD_VAT_INVOICE' && $this->FeedType != 'JSON_LISTINGS_FEED' ) {
            $decode_feed = true;
        }
        ...
        // Don't pass feedOptions to Amazon if it contains serialized data (used for internal metadata like PATCH feeds)
        // Amazon expects feedOptions in format "key1=value1;key2=value2", not PHP serialized data
        $feed_options_to_pass = $this->feedOptions;
        if ( !empty( $this->feedOptions ) && $this->isFeedOptionsSerializedData( $this->feedOptions ) ) {
            WPLA()->logger->info(
                sprintf(
                    'Feed #%d (%s): Skipping serialized feedOptions (not API format)',
                    $this->id,
                    $this->FeedType
                )
            );

            // Log the actual value at debug level for troubleshooting
            if ( get_option( 'wpla_log_level', 0 ) >= 7 ) {
                WPLA()->logger->debug(
                    sprintf(
                        'Feed #%d feedOptions value: %s',
                        $this->id,
                        substr( $this->feedOptions, 0, 200 )
                    )
                );
            }

            $feed_options_to_pass = null;
        }

        // Snapshot the item count we are about to send so we can validate after submission.
        $submitted_count = (int) $this->line_count;

        $result = $api->submitFeed( $feed_type['name'], $feed_content, $feed_options_to_pass, $this->MarketplaceIdList );
		// echo "<pre>";print_r($result);echo"</pre>";die();
        ...
        if ( !WPLA_Amazon_SP_API::isError( $result ) ) {
            ...
            // Post-submission validation: detect items inserted between the pre-submit
            // reload and the Amazon API call completing. Those items were not included
            // in the submitted feed — rescue them into a new pending feed.
            if (
                $this->FeedType === 'POST_FLAT_FILE_FULFILLMENT_DATA' &&
                get_option( 'wpla_feed_items_table', 0 )
            ) {
                global $wpdb;
                ...
                if ( $db_count > $submitted_count ) {
                    WPLA()->logger->warn( sprintf(
                        'submit() - feed #%d: submitted %d item(s) but DB has %d — %d late item(s) will be rescheduled',
                        $this->id,
                        $submitted_count,
                        $db_count,
                        $db_count - $submitted_count
                    ) );

                    $rescue_feed                       = new WPLA_AmazonFeed();
                    $rescue_feed->FeedType             = 'POST_FLAT_FILE_FULFILLMENT_DATA';
                    $rescue_feed->status               = self::STATUS_PENDING;
                    $rescue_feed->account_id           = $this->account_id;
                    $rescue_feed->date_created         = gmdate( 'Y-m-d H:i:s' );
                    $rescue_feed->data                 = '';
                    $rescue_feed->line_count           = 0;
                    $rescue_feed->FeedProcessingStatus = 'pending';
                    $rescue_feed->add();

                    if ( ! $rescue_feed->id ) {
                        WPLA()->logger->error( sprintf(
                            'submit() - feed #%d: failed to create rescue feed for %d late item(s) — they will not be retried',
                            $this->id,
                            $db_count - $submitted_count
                        ) );
                    } else {
                        // Target the late-arriving rows by their auto-increment IDs.
                        // Items inserted after our reload will have the highest IDs,
                        // since InnoDB auto-increment is strictly monotonic.
                        $late_ids = $wpdb->get_col( $wpdb->prepare(
                            "SELECT id FROM {$wpdb->prefix}amazon_fulfillment_feed_items
                             WHERE feed_id = %d
                             ORDER BY id DESC
                             LIMIT %d",
                            $this->id,
                        ...
                        if ( empty( $late_ids ) ) {
                            // Items disappeared between COUNT and SELECT (deleted in another process).
                            // Clean up the empty rescue feed record to avoid DB pollution.
                            WPLA()->logger->warn( sprintf(
                                'submit() - feed #%d: expected %d late item(s) but none found — removing empty rescue feed #%d',
                                $this->id,
                                $db_count - $submitted_count,
                                $rescue_feed->id
                            ) );
                            ...
                            $placeholders = implode( ',', array_fill( 0, count( $late_ids ), '%d' ) );
                            $wpdb->query( $wpdb->prepare(
                                "UPDATE {$wpdb->prefix}amazon_fulfillment_feed_items
                                 SET feed_id = %d
                                 WHERE id IN ($placeholders)",
                                array_merge( [ $rescue_feed->id ], $late_ids )
                            ) );

                            $rescue_feed->line_count = count( $late_ids );
                            $rescue_feed->update();

                            WPLA()->logger->info( sprintf(
                                'submit() - moved %d late item(s) to new pending feed #%d',
                                count( $late_ids ),
                                $rescue_feed->id
                            ) );
                        }
	                        ...
	                        case 'JSON_LISTINGS_FEED':
								if ( $this->template_name == 'Price & Quantity' ) {
									...
									if ( $is_delete_operation ) {
										...
										// Reset pnq_status since Product Type feeds include price/quantity data.
										// This prevents a redundant PnQ feed from being created. #73171
	...
	function loadSubmissionResult() {
		...
		if ( !WPLA_Amazon_SP_API::isError( $result ) ) {
			if ( is_string( $result ) ) {
				// Remove or replace any null bytes that can cause database issues
				...
				// If still not valid UTF-8, remove invalid sequences
	...
	static function updatePendingFeedForAccount( $account ) {
		...
		foreach ( $grouped_items as $tpl_id => $grouped_inner_items ) {
			...
			foreach ( $grouped_inner_items as $profile_id => $items ) {
				...
				if ( $profile_id == 0 || $template_type == 'LiLo' ) {
					// Filter out items that have a product type at the product level.
					// These will be handled by getPendingProductsForAccountByProductType() later.
					$lilo_items = array_filter( $items, function( $item ) {
						$id = $item['parent_id'] ? $item['parent_id'] : $item['post_id'];
						return empty( get_post_meta( $id, '_wpla_custom_product_type', true ) );
					});

					if ( ! empty( $lilo_items ) ) {
						// Use JSON feeds now for listing loader feeds
						self::buildJsonFeed( $lilo_items, $account, JsonFeedDataBuilder::OPERATION_PARTIAL_UPDATE, 'PRODUCT', null, 'Listing Loader' );
					}
					continue;
				}

				// Allow legacy feed creation but log a warning
				$template_name = $template ? $template->name : 'Unknown';
				$template_title = $template ? $template->title : 'Unknown Template';
				
				WPLA()->logger->warn("Creating legacy feed - template '{$template_name}' ('{$template_title}') is sunset and should be updated");
		...
		WPLA_AmazonFeed::buildJsonFeed( $items, $account, JsonFeedDataBuilder::OPERATION_PARTIAL_UPDATE );
	...
	function updateShipmentFeed( $post_id ) {
		...
		if ( ! $this->id = self::getPendingShipmentFeedId( $account->id ) ) {
            ...
            if ( $use_feed_items_table ) {
                ...
                // Update line_count to reflect items in feed_items table
                $items_count = $wpdb->get_var( $wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->prefix}amazon_fulfillment_feed_items WHERE feed_id = %d", $this->id ) );
                $this->line_count = $items_count;
                $this->update();

                WPLA()->logger->info('added NEW feed - id '.$this->id.' with '.$items_count.' items');
            }
...
//            }
			...
			if ( $use_feed_items_table ) {
                ...
                // Update line_count to reflect current items in feed_items table
                $items_count = $wpdb->get_var( $wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->prefix}amazon_fulfillment_feed_items WHERE feed_id = %d", $this->id ) );
                $this->line_count = $items_count;
...
//            }
	...
	/**
	 * Extract and format SKU information from PATCH feed data
	 *
	 * @param string $feed_data JSON encoded feed data
	 * @return string|null Formatted SKU display string or null if no SKUs found
	 */
	public static function formatPatchFeedSkus( $feed_data, $feed_options = null ) {
		if ( empty( $feed_data ) ) {
			return null;
		}

		// Try to get SKUs from feedOptions first (new format)
		if ( !empty( $feed_options ) ) {
			$options = maybe_unserialize( $feed_options );
			if ( isset( $options['patch_skus'] ) && is_array( $options['patch_skus'] ) ) {
				return self::formatSkuArray( $options['patch_skus'] );
			}
		}

		// Fallback to old header format for backwards compatibility
		$data = json_decode( $feed_data, true );
		if ( isset( $data['header']['skus'] ) && is_array( $data['header']['skus'] ) ) {
			return self::formatSkuArray( $data['header']['skus'] );
		}
	...
	/**
	 * Format an array of SKUs for display
	 *
	 * @param array $skus Array of SKU strings
	 * @return string|null Formatted SKU display string or null if no SKUs
	 */
	public static function formatSkuArray( $skus ) {
		...
		if ( $sku_count === 1 ) {
			...
			return $sku_count . ' SKUs: ' . implode( ', ', array_slice( $skus, 0, 3 ) ) . ( $sku_count > 3 ? '...' : '' );
		}
	}
	...
	function getPageItems( $current_page, $per_page ) {
        ...
        $log_where_sql = ' WHERE callname IN ("putListingItem", "patchListingItem") ';
        ...
        if ( isset( $_REQUEST['feed_status'] ) ) {
            ...
            if ( $status ) {
                ...
                } elseif ( $status == 'new_submissions' ) {
                    // Show only API logs for individual listing submissions
                    $feed_where_sql .= " AND 1=0 "; // Don't show any feeds for new submissions view
                    $log_where_sql .= " AND callname = 'putListingItem' "; // Only show putListingItem API logs
                } elseif ( $status == 'blocked_legacy' ) {
                    // Show only legacy feeds (InventoryLoader, ListingLoader, fptcustom)
                    $feed_where_sql .= " AND (template_name IN ('InventoryLoader', 'ListingLoader', 'LiLo') 
                                            OR template_name LIKE 'fptcustom%'
                                            OR FeedType LIKE '%INVLOADER%'
                                            OR FeedType LIKE '%LISTINGS_DATA%') ";
                    $log_where_sql .= " AND 1=0 "; // Don't show any API logs for blocked legacy view
                } else {
            ...
            // For regular views, hide legacy feeds and don't show API logs
            $feed_where_sql .= " AND (template_name IS NULL OR NOT (template_name IN ('InventoryLoader', 'ListingLoader', 'LiLo')
                                        OR template_name LIKE 'fptcustom%'
                                        OR FeedType LIKE '%INVLOADER%'
                                        OR FeedType LIKE '%LISTINGS_DATA%')) ";
            $log_where_sql .= " AND 1=0 "; // Hide API logs from regular views
        }
		...
		foreach( $items as $item ) {
			...
			if ( $row->source_type == 'feed' ) {
				...
				// Add SKU information for PATCH feeds
				if ( strpos( $row->template_name, '(PATCH)' ) !== false ) {
					$sku_display = self::formatPatchFeedSkus( $row->data, $row->feedOptions );
					if ( $sku_display ) {
						$row->display_reference = $sku_display;
					}
				}
			} else {
	...
	/**
	 * Get count of pending feed items that need processing
	 * @return int
	 */
	public static function getPendingFeedsCount() {
		global $wpdb;
		return (int) $wpdb->get_var("
			SELECT COUNT(*) 
			FROM {$wpdb->prefix}amazon_listings 
			WHERE status IN ('changed', 'prepared', 'matched') 
			AND locked = 0
		");
	}
	...
	static function getStatusSummary() {
		...
		// Apply same filtering as regular views - hide legacy feeds
		$where_clause = " WHERE (template_name IS NULL OR NOT (template_name IN ('InventoryLoader', 'ListingLoader', 'LiLo')
		                            OR template_name LIKE 'fptcustom%'
		                            OR FeedType LIKE '%INVLOADER%'
		                            OR FeedType LIKE '%LISTINGS_DATA%')) ";
		...
		// count total items as well (excluding legacy feeds)
