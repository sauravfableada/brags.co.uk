...
class AmazonProductTypesModel {
	...
	public function installDefaultProductTypes() {
		...
		$active_accounts = array_filter( WPLA()->accounts, fn( $a ) => $a->active );
		$account = current( $active_accounts );
	...
	public static function getByMarketplace( $marketplace_id ) {
		...
		return $wpdb->get_results($wpdb->prepare("SELECT product_type, display_name FROM {$wpdb->prefix}amazon_product_types WHERE marketplace_id = %s ORDER BY display_name ASC", $marketplace_id ) );
	}
	...
	public static function downloadForMarketplace( $marketplace_ids ) {
		$account_id = \WPLA_AmazonAccount::getAccountWithMarketplace( $marketplace_ids[0] );
	...
	public static function getTypesAsDropdownOptions( $account_id, $selected = '' ) {
		...
		// Add Inventory Loader as custom product type for marketplaces that support PRODUCT
		foreach ( $options as $marketplace_id => $types ) {
			...
			if ( $has_product_type ) {
				...
				usort( $options[ $marketplace_id ], function( $a, $b ) {
					return strcmp( $a['product_type'], $b['product_type'] );
				});

				// Add priority options to the top
				array_unshift( $options[ $marketplace_id ],
					[
						'product_type'  => 'LISTING_OFFER_ONLY',
						'display_name'  => 'Offer Only'
					],
					[
						'product_type'  => 'Inventory Loader',
						'display_name'  => 'Inventory Loader'
					]
				);
			}
		}
