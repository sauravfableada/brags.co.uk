...
class WPLA_ListingsModel extends WPLA_Model {
	...
	function getPageItems( $current_page, $per_page, $mode = 'default' ) {
		...
		// 					AND ( product_type <> 'variable' OR product_type IS NULL ) ";
			...
			$where_sql = "WHERE locked = '1' ";
		} elseif ( $listing_status == 'unlocked' ) {
			$where_sql = "WHERE locked <> '1' ";
		} elseif ( $listing_status != '' ) {
			// Use whitelist approach for security - only allow known status values
			$allowed_statuses = array('prepared', 'submitted', 'online', 'matched', 'ended', 'trash', 'sold', 'changed', 'failed', 'archived');
			if ( in_array( $listing_status, $allowed_statuses ) ) {
				$where_sql = $wpdb->prepare("WHERE status = %s", $listing_status);
			} else {
				$where_sql = "WHERE 1 = 1 "; // fallback to show all if invalid status
			}
		} else {
			$where_sql = "WHERE 1 = 1 ";
		}
		...
		if ( $search_query ) {
            ...
            if ( strpos( $search_query, '#' ) === 0 && strpos( $search_query, ',' ) !== false ) {
                ...
                $listing_title_parts = explode( ' ', $search_query );

                $where_queries = [];
                $line_query = '';
                foreach ( $listing_title_parts as $search_term ) {
                    $like = '%' . $wpdb->esc_like( $search_term ) . '%';
                    $line_query .= $wpdb->prepare( " AND ( ( l.listing_title LIKE %s) OR ( l.sku LIKE %s ) )", $like, $like ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                }
                $where_queries[] = $line_query;

                if ( $line_query ) {
                    $where_sql .= ' ' . implode( ') OR (', $where_queries ) . ' ';
                }
	...
	static function getStatusSummary() {
        ...
        //
        // # Only include listings from active accounts #47998
		$result = $wpdb->get_results("
			SELECT l.status, count(l.post_id) as total
			FROM $table l
			LEFT JOIN {$wpdb->prefix}amazon_accounts a ON l.account_id = a.id
	...
	function processBuyBoxPricingResult( $result, $account_id ) {
        ...
        if ( ! $result || WPLA_Amazon_SP_API::isError( $result ) ) return;

		foreach ($result as $asin => $prices ) {
			$this->updateBuyBoxPricingInfoForASIN( $asin, $prices, $account_id );
		}
	...
	function processLowestOfferPricingResult( $result, $account_id ) {
		...
		foreach ( $result as $asin => $offer_result ) {
			$this->updateLowestOfferPricingInfoForASIN( $asin, $offer_result, $account_id );
		}
	...
	function updateBuyBoxPricingInfoForASIN( $asin, $prices, $account_id ) {
        ...
        foreach ( $prices as $price ) {
            ...
            $landed_price = $price->getPrice()->getLandedPrice()->getAmount();
			if ( $landed_price < $lowest_buybox_price ) {

				// ignore seller's own price - to allow automatic price increase
				if ( $price->getBelongsToRequester() ) {
					$seller_has_buybox   = true;
					$lowest_buybox_price = $landed_price;
					continue;
				}
				...
				if ( $price->getCondition() == 'New' ) {
					$lowest_buybox_price = $landed_price;
					$lowest_compet_price = $landed_price;
				} else {
					wpla_show_message('Skipped listing at '.$landed_price.' with condition '.$price->getCondition());
				}
		...
		if ( ! empty($prices) ) {
			...
			$data['buybox_data']  = serialize( $prices );
		} else {
			$data['lowest_price'] = '';
			$data['compet_price'] = '';
			$data['buybox_price'] = '';
        ...
        do_action( 'wpla_buy_box_pricing_updated', $asin, $account_id, $data );
	...
	function updateLowestOfferPricingInfoForASIN( $asin, $offer_result, $account_id ) {
		...
		// Get account's seller ID to exclude own offers
		$account = isset( WPLA()->accounts[$account_id] ) ? WPLA()->accounts[$account_id] : null;
		$merchant_id = $account ? $account->merchant_id : null;

		// Process detailed offers first to properly exclude seller's own offers
		$detailed_offers = $offer_result->getOffers();

		if ( is_array( $detailed_offers ) && !empty( $detailed_offers ) && $merchant_id ) {
            foreach ( $detailed_offers as $offer ) {

                // Skip seller's own offers using SellerId comparison
                if ( $offer->getSellerId() == $merchant_id ) {
                    continue;
                }

                // Only process 'new' condition offers
                if ( strtolower($offer->getSubCondition()) != 'new' ) {
                    continue;
                }

                $listing_price = $offer->getListingPrice()->getAmount();
                $shipping_price = $offer->getShipping() ? $offer->getShipping()->getAmount() : 0;
                $landed_price = $listing_price + $shipping_price;

                // check for lowest competitor price - to allow automatic price increase
                if ( $landed_price < $compet_price ) {
                    $compet_price = $landed_price;
                }

                // check for lowest price of all competitors
                if ( $landed_price < $lowest_price ) {
                    $lowest_price = $landed_price;
                }

            } // each detailed offer
        } else {
            // Fallback to summary pricing if detailed offers not available or no merchant_id
            $prices = $offer_result->getSummary()->getLowestPrices();
            ...
            if ( is_array( $prices ) && !empty( $prices ) ) {
                foreach ( $prices as $price ) {
                    ...
                    $landed_price = $price->getLandedPrice()->getAmount();
                    if (  $landed_price < $lowest_price ) {

                        if ( strtolower($price->getCondition()) == 'new' ) {
                            $lowest_price = $landed_price;
                        } else {
                            wpla_show_message('Skipped offer at '.$landed_price.' with condition '.$price->getCondition());
                        }
		...
		if ( ! empty($offer_result->getOffers()) ) {
			$data['loffer_price'] = $lowest_price;
			$data['loffer_data']  = serialize( $offer_result->getOffers() );
		} else {
			$data['loffer_price'] = '';
			$data['loffer_data']  = '';
		}
    ...
    public function updateCatalogAttributes( $attributes, $id ) {
	    global $wpdb;
        if ( empty( $attributes['item_name'] ) ) {
            WPLA()->logger->error( 'could not update ItemAttributes: '.print_r($attributes,1) );
            return false;
        }

        $data = array();
        $data['listing_title'] = $attributes['item_name'];
    ...
    public function flattenCatalogAttributes( $attributes ) {
	    global $wpdb;
        $flat = [];
        foreach ( $attributes as $key => $attribute_list ) {

            // if there's only 1 element, check and extract the value
            if ( count( $attribute_list ) == 1 ) {
                if ( isset( $attribute_list[0]->value ) ) {
                    $flat[ $key ] = $attribute_list[0]->value;
                } else {
                    $flat[ $key ] = $attribute_list;
                }
            } else {
                // if row has more than 1 element, loop through each and extract the value
                $values = [];
	    ...
	    return json_decode( json_encode( $flat ), false );
    }
	...
	{
		global $wpdb;
        $ItemAttributes = $this->flattenCatalogAttributes( $ItemAttributes );
		// skip if no product...
		if ( empty( $ItemAttributes->item_name ) ) {
			WPLA()->logger->error( 'could not update ItemAttributes: '.print_r($ItemAttributes,1) );
			return false;
		}

		$data = array();
		$data['listing_title'] = $ItemAttributes->item_name;
	...
	function insertMatchedProduct( $post_id, $asin, $account_id, $profile_id = null )
	{
		global $wpdb;

		WPLA()->logger->info( 'insertMatchedProduct #'. $post_id );
		...
		// Assign a default profile to all matched listings #47881
        if ( is_null( $profile_id ) ) {
            $profile_id = get_option( 'wpla_default_matched_profile', null );
            WPLA()->logger->info( 'assigned default profile: '. $profile_id );

            // check if the product is a variation and try to inherit the parent's profile #50048
            if ( $product->is_type( 'variation' ) ) {
                $parent = $this->getItemByPostID( $product->get_parent_id() );

                if ( $parent && $parent->profile_id ) {
                    $profile_id = $parent->profile_id;
                    WPLA()->logger->info( 'Inherited the parent profile: '. $profile_id );
                }
            }
        }
		...
		// Sync min/max prices from product meta to listing for repricer
		...
		if ( $product->is_type( 'variation' ) ) {
			...
			// Get the product's profile from other variations
            if ( $profile_id ) {
                $data['profile_id'] = $profile_id;
            } else {
                $data['profile_id'] = WPLA_AmazonProfile::getProfileForProduct( WPLA_ProductWrapper::getVariationParent( $post_id ) );
            }
	...
	function updateItemFromReportCSV( $csv, $account_id )
	{
		...
		if ( $db_error ) {
            WPLA()->logger->error( $db_error );
            WPLA()->logger->error('sql: '.$wpdb->last_query );

            wpla_show_message('Error: Unable to update listing. Please check your logs for more information.','error');
            return false;
        }

		do_action( 'wpla_item_updated_from_report', $data, $csv, $account_id );
	...
	static function mapMerchantReportItemCSVToDB( $csv ) {
		...
		$data['description']		= $csv['item-description'] ? self::convertToUTF8( $csv['item-description'] ) : '';
		...
		$csv['merchant-shipping-group'] = self::convertToUTF8( @$csv['merchant-shipping-group'] );
	...
	static function convertToUTF8( $string ) {
        if ( !seems_utf8( $string ) && function_exists( 'mb_convert_encoding' ) ) {
            ...
            // return $string immediately to prevent it from going through utf8_encode unnecessarily #56176
            return $string;
        }
	...
	static public function updateBySkuAndAccount( $sku, $account_id, $data ) {
        ...
        if ( ! get_option( 'wpla_case_sensitive_sku_matching', 0 ) ) {
            ...
            $result = self::updateWhere( array( 'sku' => $sku, 'account_id' => $account_id ), $data );
            
            // Trigger hook for FBA stock monitoring if fba_quantity was updated
            if ($result !== false && isset($data['fba_quantity'])) {
                // Get affected listing IDs for the hook
                $listing_ids = self::getListingIdsByWhere(array( 'sku' => $sku, 'account_id' => $account_id ));
                foreach ($listing_ids as $listing_id) {
                    do_action('wpla_after_listing_update', $listing_id, $data, array('sku' => $sku, 'account_id' => $account_id));
                }
            }
        } else {
            /*
            ...
            $result = $wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->prefix}amazon_listings SET {$update_data} WHERE sku collate utf8_bin = %s AND account_id = %d", $sku, $account_id ) );
            
            // Trigger hook for FBA stock monitoring if fba_quantity was updated
            if ($result !== false && isset($data['fba_quantity'])) {
                // Get affected listing IDs for case-sensitive updates
                $listing_ids = $wpdb->get_col($wpdb->prepare(
                    "SELECT id FROM {$wpdb->prefix}amazon_listings WHERE sku collate utf8_bin = %s AND account_id = %d",
                    $sku, $account_id
                ));
	...
	/**
	 * Get listing IDs that match the given WHERE conditions
	 * Used for triggering hooks after updates
	 */
	static private function getListingIdsByWhere( $where ) {
		global $wpdb;
		$table = $wpdb->prefix . self::TABLENAME;
	...
	function getAllPendingProductsForAccount_TemplateType( $account_id ) {
        ...
        $list_drafts = get_option( 'wpla_allow_listing_drafts', 0 );

        $drafts_filter = '';
        $drafts_join = '';
        if ( !$list_drafts ) {
            $drafts_filter = " AND posts.post_status <> 'draft' ";
            $drafts_join = " LEFT JOIN {$wpdb->posts} posts ON (l.post_id = posts.ID OR l.parent_id = posts.ID) ";
        }

        // There's a possibility that this query hits the MAX_JOIN_SIZE limit in the MySQL server.
...
//		), ARRAY_A);
	...
	function getAllOnlineWithoutASIN( $account_id = false, $limit = false, $format = ARRAY_A ) {
        ...
        /**
         * We do not need to exclude the variable products anymore when trying to fetch the ASINs from Amazon
         */
		$items = $wpdb->get_results("
			SELECT *
			FROM $table
	...
	public static function countProductVariations( $post_id ) {
	    global $wpdb;
        $table = $wpdb->prefix . self::TABLENAME;
	...
	public function markItemAsModified( $post_id, $skip_updating_feeds = false ) {
		...
		foreach ($listings as $item) {
			...
			// Use cached profile to avoid repeated database queries during bulk operations
			$profile = false;
			if ( $item->profile_id ) {
				if ( ! isset( self::$_profile_cache[ $item->profile_id ] ) ) {
					self::$_profile_cache[ $item->profile_id ] = new WPLA_AmazonProfile( $item->profile_id );
				}
				$profile = self::$_profile_cache[ $item->profile_id ];
			}
			if ( $profile ) {
			...
			$listing_data = apply_filters( 'wpla_mark_item_as_modified_data', $listing_data, $listing_id, $post_id );
		...
		if ( $there_were_changes && ! $skip_updating_feeds ) {
			if ( get_option( 'wpla_feed_background_processing', 0 ) ) {
				// Beta: Use ActionScheduler for background processing #73305
				if ( function_exists( 'as_enqueue_async_action' ) ) {
					as_enqueue_async_action( 'wpla_update_feeds', [], 'WPLA' );
				} else {
					as_schedule_single_action( time(), 'wpla_update_feeds', [], 'WPLA' );
				}
			} else {
	...
	/**
	 * Submit a NEW single listing to Amazon using the putListingsItem endpoint
	 * Used by both immediate submission and queue processing
	 *
	 * @param int $listing_id The listing ID to submit
	 * @param array $options Optional settings:
	 *   - allow_status: Array of allowed statuses (default: all submittable)
	 *   - remove_from_queue: Boolean (default: true)
	 *   - retry_on_throttle: Boolean (default: false)
	 * @return object Result object with success, error_code, error_message, should_retry properties
	 */
	public function submitSingleListing( $listing_id, $options = [] ) {
		// Default options
		$options = array_merge([
			'allow_status' => [self::STATUS_PREPARED, self::STATUS_MATCHED],
		...
		// Validate listing exists and has allowed status
		if ( !$listing ) {
			$result->error_code = 'listing_not_found';
			$result->error_message = sprintf( __( 'Listing #%d not found.', 'wp-lister-for-amazon' ), $listing_id );
			return $result;
		}

		if ( !in_array( $listing['status'], $options['allow_status'] ) ) {
			$result->error_code = 'invalid_status';
			$result->error_message = sprintf(
				__( 'Listing #%d cannot be submitted (status: %s). Allowed statuses: %s', 'wp-lister-for-amazon' ),
				$listing_id,
				$listing['status'] ?? 'unknown',
				implode(', ', $options['allow_status'])
			);
			return $result;
		}
		...
		// Validate listing can be submitted
		...
		if ( is_wp_error( $validation_result ) ) {
			...
			$this->updateListing( $listing_id, [
				'status' => self::STATUS_FAILED,
				'history' => serialize( $history )
			]);
			$result->status_updated = true;

			if ( $options['remove_from_queue'] ) {
				self::removeListingFromPublishingQueue( $listing_id );
			}

			$result->error_code = $validation_result->get_error_code();
			$result->error_message = $validation_result->get_error_message();
			return $result;
		}

		// Use profile account_id, fallback to listing account_id if profile has invalid account_id
		$account_id = $profile->account_id;
		if ( empty( $account_id ) || !is_numeric( $account_id ) ) {
			$account_id = $listing['account_id'] ?? null;
		}

		// Validate we have a valid account_id
		if ( empty( $account_id ) || !is_numeric( $account_id ) ) {
			$history = [
				'errors' => [
					[
						'error-code'    => 'invalid_account_id',
						'error-message' => 'No valid account_id found for this listing',
						'error-type'    => 'Error'
					]
				],
				'warnings' => []
			...
			$result->error_code = 'invalid_account_id';
			$result->error_message = __( 'No valid account_id found for this listing.', 'wp-lister-for-amazon' );
			return $result;
		}
		...
		try {
			...
			$api_result = $api->putListingsItem( $listing, $profile );

			if ( !WPLA_Amazon_SP_API::isError( $api_result ) ) {
				if ( $api_result->getStatus() == 'ACCEPTED' ) {
					// SUCCESS: update listing status to submitted
					$this->updateListing( $listing_id, [
						'status' => self::STATUS_SUBMITTED
					]);
					$result->status_updated = true;

					// Remove from publish queue if requested
					if ( $options['remove_from_queue'] ) {
						self::removeListingFromPublishingQueue( $listing_id );
					}

					WPLA()->logger->info( "Listing {$listing_id} submitted successfully" );
					$result->success = true;
					return $result;
				} else {
					// Handle issues from Amazon response
					$history = [
						'errors' => [],
						'warnings' => []
					];
					foreach ( $api_result->getIssues() as $issue ) {
						$error = [
					...
					$status = empty( $history['errors'] ) ? self::STATUS_SUBMITTED : self::STATUS_FAILED;
					$this->updateListing( $listing_id, [
						'status' => $status,
						'history' => serialize( $history )
					]);
					...
					if ( !empty( $history['errors'] ) ) {
						$error_messages = array_column( $history['errors'], 'error-message' );
						$result->error_code = 'submission_failed';
						$result->error_message = implode( ', ', $error_messages );
					} else {
						$result->success = true;
					}
				...
				// API error - check for throttling
				$status_code = $api_result->StatusCode ?? null;
				if ( $options['retry_on_throttle'] && in_array( $status_code, [429, 503], true ) ) {
					$result->should_retry = true;
					$result->error_code = 'throttled';
					$result->error_message = "Amazon API throttled (HTTP {$status_code})";
					return $result;
				}

				// Non-retryable API error
				$error_message = $api_result->ErrorMessage ?? $api_result->Error['Message'] ?? 'Unknown API error';
				$error_code = $api_result->ErrorCode ?? $status_code ?? 'api_error';
			...
			// Exception occurred - check for throttling
			if ( $options['retry_on_throttle'] && intval( $e->getCode() ) === 429 ) {
				$result->should_retry = true;
				$result->error_code = 'throttled';
				$result->error_message = "HTTP 429 exception: " . $e->getMessage();
				return $result;
			}
	...
	/**
	 * Submit a prepared listing immediately, bypassing the queue
	 *
	 * @param int $listing_id The listing ID to submit
	 * @return true|WP_Error Returns true on success, WP_Error on failure
	 */
	public function submitListingImmediately( $listing_id ) {
		$result = $this->submitSingleListing( $listing_id, [
			'allow_status' => [self::STATUS_PREPARED],
			'remove_from_queue' => true,
			'retry_on_throttle' => false
		]);

		// Convert result to WP_Error format for backwards compatibility
		if ( $result->success ) {
			return true;
		} else {
			return new WP_Error( $result->error_code, $result->error_message );
		}
	}

	public function setLockedStatus( $id, $locked = true ) {
	    $locked_value = ($locked) ? 1 : 0;
	    $this->updateWhere( ['id' => $id], ['locked' => $locked_value] );
    }
	...
	public function enqueuePreparedListings() {
		...
		foreach ( $prepared as $item ) {
			...
			// Add to queue without scheduling ActionScheduler (we'll do that once after the loop)
			if ( $this->addToPublishQueue( $item->id ) ) {
				$queue_modified = true;
			}
		}

		// Schedule ActionScheduler ONCE after processing all items, only if queue was modified
		if ( $queue_modified && ! as_next_scheduled_action( self::PUBLISH_QUEUE_CRON, [], 'WPLA' ) ) {
			as_schedule_single_action( time() + 1, self::PUBLISH_QUEUE_CRON, [], 'WPLA' );
		}
	}

	/**
	 * Add a listing to the publish queue without scheduling ActionScheduler.
	 *
	 * This is a helper method for bulk operations where you want to add multiple
	 * listings to the queue and schedule ActionScheduler only once after all additions.
	 *
	 * @param int $listing_id The unique identifier of the listing to add to the queue.
	 * @return bool True if the listing was added to the queue, false if it was already present.
	 */
	private function addToPublishQueue( $listing_id ) {
	...
	public function enqueueForPublish( $listing_id ) {
		// Add to queue
		$was_added = $this->addToPublishQueue( $listing_id );

		// Schedule ActionScheduler only if the listing was actually added and no action is scheduled
		if ( $was_added && ! as_next_scheduled_action( self::PUBLISH_QUEUE_CRON, [], 'WPLA' ) ) {
			as_schedule_single_action( time() + 1, self::PUBLISH_QUEUE_CRON, [], 'WPLA' );
		}
	}
	...
	public function prepareProductForListing( $post_id, $profile_id, $update_pending_feeds = true ) {
		...
		// Allow third-party code to remove certain products from being submitted to Amazon
		if ( apply_filters( 'wpla_should_skip_product_from_listing', false, $post_id, $profile_id ) ) {
			$this->warnings[] = sprintf( __('"%s" has been skipped because `wpla_should_skip_product_from_listing` returned TRUE.','wp-lister-for-amazon'), get_the_title($post_id), $profile->account_id );
			return false;
		}
		...
		if ( $product->is_type( array( 'variable', 'variable-product-part' ) ) ) {
			$last_variation_data = $this->prepareVariations( $post_id, $profile, $post, $data );
			$data['vtheme']      = $last_variation_data['vtheme'];
		}

		if (
			apply_filters( 'wpla_skip_flat_variation_parent_listing', false ) &&
			@$profile->data['variations_mode'] == 'flat' ||
			apply_filters( 'wpla_should_skip_parent_from_listing', false, $post_id, $profile_id )
		) {
		...
		// Note: Do NOT call repricing helpers here for newly prepared listings.
		// Repricing is designed for active (online) listings with competitive pricing data.
		// Calling it on new listings would incorrectly reset price to max_price when
		// min/max are set but no lowest_price exists yet. #73318
        // $items = array( $this->getItem( $listing_id, OBJECT ) );
        // WPLA_RepricingHelper::adjustLowestPriceForProducts(  $items );
        // WPLA_RepricingHelper::resetProductsToMaxPrice( $items );
	...
	public function prepareVariations( $post_id, $profile, $post, $data ) {
		...
		foreach ( $variations as $var ) {
            ...
            $data['min_price']     = get_post_meta( $variation_id, '_amazon_minimum_price', true );
            $data['max_price']     = get_post_meta( $variation_id, '_amazon_maximum_price', true );
			...
			// Note: Do NOT call repricing helpers here - see prepareProductForListing() comment
			// $items = array( $this->getItem( $listing_id, OBJECT ) );
            // WPLA_RepricingHelper::adjustLowestPriceForProducts(  $items );
            // WPLA_RepricingHelper::resetProductsToMaxPrice( $items );
        }
    ...
    private function processVariationsBatch( $variation_ids, $parent_listing, $account ) {
        ...
        foreach ( $variation_ids as $variation_id ) {
            ...
            // check if this variation already has a listing for this marketplace - skip if it does
            $existing_listings = $lm->getAllItemsByPostID( $variation_id );
            if ( ! empty( $existing_listings ) ) {
                foreach ( $existing_listings as $existing_listing ) {
                    // Get the marketplace_id for the existing listing's account
                    $existing_account = WPLA_AmazonAccount::getAccount( $existing_listing->account_id );
                    if ( $existing_account && $existing_account->marketplace_id == $account->marketplace_id ) {
                        WPLA()->logger->info( 'Variation #' . $variation_id . ' already has a listing for marketplace ' . $account->marketplace_id . '. Skipping' );
                        continue 2; // Skip to next variation
                    }
                }
            }
    ...
    /**
     * For profiles with flat variations, make sure to set the parent listing status to online
     *
     * @param $listing
     */
    public function maybeUpdateParentStatus( $listing ) {
	...
	public function removeASINFromProducts( $listing_id ) {
	    WPLA()->logger->info( 'removeASINFromProducts( '. $listing_id .')' );
	    $listing = $this->getItem( $listing_id );
        ...
        if ( $listing ) {
            if ( $listing['parent_id'] ) {
                ...
                WPLA()->logger->info( 'deleted _wpla_asin for the parent_id '. $listing['parent_id'] );
                ...
                if ( $children ) {
                    foreach ( $children as $listing ) {
                        ...
                        WPLA()->logger->info( 'deleted _wpla_asin for the post_id '. $listing->post_id );
                    }
                }
            } else {
                delete_post_meta( $listing['post_id'], '_wpla_asin' );
