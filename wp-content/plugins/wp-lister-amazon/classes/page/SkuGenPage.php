...
class WPLA_SkuGenPage extends WPLA_Page {
	...
	public function generateNewSKUs( $product_ids ) {
		...
		$duplicate_mode = get_option( 'wpla_skugen_duplicate_mode', 'skip' );

		// arrays to collect details for summary
		$skipped_products = array();
		$resolved_products = array();

		foreach ($product_ids as $post_id) {
			$base_sku = WPLA_SkuGenerator::generateNewSKU( $post_id );

			if ( in_array( $base_sku, $this->existing_skus ) ) {

				if ( $duplicate_mode == 'append_number' ) {
					// Resolve duplicate by appending number
					$final_sku = WPLA_SkuGenerator::resolveDuplicateSKU( $base_sku, $this->existing_skus );
					update_post_meta( $post_id, '_sku', $final_sku );
					$this->existing_skus[] = $final_sku;
					$updated_count++;
					$resolved_count++;
					$resolved_products[] = array( 'id' => $post_id, 'original' => $base_sku, 'final' => $final_sku );
				} else {
					// Skip with warning (original behavior)
					$skipped_count++;
					$skipped_products[] = array( 'id' => $post_id, 'sku' => $base_sku );
				}

			} else {
				// SKU does not exist - update SKU and add to list
				update_post_meta( $post_id, '_sku', $base_sku );
				$this->existing_skus[] = $base_sku;
				$updated_count++;
			}
		}

		// Display summary message
		$message = "SKUs for $updated_count products have been updated.";

		if ( $resolved_count > 0 ) {
			$message .= " $resolved_count products had duplicate SKUs that were resolved by appending numbers.";
		}

		if ( $skipped_count > 0 ) {
			$message .= " $skipped_count products were skipped due to duplicate SKUs.";
		}
	...
	public function saveSkuGenOptions() {
		...
		update_option( 'wpla_skugen_mode_simple', 	 wpla_clean($_REQUEST['wpla_skugen_mode_simple']) );
		update_option( 'wpla_skugen_mode_variation', wpla_clean($_REQUEST['wpla_skugen_mode_variation']) );
		update_option( 'wpla_skugen_mode_case', 	 wpla_clean($_REQUEST['wpla_skugen_mode_case']) );
		update_option( 'wpla_skugen_duplicate_mode', wpla_clean($_REQUEST['wpla_skugen_duplicate_mode']) );
	...
	public function displaySkuGenPage() {
		...
		if ( $this->requestAction() == 'wpla_bulk_generate_skus' ) {
			...
			$this->generateNewSKUs( wpla_clean($_REQUEST['product']) );
		}
		...
		$aData = array(
			...
			'skugen_duplicate_mode'		=> get_option( 'wpla_skugen_duplicate_mode', 'skip' ),
