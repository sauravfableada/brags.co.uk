		...
		if ( $action == 'wpla_convert_profiles' ) {
			...
			foreach ( $tpl_actions as $tpl_id => $convert ) {
				...
				try {
					...
					WPLA()->logger->error( 'Product Type save failed during profile conversion: ' . $e->getMessage() );
					wpla_show_message(
						sprintf(
							__( 'Failed to save Product Type "%s". The product types table may need to be repaired. Please visit Tools > Developer Tools > Reinstall Product Types Table. Error: %s', 'wp-lister-for-amazon' ),
							$product_type,
							$e->getMessage()
						),
						'error',
