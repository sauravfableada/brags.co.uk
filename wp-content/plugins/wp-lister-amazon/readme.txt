...
= 2.9.2.1 - 2026-04-20 =
Fix: Stock being reduced multiple times per order due to DB migration not running before cron

= 2.9.2 - 2026-04-16 =
New: Multi-Inventory integration hooks for warehouse-specific stock sync
New: Add metabox filter fallbacks to Order Fulfillment feed builder
Fix: Product matcher not returning results when searching by SKU or EAN
Fix: Prevent custom size map from being bypassed in JSON feed builder
Fix: Persist correct enum dropdown selection across profile saves
Fix: Process shortcodes in array-type profile field values
Fix: Prevent double-wrapping of standardized_values in JSON feed builder
Fix: Item type keyword dropdown missing options due to duplicate enum values
Fix: Sale price clearing PATCH fails for variable products and matched listings
Fix: Prevent empty image URLs from being sent to Amazon feeds
Fix: Only deduct repricing shipping fee from products with repricing enabled
Fix: Reload fulfillment feed data at submit time to catch concurrent webhook items
Fix: Pre-populate shipping defaults in order meta box
Fix: Use correct identifier type when searching catalog by SKU
Dev: Include the current WPLA version in the order data
Dev: Log product matcher API calls to the wpla-log table

= 2.9.1 - 2025-03-12 =
New: Fallback buyer email pattern setting for Amazon orders
