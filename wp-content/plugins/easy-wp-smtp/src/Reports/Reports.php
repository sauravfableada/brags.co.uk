...
class Reports {
	 ...
	 * @const string
	 */
	const SENT_EMAILS_COUNTER_OPTION_KEY = 'easy_wp_smtp_lite_sent_email_counter';
	 ...
	 * @const string
	 */
	const WEEKLY_SENT_EMAILS_COUNTER_OPTION_KEY = 'easy_wp_smtp_lite_weekly_sent_email_counter';
	...
	private function public_hooks() {
		...
		add_action( 'easy_wp_smtp_mailcatcher_smtp_send_after', [ $this, 'update_sent_emails_stats' ] );

		// Update sent email counter when all other mailers are used.
		add_action( 'easy_wp_smtp_mailcatcher_send_after', [ $this, 'update_sent_emails_stats' ] );
	}
	...
	private function admin_hooks() {
		...
		add_action(
			'load-' . easy_wp_smtp()->get_admin()->get_admin_page_hook(),
			[ $this, 'summary_report_email_preview' ]
		);
	 ...
	 *
	 * @since 2.1.0
	 * @since 2.15.0 Added 'two_weeks_ago' identifier and year-boundary handling.
	 *
	...
	public function summary_report_email_preview() {
		...
		if ( ! current_user_can( easy_wp_smtp()->get_admin()->get_logs_access_capability() ) ) {
			return;
		}
