...
class WP {
	...
	private static function get_debug_event_detail_html() {
		...
		if ( ! empty( $detail ) ) {
			$cache[ $event_id ] = '<code class="easy-wp-smtp-notice__debug-detail">' . esc_html( $detail ) . '</code>';
		}
		...
		foreach ( (array) self::$admin_notices as $notice ) :
			...
			<div class="notice easy-wp-smtp-notice <?php echo esc_attr( $notice['class'] ); ?> notice <?php echo esc_attr( $dismissible ); ?>" <?php echo ! empty( $notice['key'] ) ? 'data-notice="' . esc_attr( $notice['key'] ) . '"' : ''; ?>>
				...
				<?php if ( ! empty( $notice['error_code'] ) ) : ?>
					<div class="easy-wp-smtp-notice__error-code">
						<code><?php echo esc_html( $notice['error_code'] ); ?></code>
						<button type="button" class="easy-wp-smtp-notice__copy-btn" title="<?php esc_attr_e( 'Copy error code', 'easy-wp-smtp' ); ?>">
							<svg class="easy-wp-smtp-notice__icon-copy" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path fill="currentColor" d="M433.941 65.941l-51.882-51.882A48 48 0 0 0 348.118 0H176c-26.51 0-48 21.49-48 48v48H48c-26.51 0-48 21.49-48 48v320c0 26.51 21.49 48 48 48h224c26.51 0 48-21.49 48-48v-48h80c26.51 0 48-21.49 48-48V99.882a48 48 0 0 0-14.059-33.941zM266 464H54a6 6 0 0 1-6-6V150a6 6 0 0 1 6-6h74v224c0 26.51 21.49 48 48 48h96v42a6 6 0 0 1-6 6zm128-96H182a6 6 0 0 1-6-6V54a6 6 0 0 1 6-6h106v88c0 13.255 10.745 24 24 24h88v202a6 6 0 0 1-6 6zm6-256h-64V48h9.632c1.591 0 3.117.632 4.243 1.757l48.368 48.368a6 6 0 0 1 1.757 4.243V112z"/></svg>
							<svg class="easy-wp-smtp-notice__icon-check" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" style="display:none;"><path fill="#0f8a56" d="M256 512c141.4 0 256-114.6 256-256S397.4 0 256 0S0 114.6 0 256S114.6 512 256 512zM369 209L241 337c-9.4 9.4-24.6 9.4-33.9 0l-64-64c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.4 33.9 0l47 47L335 175c9.4-9.4 24.6-9.4 33.9 0s9.4 24.6 0 33.9z"/></svg>
						</button>
					</div>
				<?php endif; ?>
			</div>
	...
	public static function datetime_format() {
		...
		return sprintf( /* translators: %1$s - date, \a\t - specially escaped "at", %2$s - time. */
			esc_html__( '%1$s \a\t %2$s', 'easy-wp-smtp' ),
	...
	public static function admin_url( $path = '', $scheme = 'admin' ) {
		...
		return apply_filters( 'easy_wp_smtp_admin_url', \admin_url( $path, $scheme ), $path, $scheme );
	}
	...
	public static function is_doing_self_ajax() {
		...
		return self::is_doing_ajax() && $action && substr( $action, 0, 12 ) === 'easy_wp_smtp';
	}
	...
	public static function get_initiator( $file_path ) { // phpcs:ignore Generic.Metrics.CyclomaticComplexity.TooHigh
		...
		$cache_key = 'easy_wp_smtp_initiators_data';
	...
	private static function get_initiator_wp_core( $file_path ) {
		...
		if (
			...
			return [
				'name' => esc_html__( 'WP Core', 'easy-wp-smtp' ),
				'slug' => 'wp-core',
				'type' => 'wp-core',
			];
		}
	...
	public static function get_site_url() {
		 ...
		 * @return bool
		 */
		if ( apply_filters( 'easy_wp_smtp_wp_get_site_url_unfiltered', false ) ) { // phpcs:ignore WPForms.PHP.ValidateHooks.InvalidHookName
