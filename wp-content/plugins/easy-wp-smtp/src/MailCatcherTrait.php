...
trait MailCatcherTrait {
	...
	public function send() { // phpcs:ignore Generic.Metrics.CyclomaticComplexity.TooHigh
		...
		$this->XMailer = 'EasyWPSMTP/Mailer/' . $mailer_slug . ' ' . EasyWPSMTP_PLUGIN_VERSION;
		...
		try {
			...
			if ( DebugEvents::is_debug_enabled() || $this->is_test_email ) {
				...
				if ( $this->is_test_email ) {
					 ...
					 */
					$this->SMTPDebug = apply_filters( 'easy_wp_smtp_admin_test_email_smtp_debug', 3 );
				} else {
					$this->SMTPDebug = 3;
				}
			}
			 ...
			 *
			 * @since 1.5.0
			 * @since 2.15.0 Fires exactly once per send and only on success.
			 *
			 ...
			 */
			do_action( 'easy_wp_smtp_mailcatcher_smtp_send_after', true, $this->to, $this->cc, $this->bcc, $this->Subject, $this->MIMEBody, $this->From );
			...
			$error_message = 'Mailer: ' . esc_html( easy_wp_smtp()->get_providers()->get_options( $mailer_slug )->get_title() ) . "\r\n" . $this->ErrorInfo;
			...
			if ( DebugEvents::is_debug_enabled() && ! empty( $debug_output ) ) {
				$debug_event_message .= "\r\n" . esc_html__( 'SMTP Debug:', 'easy-wp-smtp' ) . "\r\n";
			 ...
			 */
			do_action( 'easy_wp_smtp_mailcatcher_send_failed', $this->ErrorInfo, $this, $mailer_slug, $error_code, 0, $connection, $error_key );
	...
	private function api_send( $connection ) { // phpcs:ignore Generic.Metrics.CyclomaticComplexity.TooHigh
		...
		try {
			...
			$mailer = easy_wp_smtp()->get_providers()->get_mailer( $mailer_slug, $this, $connection );
			...
			$message = 'Mailer: ' . esc_html( easy_wp_smtp()->get_providers()->get_options( $mailer_slug )->get_title() ) . "\r\n";
			 ...
			 * @param string               $error_key     Composite error key.
			 */
			do_action( 'easy_wp_smtp_mailcatcher_send_failed', $e->getMessage(), $this, $mailer_slug, $error_code, $response_code, $connection, $error_key );
	 ...
	 * @return string
	 */
	private function build_error_key( $prefix, $error_code, $error_message ) { // phpcs:ignore Generic.Metrics.CyclomaticComplexity.TooHigh, Generic.Metrics.CyclomaticComplexity.MaxExceeded
	...
	private function record_send_failure( $connection, $mailer_slug, $error_code, $error_message, $debug_event_id, $smtp_debug = '', $error_key = '' ) {
		...
		$initiator          = easy_wp_smtp()->get_wp_mail_initiator();
		$connection_options = $connection->get_options();
