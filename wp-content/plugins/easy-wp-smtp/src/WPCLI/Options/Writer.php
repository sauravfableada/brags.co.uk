...
class Writer {
	...
	public function validate( array $resolved ) { // phpcs:ignore Generic.Metrics.CyclomaticComplexity.MaxExceeded, Generic.Metrics.NestingLevel.MaxExceeded
		...
		foreach ( $this->registry->get_args() as $arg ) {
			...
			if ( ! empty( $arg['required'] ) && ! $present ) {
				$errors[] = sprintf(
					...
					__( 'Missing required flag: --%s', 'easy-wp-smtp' ),
					$flag
				);
			...
			if ( ! empty( $arg['required_if'] ) ) {
				...
				if ( $rule_match['all_match'] && ! $present ) {
					$errors[] = sprintf(
						...
						__( 'Missing required flag: --%1$s (required when %2$s)', 'easy-wp-smtp' ),
						$flag,
				...
				if ( $rule_match['any_match'] && ! empty( $rule_match['missing'] ) ) {
					foreach ( $rule_match['missing'] as $missing_gate ) {
						...
						$errors[] = sprintf(
							...
							__( 'Missing required flag: --%s (needed to fully specify the configuration when other gating flags are set)', 'easy-wp-smtp' ),
							$missing_gate
						);
					}
				}
			...
			if ( ( $arg['type'] ?? null ) === 'enum' && ! in_array( $resolved[ $flag ], $arg['enum'], true ) ) {
				$errors[] = sprintf(
					...
					__( 'Invalid value for --%1$s: %2$s (allowed: %3$s)', 'easy-wp-smtp' ),
					$flag,
					$resolved[ $flag ],
					implode( ', ', $arg['enum'] )
				);
			...
			if ( ( $arg['type'] ?? null ) === 'email' && ! is_email( $resolved[ $flag ] ) ) {
				$errors[] = sprintf(
					...
					__( 'Invalid email for --%1$s: %2$s', 'easy-wp-smtp' ),
					$flag,
					$resolved[ $flag ]
				);
			}
			...
			if ( ( $arg['type'] ?? null ) === 'int' && ! is_numeric( $resolved[ $flag ] ) ) {
				$errors[] = sprintf(
					...
					__( 'Invalid integer for --%1$s: %2$s', 'easy-wp-smtp' ),
					$flag,
					$resolved[ $flag ]
				);
			}
		...
		if ( ! empty( $errors ) ) {
			WP_CLI::error( __( 'Configuration errors:', 'easy-wp-smtp' ) . "\n  - " . implode( "\n  - ", $errors ) );
		}
	}
	...
	public function write( array $resolved ) { // phpcs:ignore Generic.Metrics.CyclomaticComplexity.TooHigh
		...
		foreach ( $resolved as $flag => $value ) {
			...
			// Shadow check is limited to the top-level group/key pair, matching ESMTP's shallow EASY_WP_SMTP_<GROUP>_<KEY> constants.
		...
		foreach ( $result['shadowed'] as $flag ) {
			WP_CLI::warning(
				sprintf(
					...
					__( 'A wp-config constant is defined for %s. The value was not stored. Remove the constant first, then re-run this command.', 'easy-wp-smtp' ),
					$flag
				)
			);
		}
	...
	private function resolve_one_arg( array $arg, array $assoc_args ) {
			...
			if ( ! is_readable( $path ) ) {
				WP_CLI::error(
					sprintf(
						...
						__( 'Cannot read --%1$s file: %2$s', 'easy-wp-smtp' ),
						$file_flag,
						$path
					)
				);
		...
		$env_var = $arg['env_var'] ?? ( 'EASY_WP_SMTP_' . strtoupper( str_replace( '.', '_', $flag ) ) );
		$env     = getenv( $env_var );
