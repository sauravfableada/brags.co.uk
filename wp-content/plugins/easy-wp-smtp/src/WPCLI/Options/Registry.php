 ...
 * `easy_wp_smtp_wpcli_options_registry_get_args` filter.
 *
 * @since 2.15.0
 */
class Registry {
	 ...
	 *   - env_var      (string|null) Explicit override; else derived as EASY_WP_SMTP_<FLAG_UPPER>.
	...
	public function get_args() {
		 ...
		 */
		return apply_filters( 'easy_wp_smtp_wpcli_options_registry_get_args', $args ); // phpcs:ignore WPForms.PHP.ValidateHooks.InvalidHookName
	}
	...
	private function mail_args() {
		...
		return [
			[
				...
				'type'        => 'email',
				'required'    => true,
				'description' => __( 'From email address.', 'easy-wp-smtp' ),
			],
			[
				'flag'        => 'mail.from_name',
				'type'        => 'string',
				'required'    => true,
				'description' => __( 'From name.', 'easy-wp-smtp' ),
			],
			[
				'flag'             => 'mail.mailer',
				'type'             => 'enum',
				...
				'description'      => sprintf(
					...
					__( 'Mailer to use. One of: %s.', 'easy-wp-smtp' ),
			...
			[
				...
				'type'        => 'bool',
				'description' => __( 'Set Return-Path to match From email.', 'easy-wp-smtp' ),
			],
			[
				'flag'        => 'mail.from_email_force',
				'type'        => 'bool',
				'description' => __( 'Force From email on every outgoing message.', 'easy-wp-smtp' ),
			],
			[
				'flag'        => 'mail.from_name_force',
				'type'        => 'bool',
				'description' => __( 'Force From name on every outgoing message.', 'easy-wp-smtp' ),
			],
		];
	}
	...
	private function general_args() {
		...
		return [
			[
				...
				'type'        => 'bool',
				'description' => __( 'Disable all outgoing email.', 'easy-wp-smtp' ),
			],
			[
				'flag'         => 'general.hide_am_notifications',
				'storage_path' => 'general.am_notifications_hidden',
				'type'         => 'bool',
				'description'  => __( 'Hide Awesome Motive product notifications.', 'easy-wp-smtp' ),
			],
		];
	}
	...
	private function smtp_args() {
		...
		return [
			[
				...
				'type'        => 'string',
				'required_if' => $req,
				'description' => __( 'SMTP server hostname.', 'easy-wp-smtp' ),
			],
			[
				'flag'        => 'smtp.port',
				'type'        => 'int',
				'required_if' => $req,
				'description' => __( 'SMTP server port (e.g. 25, 465, 587).', 'easy-wp-smtp' ),
			],
			[
				'flag'        => 'smtp.encryption',
				'type'        => 'enum',
				'enum'        => [ 'none', 'ssl', 'tls' ],
				'description' => __( 'Encryption: none, ssl, or tls.', 'easy-wp-smtp' ),
			],
			[
				'flag'        => 'smtp.autotls',
				'type'        => 'bool',
				'description' => __( 'Auto TLS when supported by server.', 'easy-wp-smtp' ),
			],
			[
				'flag'        => 'smtp.auth',
				'type'        => 'bool',
				'description' => __( 'Whether the SMTP server requires authentication.', 'easy-wp-smtp' ),
			],
			[
				'flag'        => 'smtp.user',
				'type'        => 'string',
				'required_if' => $req_auth,
				'description' => __( 'SMTP username.', 'easy-wp-smtp' ),
			],
			[
				'flag'        => 'smtp.pass',
				'type'        => 'string',
				'required_if' => $req_auth,
				'sensitive'   => true,
				'description' => __( 'SMTP password.', 'easy-wp-smtp' ),
			],
		];
	}
	...
	private function sendgrid_args() {
		...
		return [
			[
				...
				'required_if' => $req,
				'sensitive'   => true,
				'description' => __( 'SendGrid API key.', 'easy-wp-smtp' ),
			],
			[
				'flag'        => 'sendgrid.domain',
				'type'        => 'string',
				'description' => __( 'Optional sending domain.', 'easy-wp-smtp' ),
			],
		];
	}
	...
	private function mailgun_args() {
		...
		return [
			[
				...
				'required_if' => $req,
				'sensitive'   => true,
				'description' => __( 'Mailgun API key.', 'easy-wp-smtp' ),
			],
			[
				'flag'        => 'mailgun.domain',
				'type'        => 'string',
				'required_if' => $req,
				'description' => __( 'Mailgun sending domain.', 'easy-wp-smtp' ),
			],
			[
				'flag'        => 'mailgun.region',
				'type'        => 'enum',
				'enum'        => [ 'US', 'EU' ],
				'description' => __( 'Mailgun region (US or EU).', 'easy-wp-smtp' ),
			],
		];
	}
	...
	private function postmark_args() {
		...
		return [
			[
				...
				'required_if'  => $req,
				'sensitive'    => true,
				'description'  => __( 'Postmark server API token.', 'easy-wp-smtp' ),
			],
			[
				'flag'        => 'postmark.message_stream',
				'type'        => 'string',
				'description' => __( 'Optional Postmark message stream.', 'easy-wp-smtp' ),
			],
		];
	}
	...
	private function sendlayer_args() {
		...
		return [
			[
				...
				'required_if' => $req,
				'sensitive'   => true,
				'description' => __( 'SendLayer API key.', 'easy-wp-smtp' ),
			],
		];
	}
	...
	private function resend_args() {
		...
		return [
			[
				...
				'required_if' => $req,
				'sensitive'   => true,
				'description' => __( 'Resend API key.', 'easy-wp-smtp' ),
			],
		];
	}
	...
	private function smtpcom_args() {
		...
		return [
			[
				...
				'required_if' => $req,
				'sensitive'   => true,
				'description' => __( 'SMTP.com API key.', 'easy-wp-smtp' ),
			],
			[
				'flag'        => 'smtpcom.channel',
				'type'        => 'string',
				'required_if' => $req,
				'description' => __( 'SMTP.com sender channel name.', 'easy-wp-smtp' ),
			],
		];
	}
	...
	private function smtp2go_args() {
		...
		return [
			[
				...
				'required_if' => $req,
				'sensitive'   => true,
				'description' => __( 'SMTP2GO API key.', 'easy-wp-smtp' ),
			],
		];
	}
	...
	private function sparkpost_args() {
		...
		return [
			[
				...
				'required_if' => $req,
				'sensitive'   => true,
				'description' => __( 'SparkPost API key.', 'easy-wp-smtp' ),
			],
			[
				'flag'        => 'sparkpost.region',
				'type'        => 'enum',
				'enum'        => [ 'US', 'EU' ],
				'description' => __( 'SparkPost region (US or EU).', 'easy-wp-smtp' ),
			],
		];
	}
	...
	private function mailjet_args() {
		...
		return [
			[
				...
				'required_if' => $req,
				'sensitive'   => true,
				'description' => __( 'Mailjet API key.', 'easy-wp-smtp' ),
			],
			[
				...
				'required_if' => $req,
				'sensitive'   => true,
				'description' => __( 'Mailjet secret key.', 'easy-wp-smtp' ),
			],
		];
	}
	...
	private function mailersend_args() {
		...
		return [
			[
				...
				'required_if' => $req,
				'sensitive'   => true,
				'description' => __( 'MailerSend API key.', 'easy-wp-smtp' ),
			],
			[
				'flag'         => 'mailersend.pro_plan',
				'storage_path' => 'mailersend.has_pro_plan',
				'type'         => 'bool',
				'description'  => __( 'Set if your MailerSend account is on a paid plan.', 'easy-wp-smtp' ),
			],
		];
	}
	...
	private function sendinblue_args() {
		...
		return [
			[
				...
				'required_if'  => $req,
				'sensitive'    => true,
				'description'  => __( 'Brevo (formerly Sendinblue) API key.', 'easy-wp-smtp' ),
			],
			[
				'flag'         => 'brevo.domain',
				'storage_path' => 'sendinblue.domain',
				'type'         => 'string',
				'description'  => __( 'Optional Brevo sending domain.', 'easy-wp-smtp' ),
			],
		];
	}
	...
	private function elasticemail_args() {
		...
		return [
			[
				...
				'required_if' => $req,
				'sensitive'   => true,
				'description' => __( 'Elastic Email API key.', 'easy-wp-smtp' ),
			],
		];
	}
	...
	private function amazonses_args() {
		...
		return [
			[
				...
				'required_if'  => $req,
				'sensitive'    => true,
				'description'  => __( 'AWS access key ID.', 'easy-wp-smtp' ),
			],
			[
				...
				'required_if'  => $req,
				'sensitive'    => true,
				'description'  => __( 'AWS secret access key.', 'easy-wp-smtp' ),
			],
			[
				'flag'        => 'amazonses.region',
				'type'        => 'string',
				'required_if' => $req,
				'description' => __( 'AWS region (e.g. us-east-1).', 'easy-wp-smtp' ),
			],
		];
	}
	...
	private function mandrill_args() {
		...
		return [
			[
				...
				'required_if' => $req,
				'sensitive'   => true,
				'description' => __( 'Mandrill API key.', 'easy-wp-smtp' ),
			],
		];
	}
}
