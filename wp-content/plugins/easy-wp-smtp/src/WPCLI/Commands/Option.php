...
/**
 * Read and write individual Easy WP SMTP settings.
 *
 * @since 2.15.0
 */
class Option {
	...
	public static function shortdesc() {
		...
		return __( 'Read and write individual Easy WP SMTP settings.', 'easy-wp-smtp' );
	}
	...
	public static function help( Registry $registry ) {
		...
		$action_desc         = __( 'One of: get, set, list.', 'easy-wp-smtp' );
		$flag_desc           = __( 'Dotted flag (e.g. smtp.host). Required for get and set.', 'easy-wp-smtp' );
		$value_desc          = __( 'Value (required for set; can also be supplied via --value-file or env var).', 'easy-wp-smtp' );
		$value_file_desc     = __( 'Read a sensitive value from a file.', 'easy-wp-smtp' );
		$show_sensitive_desc = __( 'Show sensitive values instead of masking them.', 'easy-wp-smtp' );
		$format_desc         = __( 'Output format for `list`. table | json | yaml. Default: table.', 'easy-wp-smtp' );
...
## EXAMPLES
    ...
    wp easy-wp-smtp option set smtp.host mail.example.com
    wp easy-wp-smtp option set sendgrid.api_key --value-file=/run/secret/sg
    wp easy-wp-smtp option list --format=json
...
HELP;
	...
	/**
	 * Execute the `wp easy-wp-smtp option` command.
	 *
	 * @since 2.15.0
	 *
	...
	public function __invoke( $args, $assoc_args ) {
		...
		switch ( $action ) {
			...
			default:
				WP_CLI::error( __( 'Action must be one of: get, set, list.', 'easy-wp-smtp' ) );
		}
	}
	...
	private function cmd_get( array $args, array $assoc_args ) { // phpcs:ignore Generic.Metrics.CyclomaticComplexity.TooHigh
		...
		if ( $flag === null ) {
			WP_CLI::error( __( 'Usage: wp easy-wp-smtp option get <group>.<key>', 'easy-wp-smtp' ) );
		}
		...
		if ( $arg === null ) {
			WP_CLI::error(
				sprintf(
					...
					__( 'Unknown flag: %s', 'easy-wp-smtp' ),
					$flag
				)
			);
		}
		...
		if ( $value === $sentinel ) {
			WP_CLI::log( __( '(not set)', 'easy-wp-smtp' ) );
	...
	private function cmd_set( array $args, array $assoc_args ) { // phpcs:ignore Generic.Metrics.CyclomaticComplexity.TooHigh
		...
		if ( $flag === null ) {
			WP_CLI::error( __( 'Usage: wp easy-wp-smtp option set <group>.<key> <value>', 'easy-wp-smtp' ) );
		}
		...
		if ( $value === null ) {
			WP_CLI::error(
				sprintf(
					/* translators: %1$s is the dotted CLI flag (e.g. smtp.host). %2$s is the environment variable name the operator can use (e.g. EASY_WP_SMTP_SMTP_HOST). Flag and env var name are not translated. */
					__( 'No value provided for --%1$s (positional, --value-file, or env var %2$s).', 'easy-wp-smtp' ),
					$flag,
					$arg['env_var'] ?? ( 'EASY_WP_SMTP_' . strtoupper( str_replace( '.', '_', $flag ) ) )
				)
			);
		}
		...
		if ( empty( $result['written'] ) ) {
			WP_CLI::error(
				sprintf(
					...
					__( 'Could not store %s — value is shadowed by a wp-config constant.', 'easy-wp-smtp' ),
					$flag
				)
			);
		}
