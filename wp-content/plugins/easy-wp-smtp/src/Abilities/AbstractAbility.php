...
abstract class AbstractAbility implements AbilityInterface {
	...
	protected function forbidden() {
		...
		return new WP_Error(
			'easy_wp_smtp_forbidden',
			esc_html__( 'You do not have permission to access this data.', 'easy-wp-smtp' ),
			[ 'status' => 403 ]
		);
	}
	...
	protected function limit_schema() {
		...
		return [
			'description' => esc_html__( 'Maximum number of records to return.', 'easy-wp-smtp' ),
			'type'        => 'integer',
			'minimum'     => 1,
			'maximum'     => 100,
			'default'     => 20,
	...
	protected function offset_schema() {
		...
		return [
			'description' => esc_html__( 'Number of records to skip.', 'easy-wp-smtp' ),
			'type'        => 'integer',
			'minimum'     => 0,
			'default'     => 0,
		];
