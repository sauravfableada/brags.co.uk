...
abstract class PageAbstract {
	...
	public function hooks(): void {
		...
		if ( wp_doing_ajax() ) {
			add_action( "wp_ajax_easy_wp_smtp_page_check_{$plugin}_status", [ $this, 'ajax_check_plugin_status' ] );
	...
	public function enqueue_assets(): void {
		...
		wp_enqueue_script(
			'easy-wp-smtp-admin-recommendations-' . static::get_plugin_name(),
			easy_wp_smtp()->assets_url . "/js/smtp-recommendations{$min}.js",
	...
	public function output(): void {
		...
		echo '<div id="easy-wp-smtp-admin-' . esc_attr( static::get_plugin_name() ) . '" class="easy-wp-smtp-plugin-recommendation-page">';
	...
	protected function output_section_step_install(): void {
		...
		printf(
			...
			esc_attr__( 'Step 1', 'easy-wp-smtp' ),
	...
	protected function output_section_step_setup(): void {
		...
		printf(
			...
			esc_url( easy_wp_smtp()->assets_url . '/images/recommendations/' . $step['icon'] ),
			esc_attr__( 'Step 2', 'easy-wp-smtp' ),
			esc_html( $step['heading'] ),
			esc_html( $step['description'] ),
	...
	public function ajax_check_plugin_status(): void {
		...
		// Security checks.
		if (
			! check_ajax_referer( 'easy-wp-smtp-admin', 'nonce', false ) ||
			! current_user_can( easy_wp_smtp()->get_capability_manage_options() )
		) {
			wp_send_json_error(
				[ 'error' => esc_html__( 'You do not have permission.', 'easy-wp-smtp' ) ]
			);
		}
		...
		if ( ! $this->is_plugin_available() ) {
			wp_send_json_error(
				[ 'error' => esc_html__( 'Plugin unavailable.', 'easy-wp-smtp' ) ]
			);
		}
	...
	public function plugin_activated( string $plugin_basename ): void {
		...
		$source = easy_wp_smtp()->is_pro() ? 'Easy WP SMTP Pro' : 'Easy WP SMTP';
	...
	protected function get_js_strings(): array {
		...
		$error_could_not_install = sprintf(
			wp_kses( /* translators: %1$s - Lite plugin download URL. */
				__( 'Could not install the plugin automatically. Please <a href="%1$s">download</a> it and install it manually.', 'easy-wp-smtp' ),
				[
					'a' => [
						'href' => true,
					],
		...
		$error_could_not_activate = sprintf(
			wp_kses( /* translators: %1$s - Plugins page URL. */
				__( 'Could not activate the plugin. Please activate it on the <a href="%1$s">Plugins page</a>.', 'easy-wp-smtp' ),
				[
					'a' => [
						'href' => true,
					],
		...
		return [
			...
			'install_now'              => esc_html__( 'Install Now', 'easy-wp-smtp' ),
			'activate_now'             => esc_html__( 'Activate Now', 'easy-wp-smtp' ),
			'download_now'             => esc_html__( 'Download Now', 'easy-wp-smtp' ),
			'plugins_page'             => esc_html__( 'Go to Plugins page', 'easy-wp-smtp' ),
	...
	protected function get_heading_image_url(): string {
		...
		return easy_wp_smtp()->assets_url . '/images/recommendations/plugins/' . static::get_plugin_name() . '/logo.svg'; // phpcs:ignore WPForms.Formatting.EmptyLineBeforeReturn.RemoveEmptyLineBeforeReturnStatement
	}
	...
	protected function output_section_screenshot(): void {
		...
		foreach ( $features as $feature ) {
			$list .= sprintf(
				'<li><span aria-hidden="true" class="esmtp:icon-[fa6-solid--arrow-right] esmtp:text-link esmtp:w-[14px] esmtp:h-[14px] esmtp:shrink-0"></span>%s</li>',
				esc_html( $feature )
			);
		}
		...
		printf(
			...
			esc_url( easy_wp_smtp()->assets_url . '/images/recommendations/plugins/' . static::get_plugin_name() . '/screenshot-tnail.png' ),
			esc_attr( $this->get_screenshot_alt_text() ),
			esc_url( easy_wp_smtp()->assets_url . '/images/recommendations/plugins/' . static::get_plugin_name() . '/screenshot-full@2x.png' ),
			esc_url( easy_wp_smtp()->assets_url . '/images/recommendations/plugins/' . static::get_plugin_name() . '/screenshot-tnail@2x.png' ),
			wp_kses(
				$list,
				[
					'li'   => [],
