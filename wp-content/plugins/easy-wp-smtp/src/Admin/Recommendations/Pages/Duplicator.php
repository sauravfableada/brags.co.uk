...
class Duplicator extends PageAbstract {
	 ...
	 * @var string
	 */
	public const SLUG = 'easy-wp-smtp-recommended-duplicator';
	...
	protected $config = [
		...
		'duplicator_addon_page' => 'https://duplicator.com/?utm_source=easywpsmtpplugin&utm_medium=link&utm_campaign=duplicator-page',
	...
	protected function get_heading_title(): string {
		...
		return esc_html__( 'Easy WP SMTP Delivers It. Duplicator Protects It.', 'easy-wp-smtp' ); // phpcs:ignore WPForms.Formatting.EmptyLineBeforeReturn.RemoveEmptyLineBeforeReturnStatement
	}
	...
	protected function get_heading_alt_text(): string {
		...
		return esc_attr__( 'Easy WP SMTP ♥ Duplicator', 'easy-wp-smtp' ); // phpcs:ignore WPForms.Formatting.EmptyLineBeforeReturn.RemoveEmptyLineBeforeReturnStatement
	}
	...
	protected function get_heading_strings(): array {
		...
		return [ // phpcs:ignore WPForms.Formatting.EmptyLineBeforeReturn.RemoveEmptyLineBeforeReturnStatement
			esc_html__( 'Your email settings and sender reputation live in your database. One bad update, one crash, and everything is gone. Duplicator backs up your entire WordPress site automatically so you can restore with one click.', 'easy-wp-smtp' ),
			esc_html__( 'Trusted by over 1.5 million websites.', 'easy-wp-smtp' ),
		];
	}
	...
	protected function get_screenshot_features(): array {
		...
		return [ // phpcs:ignore WPForms.Formatting.EmptyLineBeforeReturn.RemoveEmptyLineBeforeReturnStatement
			esc_html__( 'Back up your entire site automatically.', 'easy-wp-smtp' ),
			esc_html__( 'Restore your site with one click if anything goes wrong.', 'easy-wp-smtp' ),
			esc_html__( 'Store backups safely in Google Drive, Dropbox, or Amazon S3.', 'easy-wp-smtp' ),
			esc_html__( 'Schedule daily backups so you never have to think about it.', 'easy-wp-smtp' ),
		];
	}
	...
	protected function get_screenshot_alt_text(): string {
		...
		return esc_attr__( 'Duplicator screenshot', 'easy-wp-smtp' ); // phpcs:ignore WPForms.Formatting.EmptyLineBeforeReturn.RemoveEmptyLineBeforeReturnStatement
	}
	...
	protected function output_section_step_result(): void {
		...
		printf(
			...
			esc_html__( 'Set Up Scheduled Cloud Backups', 'easy-wp-smtp' ),
			esc_html__( 'Keep your data safe forever with automatic daily backups to Google Drive, Dropbox, or Amazon S3.', 'easy-wp-smtp' ),
	...
	protected function get_data_step_result(): array {
		...
		$data = [
			...
			'button_text'   => esc_html__( 'Set Up Cloud Backups', 'easy-wp-smtp' ),
		];
		...
		if ( $count && $this->is_pro_active() ) {
			...
			if ( $this->get_schedule_count() ) {
				...
				$data['button_text']  = esc_html__( 'Cloud Backups Set Up', 'easy-wp-smtp' );
			}
		}
	...
	protected function get_package_count(): int {
		...
		$table_exists_cache_key  = "easy_wp_smtp_dup_table_exists_{$blog_id}";
		$package_count_cache_key = "easy_wp_smtp_dup_package_count_{$blog_id}";

		$table_exists = wp_cache_get( $table_exists_cache_key, 'easy-wp-smtp' );
		...
		if ( $table_exists === false ) {
			...
			wp_cache_set( $table_exists_cache_key, $table_exists, 'easy-wp-smtp', 60 );
		}

		$package_count = 0;

		if ( $table_exists === $packages_table ) {
			$package_count = wp_cache_get( $package_count_cache_key, 'easy-wp-smtp' );

			if ( $package_count === false ) {
				// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				$package_count = (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$packages_table}" );

				wp_cache_set( $package_count_cache_key, $package_count, 'easy-wp-smtp', 60 );
			}
		}
	...
	protected function get_install_heading(): string {
		...
		return esc_html__( 'Install & Activate Duplicator', 'easy-wp-smtp' ); // phpcs:ignore WPForms.Formatting.EmptyLineBeforeReturn.RemoveEmptyLineBeforeReturnStatement
	}
	...
	protected function get_install_description(): string {
		...
		return esc_html__( 'Your first step toward bulletproof backups.', 'easy-wp-smtp' ); // phpcs:ignore WPForms.Formatting.EmptyLineBeforeReturn.RemoveEmptyLineBeforeReturnStatement
	}
	...
	protected function get_plugin_title(): string {
		...
		return esc_html__( 'Duplicator', 'easy-wp-smtp' ); // phpcs:ignore WPForms.Formatting.EmptyLineBeforeReturn.RemoveEmptyLineBeforeReturnStatement
	}
	...
	protected function get_install_button_text(): string {
		...
		return esc_html__( 'Install Duplicator', 'easy-wp-smtp' ); // phpcs:ignore WPForms.Formatting.EmptyLineBeforeReturn.RemoveEmptyLineBeforeReturnStatement
	}
	...
	protected function get_installed_activated_text(): string {
		...
		return esc_html__( 'Duplicator Installed & Activated', 'easy-wp-smtp' ); // phpcs:ignore WPForms.Formatting.EmptyLineBeforeReturn.RemoveEmptyLineBeforeReturnStatement
	}
	...
	protected function get_activate_text(): string {
		...
		return esc_html__( 'Activate Duplicator', 'easy-wp-smtp' ); // phpcs:ignore WPForms.Formatting.EmptyLineBeforeReturn.RemoveEmptyLineBeforeReturnStatement
	}
	...
	protected function get_setup_heading(): string {
		...
		return esc_html__( 'Create Your First Backup', 'easy-wp-smtp' ); // phpcs:ignore WPForms.Formatting.EmptyLineBeforeReturn.RemoveEmptyLineBeforeReturnStatement
	}
	...
	protected function get_setup_description(): string {
		...
		return esc_html__( 'Back up your site, in just one click.', 'easy-wp-smtp' ); // phpcs:ignore WPForms.Formatting.EmptyLineBeforeReturn.RemoveEmptyLineBeforeReturnStatement
	}
	...
	protected function get_setup_button_text(): string {
		...
		return esc_html__( 'Create First Backup', 'easy-wp-smtp' ); // phpcs:ignore WPForms.Formatting.EmptyLineBeforeReturn.RemoveEmptyLineBeforeReturnStatement
	}
	...
	protected function get_setup_completed_text(): string {
		...
		return esc_html__( 'Backup Created', 'easy-wp-smtp' ); // phpcs:ignore WPForms.Formatting.EmptyLineBeforeReturn.RemoveEmptyLineBeforeReturnStatement
	}
	...
	protected function get_pro_installed_activated_text(): string {
		...
		return esc_html__( 'Duplicator Pro Installed & Activated', 'easy-wp-smtp' ); // phpcs:ignore WPForms.Formatting.EmptyLineBeforeReturn.RemoveEmptyLineBeforeReturnStatement
	}
}
