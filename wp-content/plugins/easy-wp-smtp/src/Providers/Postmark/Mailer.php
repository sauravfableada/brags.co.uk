...
class Mailer extends MailerAbstract {
	...
	public function get_debug_info() {
		...
		$text[] = '<strong>' . esc_html__( 'Server API Token:', 'easy-wp-smtp' ) . '</strong> ' .
							( ! empty( $options['server_api_token'] ) ? 'Yes' : 'No' );
		$text[] = '<strong>' . esc_html__( 'Message Stream ID:', 'easy-wp-smtp' ) . '</strong> ' .
	...
	private function get_message_stream() {
		 ...
		 */
		return apply_filters( 'easy_wp_smtp_providers_postmark_mailer_get_message_stream', $message_stream ); // phpcs:ignore WPForms.PHP.ValidateHooks.InvalidHookName
	}
