...
use EasyWPSMTP\Pro\AdditionalConnections\Connection as AdditionalConnection;
use EasyWPSMTP\Pro\AdditionalConnections\ConnectionOptions;
use EasyWPSMTP\WP;
...
class QuickConnect {
	...
	public function hooks() {
		...
		add_action( 'wp_ajax_easy_wp_smtp_sendlayer_connect', [ $this, 'ajax_init_connect' ] );
		...
		add_action( 'wp_ajax_easy_wp_smtp_sendlayer_disconnect', [ $this, 'ajax_disconnect' ] );
		add_action( 'easy_wpsmtp_admin_connection_settings_display_after_from_email_setting_row', [ $this, 'display_from_email_field' ], 10, 3 );
		add_filter( 'easy_wp_smtp_options_get', [ $this, 'filter_option' ], 10, 3 );
		add_action( 'easy_wp_smtp_admin_pages_before_content', [ $this, 'display_sendlayer_education_banner' ] );
		add_filter( 'easy_wp_smtp_admin_connection_settings_process_data', [ $this, 'process_settings_data' ], 10, 2 );
	 ...
	 * @since 2.14.0
	 */
	public function ajax_init_connect() { // phpcs:ignore Generic.Metrics.CyclomaticComplexity.TooHigh
	 ...
	 * for verification.
	 *
	 * Listens for the `easy_wp_smtp_sendlayer_quick_connect_verify` GET parameter.
	 *
	 * @since 2.14.0
	 */
	public function handle_ownership_verification_callback() {
		...
		if ( ! isset( $_GET['easy_wp_smtp_sendlayer_quick_connect_verify'] ) ) {
			return;
		}
		...
		$challenge_secret = get_transient( 'easy_wp_smtp_sendlayer_quick_connect_challenge_secret' );
		...
		delete_transient( 'easy_wp_smtp_sendlayer_quick_connect_challenge_secret' );
	...
	public function handle_auth_complete() { // phpcs:ignore Generic.Metrics.CyclomaticComplexity.MaxExceeded, Generic.Metrics.CyclomaticComplexity.TooHigh
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		if (
			empty( $_GET['easy_wp_smtp_sendlayer_quick_connect_auth_complete'] ) ||
		...
		$return_url = ! empty( $_GET['return_url'] )
			? wp_validate_redirect( esc_url_raw( wp_unslash( $_GET['return_url'] ) ), easy_wp_smtp()->get_admin()->get_admin_page_url() )
		...
		if ( ! wp_verify_nonce( $nonce, 'easy_wp_smtp_sendlayer_quick_connect' ) ) {
		...
		if ( $mode === 'backup_mailer' && easy_wp_smtp()->is_pro() ) {
			...
			if ( ! empty( $connection_id ) && easy_wp_smtp()->is_pro() ) {
	...
	public function display_notice() {
		...
		if ( $disconnect_result === 'success' ) {
			WP::add_admin_notice(
				esc_html__( 'SendLayer disconnected successfully.', 'easy-wp-smtp' ),
		...
		if ( $result === 'success' ) {
			WP::add_admin_notice(
				esc_html__( 'SendLayer connected successfully! You can now send emails through SendLayer.', 'easy-wp-smtp' ),
				WP::ADMIN_NOTICE_SUCCESS
			);
		...
		if ( $result === 'backup_success' ) {
			WP::add_admin_notice(
				esc_html__( 'SendLayer is now set up as your Backup Connection. Emails that fail to send with your primary connection will be sent via SendLayer.', 'easy-wp-smtp' ),
				WP::ADMIN_NOTICE_SUCCESS
			);
		...
		$actionable_messages = [
			'plugin.return.permission_denied' => esc_html__( 'SendLayer connection failed. You do not have permission to perform this action.', 'easy-wp-smtp' ),
			'plugin.return.invalid_nonce'     => esc_html__( 'SendLayer connection failed. Your session has expired. Please try again.', 'easy-wp-smtp' ),
			'site.exchange.rate_limit'        => esc_html__( 'SendLayer connection failed. Too many attempts. Please wait a few minutes and try again.', 'easy-wp-smtp' ),
			'site.exchange.code_expired'      => esc_html__( 'SendLayer connection failed. Your connection code has expired. Please try again.', 'easy-wp-smtp' ),
		];
	...
	public function handle_disconnect() {
		...
		if ( ! current_user_can( easy_wp_smtp()->get_capability_manage_global_options() ) ) {
			WP::add_admin_notice(
				esc_html__( 'SendLayer disconnect failed. You do not have permission to perform this action.', 'easy-wp-smtp' ),
		...
		if ( ! wp_verify_nonce( sanitize_key( $_GET['sendlayer_quick_connect_disconnect_nonce'] ), 'sendlayer_quick_connect_disconnect' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			WP::add_admin_notice(
				esc_html__( 'SendLayer disconnect failed. The nonce is invalid. Please try again.', 'easy-wp-smtp' ),
				WP::ADMIN_NOTICE_ERROR
			);
	...
	public function display_sendlayer_education_banner() {
		...
		$mailer = easy_wp_smtp()->get_connections_manager()->get_primary_connection()->get_mailer_slug();
		...
		$is_dismissed = (bool) get_user_meta( get_current_user_id(), 'easy_wp_smtp_notice_sendlayer_education_dismissed', true );
		...
		// phpcs:ignore WordPress.Arrays.ArrayDeclarationSpacing.AssociativeArrayFound
		$sendlayer_url = easy_wp_smtp()->get_utm_url( 'https://sendlayer.com/', [ 'source' => 'easywpsmtpplugin', 'medium' => 'WordPress', 'content' => 'Default Mailer - SendLayer Education' ] );
		?>
		<div id="easy-wp-smtp-sendlayer-education-banner" class="easy-wp-smtp-sendlayer-education">
			<div class="easy-wp-smtp-sendlayer-education__content">
				<div class="easy-wp-smtp-sendlayer-education__text">
					<div class="easy-wp-smtp-sendlayer-education__heading">
						<span class="easy-wp-smtp-sendlayer-education__icon">
						...
						<?php esc_html_e( 'Seems like you don\'t have a mailer setup yet!', 'easy-wp-smtp' ); ?>
					</div>
					<p class="easy-wp-smtp-sendlayer-education__desc">
						<?php
						printf(
							wp_kses(
								/* translators: %s - URL to SendLayer. */
								__( 'You\'re using the default WordPress mailer, which often lands in spam! Switch to <a href="%s" target="_blank" rel="noopener noreferrer">SendLayer</a>, our recommended mailer, and send your <strong>first 200 emails for free with just a few clicks!</strong>', 'easy-wp-smtp' ),
								[
									'a'      => [
										'href'   => [],
										'rel'    => [],
				...
				<div class="easy-wp-smtp-sendlayer-education__actions">
					<button type="button" id="easy-wp-smtp-sendlayer-education-connect-btn" class="easy-wp-smtp-btn easy-wp-smtp-btn--primary easy-wp-smtp-btn--lg easy-wp-smtp-sendlayer-education__btn">
						<?php esc_html_e( 'Setup SendLayer', 'easy-wp-smtp' ); ?>
			...
			<div class="easy-wp-smtp-sendlayer-education__illustration">
				<img src="<?php echo esc_url( easy_wp_smtp()->assets_url . '/images/education/sendlayer-banner.svg' ); ?>" alt="" />
			</div>
			<button type="button" class="easy-wp-smtp-sendlayer-education__dismiss js-easy-wp-smtp-sendlayer-education-dismiss" title="<?php esc_attr_e( 'Dismiss', 'easy-wp-smtp' ); ?>">
			...
			<p class="desc easy-wp-smtp-sendlayer-connect-error" style="margin-top: 10px;"></p>
		</div>
		<?php
	}
	...
	public function display_from_email_field( $connection, $connection_options, $mailer ) {
		...
		?>
		<div id="easy-wp-smtp-setting-row-sendlayer-quick-connect-from_email"
		     class="easy-wp-smtp-row easy-wp-smtp-setting-row easy-wp-smtp-setting-row--text"
		     style="display: <?php echo $is_active ? 'flex' : 'none'; ?>;">
			<div class="easy-wp-smtp-setting-row__label">
				<label for="easy-wp-smtp-setting-sendlayer-quick-connect-from_email">
					<?php esc_html_e( 'From Email Address', 'easy-wp-smtp' ); ?>
				</label>
			</div>
			<div class="easy-wp-smtp-setting-row__field">
				<div class="easy-wp-smtp-sendlayer-from-email-wrap">
					<input name="easy-wp-smtp[mail][from_email_local]" type="text"
					       value="<?php echo esc_attr( $local_part ); ?>"
					       id="easy-wp-smtp-setting-sendlayer-quick-connect-from_email" spellcheck="false"
					       <?php disabled( ! $is_active ); ?>
					/>
					<span class="easy-wp-smtp-sendlayer-from-email-domain">
						@<?php echo esc_html( $sender_domain ); ?>
					</span>
				</div>
				<input type="hidden" name="easy-wp-smtp[mail][from_email_domain]"
				...
				<p class="desc">
					...
					printf(
						wp_kses(
							...
							__( 'You can customize your From Email by adding your domain on <a href="%1$s" target="_blank" rel="noopener noreferrer">SendLayer dashboard</a>.<br>Check our <a href="%2$s" target="_blank" rel="noopener noreferrer">documentation</a> on how to add a custom domain.', 'easy-wp-smtp' ),
							[
								'a'  => [
									'href'   => [],
									'rel'    => [],
						...
						// phpcs:ignore WordPress.Arrays.ArrayDeclarationSpacing.AssociativeArrayFound
						esc_url( easy_wp_smtp()->get_utm_url( 'https://app.sendlayer.com/', [ 'source' => 'easywpsmtpplugin', 'medium' => 'WordPress', 'content' => 'Plugin Settings - From Email Domain Link' ] ) ),
						// phpcs:ignore WordPress.Arrays.ArrayDeclarationSpacing.AssociativeArrayFound
	 ...
	 *
	 * Allows overriding via the EASY_WP_SMTP_SENDLAYER_SITE_URL constant.
	 *
	 * @since 2.14.0
	 *
	 * @return string
	...
	private function get_marketing_site_url() {
		...
		if ( defined( 'EASY_WP_SMTP_SENDLAYER_SITE_URL' ) && EASY_WP_SMTP_SENDLAYER_SITE_URL ) {
			return rtrim( EASY_WP_SMTP_SENDLAYER_SITE_URL, '/' );
		}
	...
	private function get_generic_error_message() {
		...
		return esc_html__( 'Something went wrong while connecting to SendLayer. Please try again, or contact support and provide the error code.', 'easy-wp-smtp' );
	}
