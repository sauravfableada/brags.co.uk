...
use EasyWPSMTP\ConnectionsManager;
use EasyWPSMTP\Helpers\Data;
...
class QuickConnectUsage {
	 ...
	 * @var string
	 */
	const STATE_OPTION = 'easy_wp_smtp_sendlayer_quick_connect';
	...
	public function __construct( $connections_manager = null ) {
		...
		$this->connections_manager = $connections_manager === null
			? easy_wp_smtp()->get_connections_manager()
			: $connections_manager;
	}
	...
	public function hooks() {
		...
		add_action( 'easy_wp_smtp_mailcatcher_send_after', [ $this, 'capture_usage_from_response' ], 10, 1 );
		add_action( 'easy_wp_smtp_mailcatcher_send_after', [ $this, 'clear_limit_reached_on_success' ], 10, 1 );
		add_action( 'easy_wp_smtp_mailcatcher_send_failed', [ $this, 'maybe_flag_limit_reached' ], 10, 6 );
	...
	private function rearm_notice( $key ) {
		...
		delete_metadata( 'user', 0, "easy_wp_smtp_notice_{$key}_dismissed", '', true );
	}
	...
	public function get_free_upgrade_url() {
		...
		return easy_wp_smtp()->get_utm_url( $target, [ 'source' => 'easywpsmtpplugin', 'medium' => 'WordPress', 'content' => 'SendLayer QC Usage Notice - Upgrade' ] );
	}
	...
	protected function custom_domain_message() {
		...
		$app_url = easy_wp_smtp()->get_utm_url( 'https://app.sendlayer.com/', [ 'source' => 'easywpsmtpplugin', 'medium' => 'WordPress', 'content' => 'QC Usage Notice - Custom Domain App' ] );
		$doc_url = easy_wp_smtp()->get_utm_url( 'https://sendlayer.com/docs/authorizing-your-domain/', [ 'source' => 'easywpsmtpplugin', 'medium' => 'WordPress', 'content' => 'QC Usage Notice - Custom Domain Docs' ] );
		// phpcs:enable WordPress.Arrays.ArrayDeclarationSpacing.AssociativeArrayFound

		$message = sprintf(
			wp_kses(
				/* translators: %1$s - SendLayer dashboard URL; %2$s - custom domain documentation URL. */
				__( 'You\'re sending through a shared SendLayer domain. For better deliverability, add your own domain on the <a href="%1$s" target="_blank" rel="noopener noreferrer">SendLayer dashboard</a> and follow our <a href="%2$s" target="_blank" rel="noopener noreferrer">custom domain guide</a>.', 'easy-wp-smtp' ),
				[
					'a' => [
						'href'   => [],
						'rel'    => [],
	...
	protected function limit_warning_message() {
		...
		$message = sprintf(
			wp_kses(
				...
				__( 'You\'ve used %1$d of your %2$d free SendLayer emails. <a href="%3$s" target="_blank" rel="noopener noreferrer">Upgrade your plan</a> to keep your emails flowing without interruption.', 'easy-wp-smtp' ),
				[
					'a' => [
						'href'   => [],
						'rel'    => [],
	...
	protected function limit_reached_message() {
		...
		$message = sprintf(
			wp_kses(
				...
				__( 'You\'ve reached your free SendLayer limit. Emails are failing and won\'t be delivered! <a href="%s" target="_blank" rel="noopener noreferrer">Upgrade your plan</a> to start sending again.', 'easy-wp-smtp' ),
				[
					'a' => [
						'href'   => [],
						'rel'    => [],
