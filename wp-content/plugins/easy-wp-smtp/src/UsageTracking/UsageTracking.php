...
namespace EasyWPSMTP\UsageTracking;
...
class UsageTracking {
	 ...
	 * @since 2.1.0
	 */
	const FAILED_SETUP_WIZARD_DATA_URL = 'https://easywpsmtpusage.com/v1/easywpsmtp-failed-wizard';
	...
	public function is_enabled() {
		...
		return (bool) apply_filters(
			'easy_wp_smtp_usage_tracking_is_enabled',
	...
	public function load() {
		...
		if ( ! (bool) apply_filters( 'easy_wp_smtp_usage_tracking_load_allowed', true ) ) {
			return;
		}

		// Deregister the action if option is disabled.
		add_action(
			'easy_wp_smtp_options_set_after',
			function () {
	...
	public function get_data() {
		...
		if ( 'smtp' === $options->get( 'mail', 'mailer' ) ) {
			$data['easy_wp_smtp_other_smtp_host']       = $options->get( 'smtp', 'host' );
			$data['easy_wp_smtp_other_smtp_encryption'] = $options->get( 'smtp', 'encryption' );
	...
	private function get_additional_data() {
		...
		$activated_dates = get_option( 'easy_wp_smtp_activated', [] );

		return [
			'easy_wp_smtp_license_key'         => easy_wp_smtp()->get_license_key(),
			'easy_wp_smtp_license_type'        => easy_wp_smtp()->get_license_type(),
			'easy_wp_smtp_is_pro'              => easy_wp_smtp()->is_pro(),
			'easy_wp_smtp_lite_installed_date' => $this->get_installed( $activated_dates, 'lite' ),
			'easy_wp_smtp_pro_installed_date'  => $this->get_installed( $activated_dates, 'pro' ),
		];
	}
	...
	private function get_domain_checker_results( $domain_checker ) {
		...
		$data['easy_wp_smtp_domain_checker_success'] = isset( $results['success'] ) ? (bool) $results['success'] : false;
		$data['easy_wp_smtp_domain_checker_message'] = isset( $results['message'] ) ? $results['message'] : '';
		...
		foreach ( $results['checks'] as $check ) {
			...
			$data[ 'easy_wp_smtp_domain_checker_check_' . $check['type'] ] = $check['state'];
		}
