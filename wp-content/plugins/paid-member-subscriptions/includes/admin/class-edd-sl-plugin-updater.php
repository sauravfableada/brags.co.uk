...
if( !class_exists('PMS_EDD_SL_Plugin_Updater') ) {
    class PMS_EDD_SL_Plugin_Updater {
        ...
        public function show_update_notification( $file, $plugin ) {
            ...
            printf(
                ...
                esc_html__( 'There is a new version of %1$s available.', 'paid-member-subscriptions' ),
                esc_html( $plugin['Name'] )
            );

            if ( ! current_user_can( 'update_plugins' ) ) {
                echo ' ';
                esc_html_e( 'Contact your network administrator to install the update.', 'paid-member-subscriptions' );
            } elseif ( empty( $update_cache->response[ $this->name ]->package ) && ! empty( $changelog_link ) ) {
                echo ' ';
                printf(
                    /* translators: 1. opening anchor tag, do not translate 2. the new plugin version 3. closing anchor tag, do not translate. */
                    esc_html__( '%1$sView version %2$s details%3$s.', 'paid-member-subscriptions' ),
                ...
                echo ' ';
                printf(
                    esc_html__( '%1$sView version %2$s details%3$s or %4$supdate now%5$s.', 'paid-member-subscriptions' ),
                    '<a target="_blank" class="thickbox open-plugin-details-modal" href="' . esc_url( $changelog_link ) . '">',
                    esc_html( $update_cache->response[ $this->name ]->new_version ),
                    '</a>',
                ...
                printf(
                    ' %1$s%2$s%3$s',
                    '<a target="_blank" class="update-link" href="' . esc_url( wp_nonce_url( $update_link, 'upgrade-plugin_' . $file ) ) . '">',
                    esc_html__( 'Update now.', 'paid-member-subscriptions' ),
                    '</a>'
                );
            }
        ...
        public function show_changelog() {
            ...
            if ( ! current_user_can( 'update_plugins' ) ) {
                wp_die( esc_html__( 'You do not have permission to install plugin updates', 'paid-member-subscriptions' ), esc_html__( 'Error', 'paid-member-subscriptions' ), array( 'response' => 403 ) );
            }
...
class PMS_Plugin_Updater {
    ...
    public function __construct(){
        ...
        if( pms_get_product_version() != 'free' ){
            ...
            // Activate current site if licence is correct
    ...
    public function edd_sanitize_license( $new ) {
        ...
        $old = $this->get_option( 'pms_serial_number' );
        if( $old && $old != $new ) {
            $this->delete_option( 'pms_license_status' ); // new license has been entered, so must reactivate
        }
        return $new;
    }
    ...
    public function check_license( $transient_data ){
        ...
        if ( false === ( $pms_check_license = get_transient( 'pms_checked_licence' ) ) ) {

            $license         = trim( $this->get_option( 'pms_serial_number' ) );
            ...
            $this->update_option('pms_license_details', $license_details);
            ...
            set_transient( 'pms_checked_licence', 'yes', DAY_IN_SECONDS );
    ...
    public function activate_license() {
        ...
        if( isset( $_POST['pms_edd_license_activate'] ) ) {
            ...
            if ( isset( $_POST['pms_serial_number'] ) && preg_match('/[*]{3,}/', $_POST['pms_serial_number']) && strlen( $_POST['pms_serial_number'] ) > 5 ) { //phpcs:ignore
                ...
                // save the license
                $license = $this->edd_sanitize_license( trim( $_POST['pms_serial_number'] ) );//phpcs:ignore
                $this->update_option( 'pms_serial_number', $license );
            }
            ...
            if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
                ...
                $message[] = ( is_wp_error( $response ) && ! empty( $response_error_message ) ) ? $response->get_error_message() : __( 'An error occurred, please try again.', 'paid-member-subscriptions' );
                ...
                if ( false === $license_data->success ) {
                    ...
                    switch( $license_data->error ) {
                        case 'expired' :
                            $message[] = sprintf(
                                __( 'Your license key expired on %s.', 'paid-member-subscriptions' ),
                                date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
                            );
                            break;
                        case 'revoked' :
                            $message[] = __( 'Your license key has been disabled.', 'paid-member-subscriptions' );
                            break;
                        case 'missing' :
                            $message[] = __( 'Invalid license.', 'paid-member-subscriptions' );
                            break;
                        case 'invalid' :
                        case 'site_inactive' :
                            $message[] = __( 'Your license is not active for this URL.', 'paid-member-subscriptions' );
                            break;
                        case 'item_name_mismatch' :
                            $message[] = sprintf( __( 'This appears to be an invalid license key for %s.', 'paid-member-subscriptions' ), $this->get_edd_product_name() );
                            break;
                        case 'no_activations_left':
                            $message[] = __( 'Your license key has reached its activation limit.', 'paid-member-subscriptions' );
                            break;
                        default :
    ...
    public function initial_site_activation(){
        ...
        if( is_multisite() )
            $edd_sl_initial_activation = get_network_option( null, 'pms_edd_sl_initial_activation', false );
        else
            $edd_sl_initial_activation = get_option( 'pms_edd_sl_initial_activation', false );

        if( $edd_sl_initial_activation != false )
            return;

        $license = $this->get_option( 'pms_serial_number' );
        ...
        if( is_multisite() )
            update_network_option( null, 'pms_edd_sl_initial_activation', 'yes' );
        else
            update_option( 'pms_edd_sl_initial_activation', 'yes', false );
    ...
    function deactivate_license() {
        ...
        // listen for our activate button to be clicked
        if( isset( $_POST['pms_edd_license_deactivate'] ) ) {
