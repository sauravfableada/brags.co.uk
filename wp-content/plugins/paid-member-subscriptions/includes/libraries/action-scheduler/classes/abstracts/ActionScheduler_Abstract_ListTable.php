...
abstract class ActionScheduler_Abstract_ListTable extends WP_List_Table {
	...
	protected function get_bulk_actions() {
		...
		foreach ( $this->bulk_actions as $action => $label ) {
			if ( ! is_callable( array( $this, 'bulk_' . $action ) ) ) {
                throw new RuntimeException( sprintf( esc_html__( 'The bulk action %s does not have a callback method.', 'paid-member-subscriptions' ), esc_html( $action ) ) );
			}
	...
	public function extra_tablenav( $which ) {
		...
		submit_button( esc_html__( 'Filter', 'paid-member-subscriptions' ), '', 'filter_action', false, array( 'id' => 'post-query-submit' ) );
		echo '</div>';
	}
	...
	protected function display_header() {
		...
		if ( $this->get_request_search_query() ) {
			...
			echo '<span class="subtitle">' . esc_attr( sprintf( esc_html__( 'Search results for "%s"', 'paid-member-subscriptions' ), $this->get_request_search_query() ) ) . '</span>';
		}
	...
	protected function display_filter_by_status() {
		...
		$status_labels['all']      = esc_html_x( 'All', 'status labels', 'paid-member-subscriptions' );
		$status_labels['past-due'] = esc_html_x( 'Past-due', 'status labels', 'paid-member-subscriptions' );
	...
	protected function get_search_box_placeholder() {
		return esc_html__( 'Search', 'paid-member-subscriptions' );
	}
