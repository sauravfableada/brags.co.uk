...
abstract class ActionScheduler_Logger {
	...
	public function log_stored_action( $action_id ) {
		$this->log( $action_id, esc_html__( 'action created', 'paid-member-subscriptions' ) );
	}
	...
	public function log_canceled_action( $action_id ) {
		$this->log( $action_id, esc_html__( 'action canceled', 'paid-member-subscriptions' ) );
	}
	...
	public function log_started_action( $action_id, $context = '' ) {
		if ( ! empty( $context ) ) {
			...
			$message = sprintf( esc_html__( 'action started via %s', 'paid-member-subscriptions' ), $context );
		} else {
			$message = esc_html__('action started', 'paid-member-subscriptions' );
		}
	...
	public function log_completed_action( $action_id, $action = null, $context = '' ) {
		if ( ! empty( $context ) ) {
			/* translators: %s: context */
			$message = sprintf( esc_html__( 'action complete via %s', 'paid-member-subscriptions' ), $context );
		} else {
			$message = esc_html__('action complete', 'paid-member-subscriptions' );
		}
		$this->log( $action_id, $message );
	}
	...
	public function log_failed_action( $action_id, Exception $exception, $context = '' ) {
		if ( ! empty( $context ) ) {
			...
			$message = sprintf( esc_html__( 'action failed via %1$s: %2$s', 'paid-member-subscriptions' ), $context, $exception->getMessage() );
		} else {
			/* translators: %s: exception message */
			$message = sprintf( esc_html__( 'action failed: %s', 'paid-member-subscriptions' ), $exception->getMessage() );
		}
		$this->log( $action_id, $message );
	}
	...
	public function log_timed_out_action( $action_id, $timeout ) {
		...
		$this->log( $action_id, sprintf( esc_html__( 'action marked as failed after %s seconds. Unknown error occurred. Check server, PHP and database error logs to diagnose cause.', 'paid-member-subscriptions' ), $timeout ) );
	}
	...
	public function log_unexpected_shutdown( $action_id, $error ) {
		if ( ! empty( $error ) ) {
			...
			$this->log( $action_id, sprintf( esc_html__( 'unexpected shutdown: PHP Fatal error %1$s in %2$s on line %3$s', 'paid-member-subscriptions' ), $error['message'], $error['file'], $error['line'] ) );
		}
	}
	...
	public function log_reset_action( $action_id ) {
		$this->log( $action_id, esc_html__( 'action reset', 'paid-member-subscriptions' ) );
	}
	...
	public function log_ignored_action( $action_id, $context = '' ) {
		if ( ! empty( $context ) ) {
			/* translators: %s: context */
			$message = sprintf( esc_html__( 'action ignored via %s', 'paid-member-subscriptions' ), $context );
		} else {
			$message = esc_html__('action ignored', 'paid-member-subscriptions' );
		}
		$this->log( $action_id, $message );
	}
	...
	public function log_failed_fetch_action( $action_id, ?Exception $exception = null ) {
		...
		if ( ! is_null( $exception ) ) {
			/* translators: %s: exception message */
			$log_message = sprintf( esc_html__( 'There was a failure fetching this action: %s', 'paid-member-subscriptions' ), $exception->getMessage() );
		} else {
			$log_message = esc_html__('There was a failure fetching this action', 'paid-member-subscriptions' );
		}
	...
	public function log_failed_schedule_next_instance( $action_id, Exception $exception ) {
		/* translators: %s: exception message */
		$this->log( $action_id, sprintf( esc_html__( 'There was a failure scheduling the next instance of this action: %s', 'paid-member-subscriptions' ), $exception->getMessage() ) );
	}
