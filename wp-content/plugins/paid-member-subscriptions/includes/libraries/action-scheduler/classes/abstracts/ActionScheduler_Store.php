...
abstract class ActionScheduler_Store extends ActionScheduler_Store_Deprecated {
	...
	protected function validate_args( $args, $action_id ) {
		...
		if ( ! is_array( $args ) ) {
            // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- Value passed to an exception, not directly output.
	...
	protected function validate_action( ActionScheduler_Action $action ) {
		if ( strlen( wp_json_encode( $action->get_args() ) ) > static::$max_args_length ) {
			...
			throw new InvalidArgumentException( sprintf( esc_html__( 'ActionScheduler_Action::$args too long. To ensure the args column can be indexed, action args should not be more than %d characters when encoded as JSON.', 'paid-member-subscriptions' ), absint( static::$max_args_length ) ) );
		}
	}
	...
	public function get_status_labels() {
		return array(
			self::STATUS_COMPLETE => esc_html__('Complete', 'paid-member-subscriptions' ),
			self::STATUS_PENDING  => esc_html__('Pending', 'paid-member-subscriptions' ),
			self::STATUS_RUNNING  => esc_html__('In-progress', 'paid-member-subscriptions' ),
			self::STATUS_FAILED   => esc_html__('Failed', 'paid-member-subscriptions' ),
			self::STATUS_CANCELED => esc_html__('Canceled', 'paid-member-subscriptions' ),
		);
	}
