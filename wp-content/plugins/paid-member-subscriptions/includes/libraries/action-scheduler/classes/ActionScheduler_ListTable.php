...
class ActionScheduler_ListTable extends ActionScheduler_Abstract_ListTable {
	...
	public function __construct( ActionScheduler_Store $store, ActionScheduler_Logger $logger, ActionScheduler_QueueRunner $runner ) {
		...
		$this->table_header = esc_html__('Scheduled Actions', 'paid-member-subscriptions' );

		$this->bulk_actions = array(
			'delete' => esc_html__('Delete', 'paid-member-subscriptions' ),
		);

		$this->columns = array(
			'hook'        => esc_html__('Hook', 'paid-member-subscriptions' ),
			'status'      => esc_html__('Status', 'paid-member-subscriptions' ),
			'args'        => esc_html__('Arguments', 'paid-member-subscriptions' ),
			'group'       => esc_html__('Group', 'paid-member-subscriptions' ),
			'recurrence'  => esc_html__('Recurrence', 'paid-member-subscriptions' ),
			'schedule'    => esc_html__('Scheduled Date', 'paid-member-subscriptions' ),
			'log_entries' => esc_html__('Log', 'paid-member-subscriptions' ),
		);
		...
		if ( empty( $request_status ) ) {
			...
			$this->columns  += array( 'claim_id' => esc_html__('Claim ID', 'paid-member-subscriptions' ) );
		...
		$this->row_actions = array(
			'hook' => array(
				'run'    => array(
					'name' => esc_html__('Run', 'paid-member-subscriptions' ),
					'desc' => esc_html__('Process the action now as if it were run as part of a queue', 'paid-member-subscriptions' ),
				),
				'cancel' => array(
					'name'  => esc_html__('Cancel', 'paid-member-subscriptions' ),
					'desc'  => esc_html__('Cancel the action now to avoid it being run in future', 'paid-member-subscriptions' ),
					'class' => 'cancel trash',
				),
			),
		);
		...
		self::$time_periods = array(
			array(
				...
				'names'   => _n_noop( '%s year', '%s years', 'paid-member-subscriptions' ),
			),
			array(
				'seconds' => MONTH_IN_SECONDS,
				/* translators: %s: amount of time */
				'names'   => _n_noop( '%s month', '%s months', 'paid-member-subscriptions' ),
			),
			array(
				'seconds' => WEEK_IN_SECONDS,
				/* translators: %s: amount of time */
				'names'   => _n_noop( '%s week', '%s weeks', 'paid-member-subscriptions' ),
			),
			array(
				'seconds' => DAY_IN_SECONDS,
				/* translators: %s: amount of time */
				'names'   => _n_noop( '%s day', '%s days', 'paid-member-subscriptions' ),
			),
			array(
				'seconds' => HOUR_IN_SECONDS,
				/* translators: %s: amount of time */
				'names'   => _n_noop( '%s hour', '%s hours', 'paid-member-subscriptions' ),
			),
			array(
				'seconds' => MINUTE_IN_SECONDS,
				/* translators: %s: amount of time */
				'names'   => _n_noop( '%s minute', '%s minutes', 'paid-member-subscriptions' ),
			),
			array(
				'seconds' => 1,
				/* translators: %s: amount of time */
				'names'   => _n_noop( '%s second', '%s seconds', 'paid-member-subscriptions' ),
			),
		);
	...
	private static function human_interval( $interval, $periods_to_include = 2 ) {
		...
		if ( $interval <= 0 ) {
			return esc_html__( 'Now!', 'paid-member-subscriptions' );
		}
		...
		for ( $time_period_index = 0, $periods_included = 0, $seconds_remaining = $interval; $time_period_index < $num_time_periods && $seconds_remaining > 0 && $periods_included < $periods_to_include; $time_period_index++ ) {
			...
			if ( $periods_in_interval > 0 ) {
				...
				$output            .= sprintf( translate_nooped_plural( self::$time_periods[ $time_period_index ]['names'], $periods_in_interval, 'paid-member-subscriptions' ), $periods_in_interval );
	...
	protected function get_recurrence( $action ) {
		...
		if ( $schedule->is_recurring() && method_exists( $schedule, 'get_recurrence' ) ) {
			...
			if ( is_numeric( $recurrence ) ) {
				...
				return sprintf( esc_html__( 'Every %s', 'paid-member-subscriptions' ), self::human_interval( $recurrence ) );
			} else {
				return $recurrence;
			}
		}

		return esc_html__( 'Non-repeating', 'paid-member-subscriptions' );
	}
	...
	public function display_admin_notices() {
			...
			foreach ( $table_list as $table_name ) {
				if ( ! in_array( $wpdb->prefix . $table_name, $found_tables, true ) ) {
					$this->admin_notices[] = array(
						'class'   => 'error',
						'message' => esc_html__('It appears one or more database tables were missing. Attempting to re-create the missing table(s).', 'paid-member-subscriptions' ),
					);
		...
		if ( $this->runner->has_maximum_concurrent_batches() ) {
			...
			if ( false === $async_request_lock_expiration || $async_request_lock_expiration < time() ) {
				...
				$async_request_message = sprintf( esc_html__( 'A new queue has begun processing. <a href="%s">View actions in-progress &raquo;</a>', 'paid-member-subscriptions' ), esc_url( $in_progress_url ) );
			} else {
				/* translators: %d: seconds */
				$async_request_message = sprintf( esc_html__( 'The next queue will begin processing in approximately %d seconds.', 'paid-member-subscriptions' ), $async_request_lock_expiration - time() );
			}
			...
			if ( 1 === absint( $notification['success'] ) ) {
				...
				switch ( $notification['row_action_type'] ) {
					case 'run':
						...
						$action_message_html = sprintf( esc_html__( 'Successfully executed action: %s', 'paid-member-subscriptions' ), $action_hook_html );
						break;
					case 'cancel':
						/* translators: %s: action HTML */
						$action_message_html = sprintf( esc_html__( 'Successfully canceled action: %s', 'paid-member-subscriptions' ), $action_hook_html );
						break;
					default:
						/* translators: %s: action HTML */
						$action_message_html = sprintf( esc_html__( 'Successfully processed change for action: %s', 'paid-member-subscriptions' ), $action_hook_html );
						break;
				}
			} else {
				$class = 'error';
				/* translators: 1: action HTML 2: action ID 3: error message */
				$action_message_html = sprintf( esc_html__( 'Could not process change for action: "%1$s" (ID: %2$d). Error: %3$s', 'paid-member-subscriptions' ), $action_hook_html, esc_html( $notification['action_id'] ), esc_html( $notification['error_message'] ) );
			}
	...
	protected function get_schedule_display_string( ActionScheduler_Schedule $schedule ) {
		...
		if ( is_a( $schedule, 'ActionScheduler_NullSchedule' ) ) {
			return esc_html__( 'async', 'paid-member-subscriptions' );
		}
		...
		if ( gmdate( 'U' ) > $next_timestamp ) {
			...
			$schedule_display_string .= sprintf( esc_html__( ' (%s ago)', 'paid-member-subscriptions' ), self::human_interval( gmdate( 'U' ) - $next_timestamp ) );
		} else {
			/* translators: %s: date interval */
			$schedule_display_string .= sprintf( esc_html__( ' (%s)', 'paid-member-subscriptions' ), self::human_interval( $next_timestamp - gmdate( 'U' ) ) );
		}
			...
			try {
				...
				error_log(
					sprintf(
						...
						__( 'Action Scheduler was unable to delete action %1$d. Reason: %2$s', 'paid-member-subscriptions' ),
						$id,
						$e->getMessage()
					)
				);
	...
	protected function get_search_box_button_text() {
		return esc_html__( 'Search hook, args and claim ID', 'paid-member-subscriptions' );
	}
