...
class ActionScheduler_WPCLI_QueueRunner extends ActionScheduler_Abstract_QueueRunner {
	...
	public function __construct( ?ActionScheduler_Store $store = null, ?ActionScheduler_FatalErrorMonitor $monitor = null, ?ActionScheduler_QueueCleaner $cleaner = null ) {
		if ( ! ( defined( 'WP_CLI' ) && WP_CLI ) ) {
			...
			throw new Exception( sprintf( esc_html__( 'The %s class can only be run within WP CLI.', 'paid-member-subscriptions' ), __CLASS__ ) );
		}
	...
	public function setup( $batch_size, $hooks = array(), $group = '', $force = false ) {
		...
		if ( $this->has_maximum_concurrent_batches() ) {
			if ( $force ) {
				WP_CLI::warning( esc_html__( 'There are too many concurrent batches, but the run is forced to continue.', 'paid-member-subscriptions' ) );
			} else {
				WP_CLI::error( esc_html__( 'There are too many concurrent batches.', 'paid-member-subscriptions' ) );
			}
		}
	...
	protected function setup_progress_bar() {
		...
		$this->progress_bar = new ProgressBar(
			...
			sprintf( _n( 'Running %d action', 'Running %d actions', $count, 'paid-member-subscriptions' ), $count ),
			$count
		);
	}
	...
	public function run( $context = 'WP CLI' ) {
		...
		foreach ( $this->actions as $action_id ) {
			...
			if ( ! in_array( $action_id, $this->store->find_actions_by_claim_id( $this->claim->get_id() ), true ) ) {
				WP_CLI::warning( esc_html__( 'The claim has been lost. Aborting current batch.', 'paid-member-subscriptions' ) );
				break;
			}
	...
	public function before_execute( $action_id ) {
		...
		WP_CLI::log(sprintf( esc_html__( 'Started processing action %s', 'paid-member-subscriptions' ), $action_id ) );
	}
	...
	public function after_execute( $action_id, $action = null ) {
		...
		WP_CLI::log( sprintf( esc_html__( 'Completed processing action %1$s with hook: %2$s', 'paid-member-subscriptions' ), $action_id, $action->get_hook() ) );
	}
	...
	public function action_failed( $action_id, $exception ) {
		WP_CLI::error(
			...
			sprintf( esc_html__( 'Error processing action %1$s: %2$s', 'paid-member-subscriptions' ), $action_id, $exception->getMessage() ),
			false
		);
	}
