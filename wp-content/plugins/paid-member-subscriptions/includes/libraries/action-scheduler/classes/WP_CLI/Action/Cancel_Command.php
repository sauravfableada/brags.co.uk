...
class Cancel_Command extends \ActionScheduler_WPCLI_Command {
	...
	protected function cancel_single( $hook, $callback_args, $group ) {
		if ( empty( $hook ) ) {
			\WP_CLI::error( esc_html__( 'Please specify hook of action to cancel.', 'paid-member-subscriptions' ) );
		}
		...
		if ( null === $result ) {
			$e = new \Exception( esc_html__( 'Unable to cancel scheduled action: check the logs.', 'paid-member-subscriptions' ) );
			$this->print_error( $e, false );
		}
	...
	protected function cancel_all( $hook, $callback_args, $group ) {
		if ( empty( $hook ) && empty( $group ) ) {
			\WP_CLI::error( esc_html__( 'Please specify hook and/or group of actions to cancel.', 'paid-member-subscriptions' ) );
		}
		 ...
		 */
		\WP_CLI::success( esc_html__( 'Request to cancel scheduled actions completed.', 'paid-member-subscriptions' ) );
	}
	...
	protected function print_success() {
		\WP_CLI::success( esc_html__( 'Scheduled action cancelled.', 'paid-member-subscriptions' ) );
	}
	...
	protected function print_error( \Exception $e, $multiple ) {
		\WP_CLI::error(
			sprintf(
				...
				__( 'There was an error cancelling the %1$s: %2$s', 'paid-member-subscriptions' ),
				$multiple ? esc_html__( 'scheduled actions', 'paid-member-subscriptions' ) : esc_html__( 'scheduled action', 'paid-member-subscriptions' ),
				$e->getMessage()
			)
		);
	}
