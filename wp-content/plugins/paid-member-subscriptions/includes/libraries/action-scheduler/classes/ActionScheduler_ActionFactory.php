...
class ActionScheduler_ActionFactory {
	...
	public function repeat( $action ) {
		...
		if ( is_null( $next ) || ! $schedule->is_recurring() ) {
			throw new InvalidArgumentException( esc_html__( 'Invalid action - must be a recurring action.', 'paid-member-subscriptions' ) );
		}
		...
		try {
			...
			error_log(
				sprintf(
					...
					__( 'Caught exception while enqueuing action "%1$s": %2$s', 'paid-member-subscriptions' ),
					$options['hook'],
					$e->getMessage()
				)
			);
