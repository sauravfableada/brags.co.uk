	...
	private function save_action_to_db( ActionScheduler_Action $action, ?DateTime $date = null, $unique = false ) {
		...
		try {
			...
			if ( is_wp_error( $action_id ) ) {
				...
				throw new \RuntimeException( $wpdb->last_error ? $wpdb->last_error : esc_html__( 'Database error.', 'paid-member-subscriptions' ) );
			}
			...
			throw new \RuntimeException( sprintf( esc_html__( 'Error saving action: %s', 'paid-member-subscriptions' ), esc_html( $e->getMessage() ) ), 0 );
		}
	}
...
",
	...
	protected function get_query_actions_sql( array $query, $select_or_count = 'select' ) {
		...
		if ( ! in_array( $select_or_count, array( 'select', 'count' ), true ) ) {
			throw new InvalidArgumentException( esc_html__( 'Invalid value for select or count parameter. Cannot query actions.', 'paid-member-subscriptions' ) );
		}
		...
		if ( ! is_null( $query['args'] ) ) {
			switch ( $query['partial_args_matching'] ) {
				case 'json':
					if ( ! $supports_json ) {
						throw new \RuntimeException( esc_html__( 'JSON partial matching not supported in your environment. Please check your MySQL/MariaDB version.', 'paid-member-subscriptions' ) );
					}
						...
						if ( ! $placeholder ) {
							throw new \RuntimeException(
								sprintf(
									...
									esc_html__( 'The value type for the JSON partial matching is not supported. Must be either integer, boolean, double or string. %s type provided.', 'paid-member-subscriptions' ),
                                    esc_html( $value_type )
								)
							);
						}
				...
				case 'off':
					...
					break;
				default:
					throw new \RuntimeException( esc_html__( 'Unknown partial args matching value.', 'paid-member-subscriptions' ) );
			}
		}
	...
	public function cancel_action( $action_id ) {
		...
		if ( false === $updated ) {
			...
			throw new \InvalidArgumentException( sprintf( esc_html__( 'Unidentified action %s: we were unable to cancel this action. It may may have been deleted by another process.', 'paid-member-subscriptions' ), esc_html( $action_id ) ) );
		}
	...
	public function delete_action( $action_id ) {
		...
		if ( empty( $deleted ) ) {
			...
			throw new \InvalidArgumentException( sprintf( esc_html__( 'Unidentified action %s: we were unable to delete this action. It may may have been deleted by another process.', 'paid-member-subscriptions' ), esc_html( $action_id ) ) );
		}
	...
	protected function get_date_gmt( $action_id ) {
		...
		if ( empty( $record ) ) {
			/* translators: %s is the action ID */
			throw new \InvalidArgumentException( sprintf( esc_html__( 'Unidentified action %s: we were unable to determine the date of this action. It may may have been deleted by another process.', 'paid-member-subscriptions' ), esc_html( $action_id ) ) );
		}
	...
	protected function claim_actions( $claim_id, $limit, ?DateTime $before_date = null, $hooks = array(), $group = '' ) {
		...
		if ( false === $rows_affected ) {
			$error = empty( $wpdb->last_error )
				? _x( 'unknown', 'database error', 'paid-member-subscriptions' )
				: $wpdb->last_error;
			throw new \RuntimeException(
				sprintf(
					/* translators: %s database error. */
					esc_html__( 'Unable to claim actions. Database error: %s.', 'paid-member-subscriptions' ),
                    esc_html( $error )
				)
			);
		}
		...
		if ( $row_updates < count( $action_ids ) ) {
			throw new RuntimeException(
				sprintf(
					...
					esc_html__( 'Unable to release actions from claim id %d.', 'paid-member-subscriptions' ),
					esc_html( $claim->get_id() )
				)
			);
		}
	}
	...
	public function mark_failure( $action_id ) {
		...
		if ( empty( $updated ) ) {
			/* translators: %s is the action ID */
			throw new \InvalidArgumentException( sprintf( esc_html__( 'Unidentified action %s: we were unable to mark this action as having failed. It may may have been deleted by another process.', 'paid-member-subscriptions' ), esc_html( $action_id ) ) );
		}
	}
		...
		if ( ! $status_updated ) {
			throw new Exception(
				sprintf(
					...
					esc_html__( 'Unable to update the status of action %1$d to %2$s.', 'paid-member-subscriptions' ),
					esc_html( $action_id ),
					esc_html( self::STATUS_RUNNING )
				)
			);
		}
	}
	...
	public function mark_complete( $action_id ) {
		...
		if ( empty( $updated ) ) {
			/* translators: %s is the action ID */
			throw new \InvalidArgumentException( sprintf( esc_html__( 'Unidentified action %s: we were unable to mark this action as having completed. It may may have been deleted by another process.', 'paid-member-subscriptions' ), esc_html( $action_id ) ) );
		}
	...
	public function get_status( $action_id ) {
		...
		if ( is_null( $status ) ) {
			throw new \InvalidArgumentException( esc_html__( 'Invalid action ID. No status found.', 'paid-member-subscriptions' ) );
		} elseif ( empty( $status ) ) {
			throw new \RuntimeException( esc_html__( 'Unknown status found for action.', 'paid-member-subscriptions' ) );
		} else {
			return $status;
		}
	}
