...
class ActionScheduler_wpPostStore extends ActionScheduler_Store {
	...
	protected function save_post_array( $post_array ) {
		...
		if ( is_wp_error( $post_id ) || empty( $post_id ) ) {
			throw new RuntimeException( esc_html__( 'Unable to save action.', 'paid-member-subscriptions' ) );
		}
		return $post_id;
	}
		...
		switch ( $post_status ) {
			...
			default:
				if ( ! array_key_exists( $post_status, $this->get_status_labels() ) ) {
					throw new InvalidArgumentException( sprintf( 'Invalid post status: "%s". No matching action status available.', esc_html( $post_status ) ) );
				}
		...
		switch ( $action_status ) {
			...
			default:
				if ( ! array_key_exists( $action_status, $this->get_status_labels() ) ) {
					throw new InvalidArgumentException( sprintf( 'Invalid action status: "%s".', esc_html( $action_status ) ) );
				}
	...
	protected function get_query_actions_sql( array $query, $select_or_count = 'select' ) {
		...
		if ( ! in_array( $select_or_count, array( 'select', 'count' ), true ) ) {
			throw new InvalidArgumentException( esc_html__( 'Invalid schedule. Cannot save action.', 'paid-member-subscriptions' ) );
		}
	...
	protected function claim_actions( $claim_id, $limit, ?DateTime $before_date = null, $hooks = array(), $group = '' ) {
		...
		if ( false === $rows_affected ) {
			throw new RuntimeException( esc_html__( 'Unable to claim actions. Database error.', 'paid-member-subscriptions' ) );
		}
	...
	protected function get_actions_by_group( $group, $limit, DateTime $date ) {
		...
		if ( ! term_exists( $group, self::GROUP_TAXONOMY ) ) {
			...
			throw new InvalidArgumentException( sprintf( esc_html__( 'The group "%s" does not exist.', 'paid-member-subscriptions' ), esc_html( $group ) ) );
		}
	...
	public function release_claim( ActionScheduler_ActionClaim $claim ) {
		...
		if ( false === $result ) {
			...
			throw new RuntimeException( sprintf( esc_html__( 'Unable to unlock claim %s. Database error.', 'paid-member-subscriptions' ), esc_html( $claim->get_id() ) ) );
		}
	}
	...
	public function unclaim_action( $action_id ) {
		...
		if ( false === $result ) {
			...
			throw new RuntimeException( sprintf( esc_html__( 'Unable to unlock claim on action %s. Database error.', 'paid-member-subscriptions' ), esc_html( $action_id ) ) );
		}
	}
	...
	public function mark_failure( $action_id ) {
		...
		if ( false === $result ) {
			/* translators: %s: action ID */
			throw new RuntimeException( sprintf( esc_html__( 'Unable to mark failure on action %s. Database error.', 'paid-member-subscriptions' ), esc_html( $action_id ) ) );
		}
	}
	...
	protected function validate_action( ActionScheduler_Action $action ) {
		try {
			...
			$message = sprintf( esc_html__( '%s Support for strings longer than this will be removed in a future version.', 'paid-member-subscriptions' ), $e->getMessage() );
