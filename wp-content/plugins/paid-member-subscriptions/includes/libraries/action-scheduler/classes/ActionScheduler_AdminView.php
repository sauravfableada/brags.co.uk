...
class ActionScheduler_AdminView extends ActionScheduler_AdminView_Deprecated {
	...
	public function register_system_status_tab( array $tabs ) {
		$tabs['action-scheduler'] = esc_html__('Scheduled Actions', 'paid-member-subscriptions' );
	...
	public function register_menu() {
		$hook_suffix = add_submenu_page(
			'tools.php',
			esc_html__( 'Scheduled Actions', 'paid-member-subscriptions' ),
			esc_html__( 'Scheduled Actions', 'paid-member-subscriptions' ),
			'manage_options',
			'action-scheduler',
	...
	public function add_help_tabs() {
		...
		$screen->add_help_tab(
			array(
				...
				'title'   => esc_html__('About', 'paid-member-subscriptions' ),
				'content' =>
					// translators: %s is the Action Scheduler version.
					'<h2>' . sprintf( esc_html__( 'About Action Scheduler %s', 'paid-member-subscriptions' ), $as_version ) . '</h2>' .
					'<p>' .
						esc_html__( 'Action Scheduler is a scalable, traceable job queue for background processing large sets of actions. Action Scheduler works by triggering an action hook to run at some time in the future. Scheduled actions can also be scheduled to run on a recurring schedule.', 'paid-member-subscriptions' ) .
					'</p>' .
					'<h3>' . esc_html__( 'Source', 'paid-member-subscriptions' ) . '</h3>' .
					'<p>' .
						esc_html__( 'Action Scheduler is currently being loaded from the following location. This can be useful when debugging, or if requested by the support team.', 'paid-member-subscriptions' ) .
					'</p>' .
					'<p>' . $as_source_markup . '</p>' .
					'<h3>' . esc_html__( 'WP CLI', 'paid-member-subscriptions' ) . '</h3>' .
					'<p>' .
						sprintf(
							/* translators: %1$s is WP CLI command (not translatable) */
							esc_html__( 'WP CLI commands are available: execute %1$s for a list of available commands.', 'paid-member-subscriptions' ),
		...
		$screen->add_help_tab(
			array(
				...
				'title'   => esc_html__('Columns', 'paid-member-subscriptions' ),
				'content' =>
					'<h2>' . esc_html__( 'Scheduled Action Columns', 'paid-member-subscriptions' ) . '</h2>' .
					'<ul>' .
					sprintf( '<li><strong>%1$s</strong>: %2$s</li>', esc_html__( 'Hook', 'paid-member-subscriptions' ), esc_html__( 'Name of the action hook that will be triggered.', 'paid-member-subscriptions' ) ) .
					sprintf( '<li><strong>%1$s</strong>: %2$s</li>', esc_html__( 'Status', 'paid-member-subscriptions' ), esc_html__( 'Action statuses are Pending, Complete, Canceled, Failed', 'paid-member-subscriptions' ) ) .
					sprintf( '<li><strong>%1$s</strong>: %2$s</li>', esc_html__( 'Arguments', 'paid-member-subscriptions' ), esc_html__( 'Optional data array passed to the action hook.', 'paid-member-subscriptions' ) ) .
					sprintf( '<li><strong>%1$s</strong>: %2$s</li>', esc_html__( 'Group', 'paid-member-subscriptions' ), esc_html__( 'Optional action group.', 'paid-member-subscriptions' ) ) .
					sprintf( '<li><strong>%1$s</strong>: %2$s</li>', esc_html__( 'Recurrence', 'paid-member-subscriptions' ), esc_html__( 'The action\'s schedule frequency.', 'paid-member-subscriptions' ) ) .
					sprintf( '<li><strong>%1$s</strong>: %2$s</li>', esc_html__( 'Scheduled', 'paid-member-subscriptions' ), esc_html__( 'The date/time the action is/was scheduled to run.', 'paid-member-subscriptions' ) ) .
					sprintf( '<li><strong>%1$s</strong>: %2$s</li>', esc_html__( 'Log', 'paid-member-subscriptions' ), esc_html__( 'Activity log for the action.', 'paid-member-subscriptions' ) ) .
					'</ul>',
			)
		);
	}
