...
function dokan_follow_store_is_following_store( $vendor_id, $follower_id ) {
    ...
    $following = $wpdb->get_var(
        $wpdb->prepare(
            "select 1 from {$wpdb->prefix}dokan_follow_store_followers
	...
	if ( ! empty( $wpdb->last_error ) ) {
		...
		return new WP_Error( 'follow_store_is_following_store_db_error', sprintf( __( 'Database Error: %s', 'dokan' ), $wpdb->last_error ) );
	}
