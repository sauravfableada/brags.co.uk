...
class ShipStationApiExport extends ShipStationApiRequest {
    ...
    public function request() {
        ...
        $export_statuses = get_user_meta( $this->seller->ID, 'shipstation_export_statuses', true );

        if ( empty( $export_statuses ) || ! is_array( $export_statuses ) ) {
            $export_statuses = array( 'wc-processing', 'wc-on-hold', 'wc-completed', 'wc-cancelled' );
        }
        ...
        foreach ( $order_ids as $order_id ) {
            ...
            if ( $meta_key = apply_filters( 'dokan_shipstation_export_custom_field_2', '' ) ) {
                $this->xml_append( $order_xml, 'CustomField2', apply_filters( 'dokan_shipstation_export_custom_field_2_value', get_post_meta( $order_id, $meta_key, true ), $order_id ) );
            }

            if ( $meta_key = apply_filters( 'dokan_shipstation_export_custom_field_3', '' ) ) {
                $this->xml_append( $order_xml, 'CustomField3', apply_filters( 'dokan_shipstation_export_custom_field_3_value', get_post_meta( $order_id, $meta_key, true ), $order_id ) );
            }
            ...
            if ( $order->get_total_discount() ) {
                ...
                $this->xml_append( $item_xml, 'Name', __( 'Total Discount', 'dokan' ) );
        ...
        $orders_xml->setAttribute( 'pages', ceil( $max_results / DOKAN_SHIPSTATION_EXPORT_LIMIT ) );
        $xml->appendChild( $orders_xml );
        echo $xml->saveXML();

        $this->log( sprintf( __( 'Exported %s orders', 'dokan' ), $exported ) );
    }
