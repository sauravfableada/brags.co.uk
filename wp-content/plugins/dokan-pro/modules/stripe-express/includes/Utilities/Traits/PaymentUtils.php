...
trait PaymentUtils {
    ...
    public function create_token_from_setup_intent( $setup_intent_id, $user_id ) {
        try {
            ...
            return dokan_pro()->module->stripe_express->payment_tokens->create_token_from_payment_method_for_user( $user_id, $stripe_payment_method );
        } catch ( Exception $e ) {
    ...
    public function maybe_show_description() {
		...
		// description for sendbox mode
        if ( $this->sandbox_mode ) {
            $description = ( ! empty( $description ) ? "$description<br>" : '' ) . $this->sandboxmode_description();
