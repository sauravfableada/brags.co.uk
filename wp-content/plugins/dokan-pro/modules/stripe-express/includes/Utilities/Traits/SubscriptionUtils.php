...
trait SubscriptionUtils {
    ...
    public function process_subscription_with_saved_payment_method( $order_id, $subscription_id, $can_retry = true ) {
        try {
            ...
            if ( $payment_needed && ! empty( $subscription->latest_invoice->confirmation_secret ) ) {
                ...
                $intent        = PaymentIntent::update( $intent_id, $intent_data );
            ...
            if ( 'requires_confirmation' === $intent->status ) {
                $intent = Payment::confirm_intent( $intent, $payment_method->id );
            }
            ...
            if ( ! empty( $intent->error ) && ! $is_authentication_required ) {
                if ( Helper::is_retryable_error( $intent->error ) ) {
                    if ( $can_retry ) {
                        ...
                        return $this->process_subscription_with_saved_payment_method( $order_id, $subscription_id, $intent->error );
                    } else {
                        $localized_message = __( 'Sorry, we are unable to process your payment at this time. Please retry later.', 'dokan' );
            ...
            if ( $is_authentication_required ) {
                ...
                // Return full URL for React: use as return_url in confirmPayment(); after 3DS Stripe redirects here.
            ...
            // Return thank you page redirect (include subscription_id for Order controller).
            $redirect_url = $this->get_return_url( $order );
            if ( ! empty( $subscription_id ) ) {
                $redirect_url = add_query_arg(
                    [
