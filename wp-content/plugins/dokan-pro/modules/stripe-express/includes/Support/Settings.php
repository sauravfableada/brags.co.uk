...
class Settings {
    ...
    /**
     * Retrieves Stripe publishable key based on mode
     *
     * @since 3.6.1
     *
     * @return string
    ...
    public static function get_publishable_key() {
        ...
        if ( self::is_sandbox_mode() ) {
            return ! empty( $settings['sandbox_publishable_key'] ) ? $settings['sandbox_publishable_key'] : '';
        }
    ...
    /**
     * Retrieves Stripe secret key based on mode
     *
     * @since 3.6.1
     *
     * @return string
    ...
    public static function get_secret_key() {
        ...
        if ( self::is_sandbox_mode() ) {
            return ! empty( $settings['sandbox_secret_key'] ) ? $settings['sandbox_secret_key'] : '';
        }
    ...
    /**
     * Retrieves webhook secret based on mode
     *
     * @since 3.6.1
     *
     * @return string
    ...
    public static function get_webhook_secret() {
        $settings = self::get();
        // If sandbox mode is enabled, return the sandbox webhook key
        if ( self::is_sandbox_mode() ) {
            return ! empty( $settings['sandbox_webhook_key'] ) ? $settings['sandbox_webhook_key'] : '';
        }
    ...
    public static function get_restricted_countries() {
        return array_filter( (array) self::get( 'restricted_countries' ) );
    }
    ...
    public static function is_cross_border_transfer_enabled() {
        return 'yes' === self::get( 'cross_border_transfer' );
    }
    ...
    public static function is_disconnect_vendors_enabled() {
        return 'yes' === self::get( 'disconnect_restricted_vendors' );
    }
    ...
    public static function is_disconnect_connected_vendors_enabled() {
        return 'yes' === self::get( 'disconnect_connected_vendors' );
    }

    /**
     * Retrieves payment method configuration
     *
     * @since 4.3.0
     *
     * @return string
     */
    public static function get_payment_method_configuration(): string {
        if ( self::is_test_mode() ) {
            return self::get( 'test_payment_method_configuration' );
        }
        return self::get( 'payment_method_configuration' );
    }
}
