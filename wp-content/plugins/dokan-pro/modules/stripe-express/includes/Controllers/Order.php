...
class Order {
    ...
    public function maybe_process_order_redirect() {
        if ( Helper::is_payment_methods_page() ) {
            ...
            try {
                ...
                $result = $customer->attach_payment_method( $payment_method_id );
                if ( is_wp_error( $result ) ) {
                    throw new DokanException( 'dokan-stripe-express-payment-method-error', $result->get_error_message() );
                }
                ...
                do_action( 'dokan_stripe_express_attach_payment_method', $user_id, $payment_method_object );
        ...
        if ( ! empty( $_GET['payment_intent_client_secret'] ) ) {
            ...
            OrderMeta::save( $order );
        } elseif ( ! empty( $_GET['subscription_id'] ) && Subscription::is_recurring_vendor_subscription_order( $order ) ) {
            /*
             * No-3DS flow (Block/classic): Stripe confirmed in-page, no redirect params.
             * Get intent from order meta (stored in process_subscription) or subscription.
             */
            ...
            if ( empty( $intent_id ) ) {
                ...
                if ( $subscription && ! is_wp_error( $subscription ) ) {
                    ...
                    $secret        = $invoice ? ( $invoice->confirmation_secret ?? null ) : null;
                    $client_secret = $secret && isset( $secret->client_secret ) ? $secret->client_secret : '';

                    if ( ! empty( $client_secret ) ) {
                        $intent_id = Helper::get_intent_id_by_client_secret( $client_secret );
                    }
                }
            }
            $subscription_id = sanitize_text_field( wp_unslash( $_GET['subscription_id'] ) );
    ...
    public function process_order_redirect( $order, $intent_id, $save_payment_method ) {
        ...
        try {
            /*
             * To avoid the conflict when both webhook and redirect tries to process the payment at the same time,
             * we disable the locking mechanism temporarily.
             */
            add_filter( 'dokan_stripe_express_should_lock_at_run_time', '__return_false' );
            Payment::process_confirmed_intent( $order, $intent_id, $save_payment_method );
            remove_filter( 'dokan_stripe_express_should_lock_at_run_time', '__return_false' );
        } catch ( Exception $e ) {
    ...
    public function handle_payment_disbursement( $order_id, $old_status, $new_status ) {
        ...
        // Route disbursement based on an order type.
        if ( $order->get_parent_id() ) {
            Payment::disburse_suborder( $order ); // Suborder completed — disburse only this vendor's transfer
        } else {
            Payment::disburse( $order ); // Parent order or single-vendor order — disburse all remaining
        }
    }
