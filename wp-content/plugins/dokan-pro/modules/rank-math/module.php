...
class Module {
    ...
    private function hooks() {
        /*
         * Load SEO content after main contents on the edit product page.
         ...
         */
        add_action( 'dokan_product_edit_after_main', [ $this, 'load_product_seo_content' ], 6, 2 );
        ...
        add_action( 'wp_footer', [ $this, 'load_hidden_product_sections' ], 99 );
        ...
        // Register the SEO section in the new (v5.0.0) Product Form Manager.
    ...
    /**
     * Emits the off-screen `#rank-math-metabox-wrapper` so Rank Math's
     * classic.js mounts into a real, server-rendered node — same flow
     * legacy uses. The new (React) editor adopts the mounted node into
     * its card slot afterwards instead of trying to remount it itself.
     *
     * @since 3.7.13
     *
     * @return void
    ...
    public function load_hidden_product_sections() {
        // `wp_footer` is site-wide; restrict it to the new editor. The dashboard hook is already scoped.
        if ( 'wp_footer' === current_filter() && ! ProductEditorFields::is_new_dashboard_request() ) {
            return;
        }
    ...
    public function load_product_scripts_for_blocks() {
        $cmb2_bootstrap = $this->get_cmb2_bootstrap_class();
    ...
    /**
     * Resolves the versioned CMB2 bootstrap class Rank Math bundles.
     *
     * Rank Math names the bootstrap after its bundled CMB2 version (2.11.0 ->
     * CMB2_Bootstrap_2110, 2.12.0 -> CMB2_Bootstrap_2120, ...), so we rebuild
     * the name from the active CMB2_VERSION instead of pinning one release — and
     * probe with autoload off so a stale name can't trip CMB2's include_once warning.
     *
     * @since 5.0.4
     *
     * @return string|null Bootstrap class name, or null when CMB2 is absent.
     */
    public function get_cmb2_bootstrap_class(): ?string {
        if ( ! defined( 'CMB2_VERSION' ) ) {
            return null;
        }

        $class = 'CMB2_Bootstrap_' . str_replace( '.', '', CMB2_VERSION );
