...
use WeDevs\DokanPro\Modules\ProductSubscription\Module as SubscriptionModule;
use WeDevs\DokanPro\Products as DokanProProducts;
...
class Module {
    ...
    public function __construct() {
        ...
        // Prevent coupon validation for booking person product.
        add_filter( 'dokan_coupon_should_validate', [ $this, 'should_validate_coupon' ], 10, 4 );
        add_action( 'woocommerce_after_checkout_validation', [ $this, 'maybe_cancelled_unpaid_booking_from_checkout' ] );
        add_filter( 'dokan_settings_fields', [ $this, 'add_booking_setting_in_delivery_time' ], 22 );
        add_filter( 'dokan_ignore_delivery_time_for_product', [ $this, 'ignore_delivery_time_for_booking_product' ], 10, 2 );
    }

    /**
     * Register REST API controllers for booking module.
     *
     * @since 3.14.0
     *
    ...
    public function rest_api_class_map( $class_map ) {
        $class_map[ DOKAN_WC_BOOKING_DIR . '/includes/REST/BookingController.php' ]         = \WeDevs\DokanPro\Modules\Booking\REST\BookingController::class;
        $class_map[ DOKAN_WC_BOOKING_DIR . '/includes/REST/BookingProductController.php' ]  = \WeDevs\DokanPro\Modules\Booking\REST\BookingProductController::class;
        $class_map[ DOKAN_WC_BOOKING_DIR . '/includes/REST/BookingResourceController.php' ] = \WeDevs\DokanPro\Modules\Booking\REST\BookingResourceController::class;
        $class_map[ DOKAN_WC_BOOKING_DIR . '/includes/REST/BookingCalendarController.php' ] = \WeDevs\DokanPro\Modules\Booking\REST\BookingCalendarController::class;

        return $class_map;
    }

    /**
     * Filter WooCommerce REST product query args for booking vendor list.
     *
     ...
     * @return array
     */
    public function filter_booking_products_query_args( $args, $request ) {
        if ( ! $request instanceof \WP_REST_Request ) {
            return $args;
        }

        $route = $request->get_route();
        if ( ! in_array( $route, [ '/wc/v3/products', '/wc/v2/products' ], true ) ) {
            return $args;
        }

        if ( ! $request->get_param( 'booking_vendor' ) ) {
            return $args;
        }
        ...
        if ( $date ) {
            $args['m'] = sanitize_text_field( $date );
        }

        $product_cat = absint( $request->get_param( 'product_cat' ) );
        if ( $product_cat > 0 ) {
            $args['tax_query'][] = [
        ...
        $product_brand = absint( $request->get_param( 'product_brand' ) );
        if ( $product_brand > 0 ) {
            $args['tax_query'][] = [
        ...
        $filter_by = sanitize_text_field( $request->get_param( 'filter_by_other' ) );
        if ( $filter_by ) {
            switch ( $filter_by ) {
                case 'featured':
                    $args['tax_query'][] = [
    ...
    **/
    ...
    public function enqueue_scripts() {
        ...
        // Enqueue vendor booking React script on all seller dashboard pages
        // (React router handles route matching via hash-based routing in latest layout)
        if ( ! is_admin() && dokan_is_seller_dashboard() ) {
            wp_enqueue_script( 'dokan-vendor-booking' );
            wp_enqueue_style( 'dokan_wc_booking-styles' );
            $vendor_id         = dokan_get_current_user_id();
            $is_seller_enabled = dokan_is_seller_enabled( $vendor_id );
            $can_add_product   = true;
            $remaining         = -1;
            $subscription_data = [
                'remaining_products' => true,
                'can_post_product'   => true,
                'subscription_url'   => '',
            ];

            if ( $is_seller_enabled && class_exists( SubscriptionModule::class ) ) {
                $subscription_module = dokan_pro()->module->product_subscription ?? null;
                if ( $subscription_module instanceof SubscriptionModule ) {
                    $subscription_data = $subscription_module->build_subscription_data( $vendor_id );
                    $remaining_raw     = $subscription_data['remaining_products'];
                    $remaining         = true === $remaining_raw ? -1 : (int) $remaining_raw;
                    $can_add_product   = (bool) $subscription_data['can_post_product'];
                }
            }

            $can_duplicate_product = class_exists( DokanProProducts::class )
                ? DokanProProducts::can_vendor_duplicate_product()
                : false;
            ...
            wp_localize_script(
                ...
                'dokanBooking',
                [
                    'bookingUrl'             => dokan_get_navigation_url( 'booking' ),
                    'isGlobalAddonRmaActive' => \Dokan_WC_Booking_Helper::is_global_addon_rma_active(),
                    'isSellerEnabled'        => $is_seller_enabled,
                    'canAddProduct'          => $can_add_product,
                    'remainingProducts'      => $remaining,
                    'subscriptionUrl'        => $subscription_data['subscription_url'],
                    'subscription'           => $subscription_data,
                    'canDuplicateProduct'    => $can_duplicate_product,
                ]
            );
        }
        ...
        if ( ! is_admin() && isset( $wp->query_vars['booking'] ) ) {
            ...
            wp_enqueue_script( 'dokan-jquery-tiptip' );
    ...
    public function register_scripts() {
        ...
        // Register vendor booking dashboard React script
        $booking_asset_file = DOKAN_WC_BOOKING_DIR . '/assets/js/vendor-booking.asset.php';
        if ( file_exists( $booking_asset_file ) ) {
            $booking_asset = include $booking_asset_file;
            wp_register_script(
                'dokan-vendor-booking',
                DOKAN_WC_BOOKING_PLUGIN_ASSET . '/js/vendor-booking.js',
                array_merge( [ 'dokan-react-components', 'dokan-utilities' ], $booking_asset['dependencies'] ),
                $booking_asset['version'],
                true
            );
            wp_set_script_translations( 'dokan-vendor-booking', 'dokan' );
        }
        // @codingStandardsIgnoreLine
    ...
    public function init_hooks() {
        ...
        add_filter( 'woocommerce_rest_product_object_query', [ $this, 'filter_booking_products_query_args' ], 10, 2 );
    ...
    public function add_booking_page( $urls ) {
        $menu = [
            'title'       => esc_html__( 'Booking', 'dokan' ),
    ...
    /**
     * Validate coupon for booking person product.
     *
     * @since 4.1.2
     *
     ...
     * @return false|mixed
     */
    public function should_validate_coupon( $should_validate, $coupon, $item, \WC_Discounts $discounts ) {
        if ( ! $should_validate ) {
            return $should_validate;
        }

        // Check if the coupon is for booking person product.
        $discount_object = $discounts->get_object();
        if ( ! is_a( $discount_object, 'WC_Order' ) ) {
            return $should_validate;
        }

        // Check if the coupon is for booking person product.
        if ( did_filter( 'woocommerce_order_recalculate_coupons_coupon_object' ) && ! did_action( 'woocommerce_order_applied_coupon' ) ) {
            return false;
        }
    ...
    /**
     * Cancels any "unpaid" bookings linked to the current checkout session.
     *
     * When a payment attempt fails, WooCommerce may leave bookings in the
     * "unpaid" state under the session's `order_awaiting_payment` order.
     * These unpaid bookings continue blocking the selected time slot and
     * prevent new checkout attempts from creating a fresh booking.
     *
     * This method checks the existing session order and cancels all
     * associated bookings still marked as "unpaid", allowing the customer
     * to retry the checkout without availability errors.
     *
     * @see https://github.com/woocommerce/woocommerce/blob/f5abdd39bef881b1481d8c613a7744fe194f2ed0/plugins/woocommerce/includes/class-wc-checkout.php#L1077
     *
     * @since 4.2.1
     *
     * @return void
     */
    public function maybe_cancelled_unpaid_booking_from_checkout() {
        $order_id = absint( WC()->session->get( 'order_awaiting_payment', 0 ) ) ?? absint( WC()->session->get( 'store_api_draft_order', 0 ) );
        if ( ! $order_id ) {
            return;
        }
        ...
        foreach ( $booking_ids as $booking_id ) {
            $booking = get_wc_booking( $booking_id );
            if ( $booking && $booking->has_status( 'unpaid' ) ) {
                $booking->update_status( 'cancelled' );
            }
        }
    }
    /**
     * Add delivery time setting for booking product in dokan delivery time settings.
     *
     * @since 4.2.2
     *
     * @param array $fields
     ...
     * @return array mixed
     */
    public function add_booking_setting_in_delivery_time( $fields ) {
        if ( isset( $fields['dokan_delivery_time'] ) ) {
            $new_item = [
                'enable_for_booking_product' => [
                    'name'    => 'enable_for_booking_product',
                    'label'   => __( 'Enable Delivery Time for Booking Products', 'dokan' ),
                    'desc'    => __( 'Control how delivery time will be shown in the checkout with booking product in cart.', 'dokan' ),
                    'default' => 'off',
                    'type'    => 'switcher',
                ],
            ];

            // Find the position of 'select_required' in the array
            $position = array_search( 'select_required', array_keys( $fields['dokan_delivery_time'] ) );

            // Insert the item after 'select_required'
            if ( $position !== false ) {
                ++$position;
                $fields['dokan_delivery_time'] = array_merge(
                    array_slice( $fields['dokan_delivery_time'], 0, $position ),
                    $new_item,
                    array_slice( $fields['dokan_delivery_time'], $position )
                );
            }
        }
    ...
    /**
     * Ignore delivery time for booking product based on settings.
     *
     * @since 4.2.2
     *
     * @param boolean     $delivery_time
     * @param \WC_Product $product
     *
     * @return boolean
     */
    public function ignore_delivery_time_for_booking_product( $delivery_time, $product ) {
        if ( ! $product || 'booking' !== $product->get_type() ) {
            return $delivery_time;
        }

        return 'off' === dokan_get_option( 'enable_for_booking_product', 'dokan_delivery_time', 'off' );
    }
}
