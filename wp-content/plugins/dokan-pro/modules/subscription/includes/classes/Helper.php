...
class Helper {
    ...
    public static function get_subscription_period_interval_strings( $interval = '' ) {
        $intervals = [ 1 => _x( 'every', 'period interval (eg "$10 _every_ 2 weeks")', 'dokan' ) ];
    ...
    public static function get_subscription_ranges( $subscription_period = '' ) {
        static $dokan_subscription_locale_ranges = [];
    ...
    public static function is_vendor_subscription_enabled() {
        return 'on' === dokan_get_option( 'enable_pricing', 'dokan_product_subscription' );
    }
    ...
    public static function is_vendor_subscription_order( $order ) {
         ...
         * @since 3.3.7
         */
        if ( 'yes' === $order->get_meta( '_dokan_vendor_subscription_order' ) || $order->meta_exists( '_pack_validity' ) ) {
            return true;
        }
    ...
    public static function delete_subscription_pack( $customer_id, $order_id ) {
        ...
        $settings = new \WeDevs\Dokan\Commission\Settings\Vendor( $customer_id );
        $settings->delete();
    ...
    public static function alert_before_two_days( $user_id ) {
        ...
        $alert_days = (int) dokan_get_option( 'no_of_days_before_mail', 'dokan_product_subscription' );
        if ( $alert_days === 0 ) {
            $alert_days = 2;
        }

        $current_date = dokan_current_datetime();
        $alert_date   = dokan_current_datetime()->modify( $pack_end_date_str )->modify( "- $alert_days days" );
    ...
    /**
     * Get current period end date ( Subscription end date not pack end date )
     */
    public static function get_current_period_end_date( $vendor_id = null ) {
        return get_user_meta( $vendor_id, '_dokan_subscription_current_period_end', true );
    }

    /**
     * Update current period end date ( Subscription end date not pack end date )
     */
    public static function update_current_period_end_date( $vendor_id = null, $subscription_end_date = '' ) {
        update_user_meta( $vendor_id, '_dokan_subscription_current_period_end', $subscription_end_date );
    }
    ...
    public static function create_renewal_order( $parent_order, $order_total = null ) {
        ...
        if ( ! $parent_order || ! $parent_order->get_customer_id() ) {
            return new \WP_Error( 'invalid_order', __( 'Invalid order or customer information not found.', 'dokan' ) );
        }
        ...
        try {
            ...
            foreach ( $parent_order->get_items( [ 'line_item', 'fee' ] ) as $item ) {
                 ...
                 */
                $meta_to_exclude = apply_filters( 'dokan_subscription_renewal_exclude_meta', $excluded_meta_keys, $item, $new_order, $parent_order );
    ...
    public static function filter_products_by_filter_by_other_helper( $args, $filter ) {
        ...
        return apply_filters( 'dokan_pro_vendor_product_filter_by_args', $args, $filter );
    }
    ...
    public static function set_default_tax_query( $args ) {
        ...
        if ( isset( $args['include_hidden_products'] ) && dokan_string_to_bool( $args['include_hidden_products'] ) ) {
            return $args;
        }
    ...
    public static function get_product_status_changer_bg_class() {
        ...
        return dokan()->bg_process->subscription_product_status_changer;
    }
