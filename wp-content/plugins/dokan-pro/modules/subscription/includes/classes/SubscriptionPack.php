...
class SubscriptionPack extends VendorSubscription {
    ...
    public function get_product_pack_end_date() {
        ...
        $pack_validity          = (int) ( $this->get_pack_valid_days() ); //_pack_validity
    ...
    /**
     * Get subscription end date ( Subscription end date not pack end date )
     * but for non-recurring pack its same as pack end date
     *
     * @return string|null
     */
    public function subscription_end_date() {
        if ( $this->is_recurring() ) {
            $subscription_end_date = Helper::get_current_period_end_date( $this->get_vendor() );
            // if subscription end date is empty, calculate it manually
    ...
    public function get_pack_valid_days() {
        $validity = get_post_meta( $this->get_id(), '_pack_validity', true );

        if ( is_numeric( $validity ) && (int) $validity === 0 ) {
            update_post_meta( $this->get_id(), '_pack_validity', -1 );
            return -1;
        }
        return $validity;
    }
