...
use WeDevs\DokanPro\Modules\ProductSubscription\Emails\SubscriptionExpiryAlertVendor;
...
class Module {
    ...
    public function __construct() {
        require_once __DIR__ . '/includes/classes/class-dps-product-pack.php';
    ...
    public function init_hooks() {
        ...
        // Pass subscription limits to the React product listing (initial page-load values).
        add_filter( 'dokan_react_frontend_localized_args', [ $this, 'add_subscription_to_product_listing' ] );

        // Pass subscription limits to the React auction listing (initial page-load values).
        add_filter( 'dokan_react_frontend_localized_args', [ $this, 'add_subscription_to_auction_listing' ] );

        // Pass subscription limits to the React vendor dashboard (initial page-load values).
        add_filter( 'dokan_product_listing_summary_data', [ $this, 'add_subscription_to_summary' ], 10, 2 );
        // Filter to inject a subscription order type for analytics.
        add_filter( 'dokan_get_order_type', [ $this, 'get_subscription_order_type' ], 10, 3 );

        // Auction listing summary — keeps subscription info live after delete/publish actions.
        add_filter( 'dokan_auction_listing_summary_data', [ $this, 'add_subscription_to_summary' ], 10, 2 );

        // Add subscription revenue to the admin report earnings summary.
        add_filter( 'dokan_admin_report_earnings_summary', [ $this, 'add_subscription_revenue_to_summary' ], 10, 2 );
    }
    ...
    public function show_custom_subscription_info() {
        ...
        if ( dokan_is_seller_enabled( $vendor_id ) ) {
            ...
            if ( (int) $remaining_product === 0 || ! self::can_post_product() ) {
     ...
     * @return integer
     */
    public function get_number_of_product_by_seller( $user_id ) {
        global $wpdb;
     ...
     */
    public function display_product_pack() {
    ...
    public function process_subscription_ordermeta( $order_id ) {
        ...
        $sub_pack      = dokan()->subscription->get( $product->get_id() );
        $pack_validity = $sub_pack->get_pack_valid_days();
        $pack_validity = $pack_validity < 0 || ! is_numeric( $pack_validity )
            ? 'unlimited'
    ...
    public function process_order_pack_product( $order_id, $old_status, $new_status ) {
        ...
        if ( empty( $customer_id ) ) {
            ...
            if ( ! $user ) {
                ...
                $user_data['enabled']       = 'automatically' === dokan_get_container()->get( \WeDevs\Dokan\Utilities\AdminSettings::class )->get_new_seller_enable_selling_status();
        ...
        if ( $order->meta_exists( '_pack_validity' ) ) {
            ...
            $sub_pack              = dokan()->subscription->get( $product->get_id() );
            $product_pack_validity = $sub_pack->get_pack_valid_days();

            $pack_validity = $product_pack_validity < 0 || ! is_numeric( $product_pack_validity )
                ? 'unlimited'
    ...
    public static function rest_api_class_map( $classes ) {
        $class = [
            __DIR__ . '/api/class-subscription-controller.php'                 => 'Dokan_REST_Subscription_Controller',
            __DIR__ . '/api/class-vendor-subscription-controller.php'          => 'Dokan_REST_Vendor_Subscription_Controller',
            __DIR__ . '/api/class-vendor-subscription-packages-controller.php' => 'Dokan_REST_Vendor_Subscription_Packages_Controller',
            __DIR__ . '/api/class-vendor-subscription-orders-controller.php'   => 'Dokan_REST_Vendor_Subscription_Orders_Controller',
        ];
    ...
    public static function register_email_action( $actions ) {
        ...
        $actions[] = 'dokan_subscription_expiry_alert';
    ...
    public static function register_email_templates( $template ) {
        ...
        $template[] = 'dokan-subscription-expiry-alert-vendor.php';
    ...
    public static function load_subscription_class( $classes ) {
        ...
        if ( is_admin() && Helper::is_vendor_subscription_enabled() ) {
            $classes['admin_dashboard'] = ( new AdminDashboard() )->register_hooks();
        }
    ...
    public function count_filter( $arr, $count ) {
        return array_filter(
            $arr, function ( $item, $key ) use ( $count ) {
				return $key <= $count - 1;
			}, ARRAY_FILTER_USE_BOTH
        );
    }
    ...
    public function update_vendor_subscription_data( $vendor_id, $data ) {
        ...
        if ( ! empty( $subscription_id ) && $subscription_id !== $previous_subscription_id ) {
            ...
            $sub_pack      = dokan()->subscription->get( $subscription_id );
            $pack_validity = $sub_pack->get_pack_valid_days();
    ...
    /**
     * Build the subscription data array for a vendor.
     *
     * Public API — callers (incl. cross-module consumers like booking) rely on
     * the returned keys and shape. Changes should preserve backwards
     * compatibility.
     *
     * @since 5.0.0 Made public for cross-module reuse (booking).
     *
     * @param int  $vendor_id Vendor user ID.
     * @param bool $new_url  
     *
     * @return array{remaining_products: true|int, can_post_product: bool, subscription_url: string}
     */
    public function build_subscription_data( int $vendor_id, bool $new_url = true ): array {
        $remaining_product = Helper::get_vendor_remaining_products( $vendor_id );
        $can_post          = self::can_post_product();

        if ( self::is_dokan_plugin() ) {
            $subscription_url = esc_url_raw( dokan_get_navigation_url( 'subscription', $new_url ) );
        } else {
            $page_id          = dokan_get_option( 'subscription_pack', 'dokan_product_subscription' );
            $subscription_url = esc_url_raw( (string) get_permalink( $page_id ) );
        }

        return [
            'remaining_products' => $remaining_product,
            'can_post_product'   => $can_post,
            'subscription_url'   => $subscription_url,
        ];
    }

    /**
     * Pass vendor subscription limits to the React product listing via dokanFrontend.
     *
     * @since 5.0.0
     *
     ...
     * @return array
     */
    public function add_subscription_to_product_listing( array $args ): array {
        if ( ! is_user_logged_in() ) {
            return $args;
        }
        ...
        if ( ! isset( $args['product_listing'] ) ) {
            $args['product_listing'] = [];
        }

        $args['product_listing']['subscription'] = $this->build_subscription_data( $vendor_id );

        return $args;
    }

    /**
     * Pass subscription limits to the React auction listing.
     *
     * @since 5.0.0
     *
     * @param array $args Existing localized args.
     ...
     * @return array
     */
    public function add_subscription_to_auction_listing( array $args ): array {
        if ( ! is_user_logged_in() ) {
            return $args;
        }
        ...
        if ( ! isset( $args['auction_listing'] ) ) {
            $args['auction_listing'] = [];
        }

        $args['auction_listing']['subscription'] = $this->build_subscription_data( $vendor_id );

        return $args;
    }

    /**
     * Append fresh subscription info to the product listing summary REST response.
     *
     * @since 5.0.0
     *
     * @param array $data      Summary data from /dokan/v1/products/summary.
     * @param int   $vendor_id Current vendor ID.
     *
     * @return array
     */
    public function add_subscription_to_summary( array $data, int $vendor_id ): array {
        if ( ! is_user_logged_in() || ! dokan_is_seller_enabled( $vendor_id ) ) {
            return $data;
        }

        $data['subscription_remaining'] = [
            'remaining_products' => Helper::get_vendor_remaining_products( $vendor_id ),
            'can_post_product'   => self::can_post_product(),
        ];
    ...
    /**
     * Get the subscription order type for analytics.
     *
     * This method is hooked to 'dokan_get_order_type' filter to inject
     * the subscription order type when the order is a vendor subscription order.
     *
     * @since 5.0.0
     *
     ...
     *
     * @return int|null The order type constant, or null if not a subscription order.
     */
    public function get_subscription_order_type( $order_type, $order_to_check, $is_refund ) {
        // If an order type is already determined by another module, return it.
        if ( $order_type !== null ) {
            return $order_type;
        }

        // Check if the order is a vendor subscription order.
        if ( Helper::is_vendor_subscription_order( $order_to_check ) ) {
            return $is_refund
    ...
    /**
     * Add a subscription revenue field to the admin report earnings summary.
     *
     * @since 5.0.0
     *
     * @param array $summary      The filtered summary data.
     * @param array $full_summary The full summary data from the store.
     *
     * @return array
     */
    public function add_subscription_revenue_to_summary( array $summary, array $full_summary ): array {
        if ( isset( $full_summary['subscription_revenue'] ) ) {
            $summary['subscription_revenue'] = $full_summary['subscription_revenue'];
        }
