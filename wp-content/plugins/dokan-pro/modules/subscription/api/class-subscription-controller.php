...
class Dokan_REST_Subscription_Controller extends DokanRESTController {
    ...
    public function register_routes() {
        ...
        register_rest_route(
            $this->namespace, '/' . $this->base . '/save-commission', [
                [
                    'methods'             => WP_REST_Server::EDITABLE,
                    'callback'            => [ $this, 'save_commission' ],
                    'permission_callback' => [ $this, 'check_permission' ],
                    'args'                => [
                        'product_id'      => [
                            'type'              => 'integer',
                            'description'       => __( 'Dokan subscription product id.', 'dokan' ),
                            'sanitize_callback' => 'absint',
                            'validate_callback' => 'rest_validate_request_arg',
                            'required'          => true,
                        ],
                        'commission_type' => [
                            'type'              => 'string',
                            'description'       => __( 'Commission type', 'dokan' ),
                            'required'          => true,
                            'default'           => 'fixed',
                        ...
                        'commission'      => [
                            ...
                            'properties'  => [
                                'fixed'          => [
                                    'type'        => 'object',
                                    'description' => __( 'Fixed commission values', 'dokan' ),
                                    'properties'  => [
                                        'flat'       => array(
                                            'type'        => array( 'number', 'string', 'null' ),
                                            'description' => __( 'Flat commission value', 'dokan' ),
                                            'pattern'     => '^$|^[0-9]+$|^[0-9]+\.[0-9]+$',
                                            'default'     => '',
                                        ),
                                        'percentage' => [
                                            'type'        => 'string',
                                            'description' => __( 'Percentage commission value', 'dokan' ),
                                            'default'     => '',
                                            'pattern'     => '^$|^[0-9]+$|^[0-9]+\.[0-9]+$',
                                        ],
                                    ],
                                ...
                                'category_based' => [
                                    'type'        => 'object',
                                    'description' => __( 'Category based commission values', 'dokan' ),
                                    'properties'  => [
                                        'all'   => [
                                            'type'        => 'object',
                                            'description' => __( 'All commission values', 'dokan' ),
                                            'properties'  => [
                                                'flat'       => [
                                                    'type'        => 'string',
                                                    'description' => __( 'Flat commission value', 'dokan' ),
                                        ...
                                        'items' => [
                                            'type'        => 'array',
                                            'description' => __( 'Items commission values', 'dokan' ),
                                            'items'       => [
                                                'type'       => 'object',
                                                'properties' => [
                                                    'category_id' => [
                        ...
                        'reset_sub_category' => [
                            'type'              => 'string',
                            'description'       => __( 'Category commission reset setting.', 'dokan' ),
                            'required'          => false,
                            'default'           => true,
    ...
    public function update_subscription( $request ) {
        ...
        $seller         = dokan()->vendor->get( $user_id );
     ...
     *
     * @param \WeDevs\Dokan\Vendor\Vendor $seller
    ...
    public function prepare_item_for_response( $seller, $request ) {
        ...
        if ( ! $subscription ) {
            return new WP_Error( 'no_subscription', __( 'No subscription is found.', 'dokan' ), [ 'status' => 404 ] );
        }
        ...
        $data = [
            ...
            'no_of_allowed_products'   => '-1' !== $subscription->get_number_of_products() ? $subscription->get_number_of_products() : __( 'unlimited', 'dokan' ),
            ...
            'end_date'              => ( 'unlimited' === $end_date || '-1' === $end_date ) ? __( 'Unlimited', 'dokan' ) : dokan_format_date( $end_date ),
            'subscription_end_date' => ( 'unlimited' === $subscription_end_date || '-1' === $subscription_end_date ) ? __( 'Unlimited', 'dokan' ) : dokan_format_date( $subscription_end_date ),
        ...
        return apply_filters( 'dokan_rest_prepare_subscription', $data, new WP_User( $seller->get_id() ), $request, $seller );
    }
    ...
    public function save_commission( $request ) {
        ...
        $reset_sub_category = $request->get_param( 'reset_sub_category' );

        $product = dokan()->product->get( $product_id );

        $product->update_meta_data( '_subscription_product_admin_commission_type', $commission_type );


        if ( isset( $commission['fixed'] ) ) {
            $flat       = isset( $commission['fixed']['flat'] ) ? $commission['fixed']['flat'] : '';
            $percentage = isset( $commission['fixed']['percentage'] ) ? $commission['fixed']['percentage'] : '';

            $product->update_meta_data( '_subscription_product_admin_commission', $percentage );
            $product->update_meta_data( '_subscription_product_admin_additional_fee', $flat );
        }

        if ( isset( $commission['category_based'] ) ) {
            $category_based = $commission['category_based'];
            $items          = [];

            if ( isset( $category_based['items'] ) ) {
                foreach ( $category_based['items'] as $item ) {
            ...
            $category_based['items'] = $items;

            $product->update_meta_data( '_subscription_product_admin_category_based_commission', $category_based );
            $product->update_meta_data( 'reset_sub_category', $reset_sub_category );
        }
