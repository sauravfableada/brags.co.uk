...
class Dokan_Product_Addon_Vendor_Product {
    ...
    public function __construct() {
        ...
        add_action( 'dokan_new_product_added', [ $this, 'save_addon_options' ], 35, 2 );
    ...
    public function add_addons_section( $post, $post_id ) {
        dokan_get_template_part(
            'product-addon/html-product-addons', '', array(
    ...
    public function save_addon_options( $post_id, $postdata ) {
        ...
        // New (v5) Product Form Manager already persisted addons via the
        // schema payload resolver — bailing here prevents this legacy parser
        // from re-reading an empty $_POST and wiping the meta. Keyed by product
        // id so it only suppresses the product the new editor actually saved.
        if ( ! empty( $GLOBALS[ \WeDevs\DokanPro\Modules\ProductAddon\ProductEditorFields::SAVED_FLAG_KEY ][ $post_id ] ) ) {
            return;
        }
