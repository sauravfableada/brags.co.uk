...
<div class="woocommerce dokan-pa-all-addons">
	<?php $settings_url = trailingslashit( dokan_get_navigation_url( 'settings' ) ) . 'product-addon/'; ?>
	<a class="dokan-btn dokan-btn-theme dokan-pa-create-btn" href="<?php echo add_query_arg( 'add', true, $settings_url ); ?>" class="add-new-h2">
		<?php esc_html_e( 'Create New addon', 'dokan' ); ?>
	</a>
		...
		<tbody id="the-list">
			...
			if ( $global_addons ) {
				foreach ( $global_addons as $global_addon ) {
					?>
					<tr>
						<td><a href="<?php echo esc_url( add_query_arg( 'edit', $global_addon['id'], $settings_url ) ); ?>"><?php echo $global_addon['name']; ?></a>
							<div class="row-actions">
								<span class="edit"><a href="<?php echo esc_url( add_query_arg( 'edit', $global_addon['id'], $settings_url ) ); ?>"><?php esc_html_e( 'Edit', 'dokan' ); ?></a> | </span>
								<span class="delete"><a class="delete" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'delete', $global_addon['id'], $settings_url ), 'delete_addon' ) ); ?>"><?php esc_html_e( 'Delete', 'dokan' ); ?></a></span>
							</div>
						</td>
