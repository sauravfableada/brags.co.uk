...
trait RMACommon {
     ...
     * @return array
     */
    public function transform_request_conversation( array $request ): array {
     ...
     */
    public function transform_warranty_requests( array $request ) {
        ...
        if ( ! $order ) {
            return $this->transform_deleted_order_warranty_request( $request );
        }

        // Original logic for existing orders
        foreach ( $product_ids as $key => $product_id ) {
            $product   = wc_get_product( $product_id );
            $line_item = $order->get_item( absint( $item_ids[ $key ] ) );
        ...
        $reason_label = dokan_rma_refund_reasons( $request['reasons'] );
        if ( is_array( $reason_label ) || '' === $reason_label ) {
            $reason_label = esc_html__( 'No reason provided', 'dokan' );
        }

        $has_already_refund_request = false;
        if ( 'refund' === $request['type'] && dokan_pro()->refund->has_pending_request( $order->get_id() ) ) {
            $has_already_refund_request = true;
        }
        ...
        return apply_filters(
            ...
            [
                ...
                'type'             => $request['type'],
                'type_label'       => dokan_warranty_request_type( $request['type'] ),
                'status'           => $request['status'],
                'reasons'          => $request['reasons'],
                'reasons_label'    => $reason_label,
     ...
     * @return array
     */
    public function transform_rma_settings( $request = [] ): array {
        $data = [];
     ...
     * @return array
     */
    public function get_settings( int $product_id = 0 ): array {
        $rma_settings   = [];
     ...
     * @return bool
     */
    public function check_required_warranty( int $product_id ): bool {
        $warranty = $this->get_settings( $product_id );

        return $warranty['type'] === 'addon_warranty' && ! isset( $_REQUEST['dokan_warranty'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
    }

    /**
     * Transform warranty request for deleted orders
     *
     * @since 3.7.0
     *
     * @param array $request
     ...
     * @return array
     */
    private function transform_deleted_order_warranty_request( $request ) {
        $product_ids = explode( ',', $request['products'] );
        $quantities = explode( ',', $request['quantity'] );
        $item_ids = explode( ',', $request['item_id'] );
        // Return basic information for deleted orders
        $vendor = dokan()->vendor->get( $request['vendor_id'] );
        $customer = get_user_by( 'id', $request['customer_id'] );
        $items = [];
        ...
        return apply_filters(
            ...
            [
                ...
                'order_id'    => $request['order_id'],
                'vendor'   => [
                    'store_id'   => $vendor ? $vendor->get_id() : 0,
                    'store_name' => $vendor ? $vendor->get_shop_name() : __( 'Unknown Store', 'dokan' ),
                    'store_url'  => $vendor ? $vendor->get_shop_url() : '#',
                ],
                'customer' => [
                    'id'   => $customer ? $customer->ID : 0,
                    'name' => $customer ? $customer->display_name : __( 'Unknown Customer', 'dokan' ),
                ],
                'type'             => $request['type'],
                'type_label'       => dokan_warranty_request_type( $request['type'] ),
                'status'           => 'info_removed', // New status for deleted orders
                'reasons'          => $request['reasons'],
                'reasons_label'    => $reason_label,
                'details'          => $request['details'],
                'note'             => $request['note'],
