...
class Vendor {
    ...
    public function __construct() {
        ...
        // Expose delivery-time permissions in the vendor-staff "Manage Permissions" UI.
        add_filter( 'dokan_get_all_cap', [ $this, 'add_capabilities' ] );

        add_filter( 'dokan_rest_prepare_shop_order_object', [ $this, 'add_delivery_time_to_order_response' ], 10, 2 );
    }

    /**
     * Add delivery time capabilities for vendor staff.
     *
     * @since 5.0.0
     *
    ...
    public function add_capabilities( array $capabilities ): array {
        $capabilities['menu']['dokan_view_delivery_time_menu']       = esc_html__( 'View delivery time menu', 'dokan' );
        $capabilities['menu']['dokan_view_store_delivery_time_menu'] = esc_html__( 'View delivery time settings menu', 'dokan' );
     ...
     * @return void
     */
    public static function add_role_capabilities(): void {
        global $wp_roles;
        ...
        $capabilities = [
            'dokan_view_delivery_time_menu',
            'dokan_view_store_delivery_time_menu',
        ];
    ...
    public function register_dashboard_menu( $urls ) {
        ...
        if ( ! user_can( get_current_user_id(), 'dokan_view_store_delivery_time_menu' ) ) {
            return $urls;
        }
        ...
        $urls['delivery-time'] = array(
            ...
            'pos'         => 61,
            'react_route' => 'settings/delivery-time',
            'permission'  => 'dokan_view_store_delivery_time_menu',
        );
    ...
    public function register_delivery_calender_menu( $urls ) {
        if ( ! user_can( get_current_user_id(), 'dokan_view_delivery_time_menu' ) ) {
            return $urls;
        }
        ...
        $urls['delivery-time-dashboard'] = [
            ...
            'permission'  => 'dokan_view_delivery_time_menu',
        ];
    ...
    public function load_scripts() {
        ...
        if ( dokan_is_seller_dashboard() ) {
            wp_enqueue_script( 'delivery-time-dashboard' );
            wp_enqueue_style( 'delivery-time-dashboard' );
        }
    ...
    /**
     * Adds delivery time info to the order REST API response.
     *
     * @since 4.3.2
     *
     ...
     */
    public function add_delivery_time_to_order_response( WP_REST_Response $response, WC_Order $order ) {
        ...
        $delivery_info = Helper::get_order_delivery_info( $vendor_id, $order->get_id() );
        ...
        $delivery_type_data = $delivery_info->delivery_type ?? '';

        $data['dokan_delivery_time'] = [
            'date'            => $delivery_info->date ?? '',
            'slot'            => $delivery_info->slot ?? '',
            'delivery_type'   => $delivery_type_data,
            'pickup_location' => '',
        ];

        if ( 'store-pickup' === $delivery_type_data ) {
            $data['dokan_delivery_time']['pickup_location'] = $order->get_meta( 'dokan_store_pickup_location' );
        }
