...
class Messenger {
    ...
    public function shortcode( $atts ) {
        $page_id = ! empty( $atts['page_id'] ) ? sanitize_text_field( $atts['page_id'] ) : '';
    ...
    public function render_live_chat_button( $vendor_id ) {
        ...
        <li class="dokan-store-support-btn-wrap dokan-right">
            ...
            <?php echo do_shortcode( sprintf( '[dokan-live-chat-messenger page_id="%s"]', esc_attr( $page_id ) ) ); ?>
        </li>
        <?php
    }
    ...
    public function render_live_chat_button_product_page() {
        ...
        printf( '<button style="margin-left: 5px;" class="dokan-live-chat dokan-live-chat-messenger button alt">%s</button>', esc_html__( 'Chat Now', 'dokan' ) );
        echo do_shortcode( sprintf( '[dokan-live-chat-messenger page_id="%s"]', esc_attr( $page_id ) ) );
    }
