...
class Whatsapp {
    ...
    public function shortcode( $atts ) {
        $whatsapp_number = ! empty( $atts['number'] ) ? sanitize_text_field( $atts['number'] ) : '';
        $this->seller_id = 0;
        ...
        <a href="<?php echo esc_url( $url ); ?>" class="whatsapp-live-widget" target="_blank">
            <img src="<?php echo esc_url( DOKAN_LIVE_CHAT_ASSETS . '/images/whatsapp.svg' ); ?>" alt="">
        </a>
    ...
    public function render_live_chat_button( $seller_id ) {
        ...
        <li class="dokan-store-support-btn-wrap dokan-right">
            ...
            <?php echo do_shortcode( sprintf( '[dokan-live-chat-whatsapp number="%s"]', esc_attr( $whatsapp_number ) ) ); ?>
        </li>
        <?php
    }
    ...
    private function render_on_product_single_page() {
        ...
        printf( '<button style="margin-left: 5px;" class="dokan-live-chat dokan-live-chat-whatsapp button alt">%s</button>', esc_html__( 'Chat Now', 'dokan' ) );

        echo do_shortcode( sprintf( '[dokan-live-chat-whatsapp number="%s"]', esc_attr( $whatsapp_number ) ) );
    }
    ...
    private function get_whatsapp_url( $whatsapp_number ) {
        ...
        $url        = "https://{$sub_domain}.whatsapp.com/send/?phone=" . rawurlencode( $whatsapp_number );

        if ( ! empty( $pre_filled_message ) ) {
            $url .= '&text=' . rawurlencode( $pre_filled_message );
        }
