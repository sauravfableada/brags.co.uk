...
use WeDevs\DokanPro\Dependencies\GuzzleHttp\Cookie\CookieJar;
use WeDevs\DokanPro\Dependencies\GuzzleHttp\Cookie\CookieJarInterface;
use WeDevs\DokanPro\Dependencies\GuzzleHttp\Exception\GuzzleException;
use WeDevs\DokanPro\Dependencies\GuzzleHttp\Exception\InvalidArgumentException;
use WeDevs\DokanPro\Dependencies\GuzzleHttp\Handler\CurlShareHandleState;
use WeDevs\DokanPro\Dependencies\GuzzleHttp\Promise as P;
use WeDevs\DokanPro\Dependencies\GuzzleHttp\Promise\PromiseInterface;
...
use WeDevs\DokanPro\Dependencies\Psr\Http\Message\StreamInterface;
use WeDevs\DokanPro\Dependencies\Psr\Http\Message\UriInterface;

/**
 * @final
 */
class Client implements ClientInterface, \WeDevs\DokanPro\Dependencies\Psr\Http\Client\ClientInterface
{
    use ClientTrait;
    ...
    private static function warnAboutInvalidRequestOptionTypes(array $options): void
    {
        ...
        if (isset($options['body']) && \is_array($options['body'])) {
            self::warnInvalidRequestOptionType('body', 'resource|string|null|int|float|bool|StreamInterface|(callable&object)|\Iterator|\WeDevs_DokanPro_WeDevs_DokanPro_Stringable', $options['body']);
        }
    ...
    private static function createBodyStream($body): StreamInterface
    {
        ...
        throw new InvalidArgumentException(\sprintf(
            'Passing %s to request option "body" is invalid; expected resource|string|null|int|float|bool|StreamInterface|callable&object|Iterator|WeDevs_DokanPro_WeDevs_DokanPro_Stringable.',
            \get_debug_type($body)
        ));
    }
