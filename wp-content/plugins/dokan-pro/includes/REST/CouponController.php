...
class CouponController extends DokanRESTController {
    ...
    public function register_routes() {
        ...
        register_rest_route(
            $this->namespace, '/' . $this->base . '/marketplace', array(
                array(
                    'methods'             => WP_REST_Server::READABLE,
                    'callback'            => array( $this, 'get_marketplace_coupons' ),
                    'permission_callback' => array( $this, 'get_coupon_permission_check' ),
                    'args'                => $this->get_collection_params(),
                ),
            )
    ...
    public function validation_before_create_item( $request ) {
        ...
        if ( ! empty( $request['id'] ) ) {
            return new WP_Error( "dokan_rest_{$this->post_type}_exists", __( 'Cannot create existing product.', 'dokan' ), array( 'status' => 400 ) );
        }

        if ( empty( $request['code'] ) ) {
            return new WP_Error( 'dokan_coupon_no_code_found', __( 'Coupon code is required.', 'dokan' ), array( 'status' => 404 ) );
        }
    ...
    protected function prepare_object_for_database( $request, $creating = false ) {
        ...
        // add is_admin_coupon = false in meta_data
        $coupon->update_meta_data( 'is_admin_coupon', 'no' );
        // add coupon_commissions_type = from_vendor
        $coupon->update_meta_data( 'coupon_commissions_type', 'from_vendor' );
    ...
    /**
     * Format marketplace coupon data
     * @since 4.0.0
     *
     * @param WP_Post $coupon
     * @return array
     */
    private function format_marketplace_coupon( $coupon ) {
        $wc_coupon = new WC_Coupon( $coupon->ID );
        $coupon_data = $wc_coupon->get_data();
        ...
        return [
            ...
            'date_created'    => wc_rest_prepare_date_response( $coupon_data['date_created'] ),
            'date_expires'    => $coupon_data['date_expires'] ? wc_rest_prepare_date_response( $coupon_data['date_expires'] ) : null,
            ...
            'email_restrictions' => $coupon_data['email_restrictions'],
            'meta_data'      => $this->get_marketplace_coupon_meta( $coupon->ID ),
            'status'         => $coupon->post_status,
            'admin_shared_amount' => $admin_shared_amount,
            'shared_coupon_type'  => $shared_coupon_type,
            'commissions_type'    => dokan_get_admin_coupon_commissions_type()[ $commissions_type ],
            'commissions_type_value' => $commissions_type,
            'enabled_all_vendor'  => $enabled_all_vendor,
            'get_product_ids'     => $get_product_ids,
            'created_by'     => [
                'id'   => $coupon->post_author,
                'name' => get_user_by( 'id', $coupon->post_author )->display_name,
            ],
        ];
    }

    /**
     * Get marketplace coupon meta data
     *
     * @param int $coupon_id
     * @return array
     */
    private function get_marketplace_coupon_meta( $coupon_id ) {
        ...
        return [
            ...
            'vendor_ids'         => array_map( 'intval', explode( ',', $vendor_ids ? $vendor_ids : '' ) ),
            'exclude_vendor_ids' => array_map( 'intval', explode( ',', $exclude_vendor_ids ? $exclude_vendor_ids : '' ) ),
            'show_on_store'      => get_post_meta( $coupon_id, 'admin_coupons_show_on_stores', true ) === 'yes',
        ];
    }
     ...
     */
    public function get_marketplace_coupons( $request ) {
        try {
            $store_id = dokan_get_current_user_id();
            $response = [];

            if ( ! empty( $store_id ) ) {
                $show_on_store = isset( $request['show_on_store'] ) ? (bool) $request['show_on_store'] : false;
                $search         = ! empty( $request['search'] ) ? sanitize_text_field( $request['search'] ) : '';
                $seller_coupons = dokan_get_marketplace_seller_coupon( $store_id, $show_on_store, $search );

                if ( ! empty( $seller_coupons ) ) {
                    foreach ( $seller_coupons as $coupon ) {
                        $response[] = $this->format_marketplace_coupon( $coupon );
                    }
                }
                ...
                return rest_ensure_response(
                    [
                        'success' => true,
                        'data'    => $response,
                        'message' => ! empty( $response ) ? __( 'Marketplace coupons retrieved successfully', 'dokan' ) : __( 'No marketplace coupons found', 'dokan' ),
                    ]
                );
            }
            ...
            return rest_ensure_response(
                [
                    'success' => false,
                    'data'    => [],
                    'message' => __( 'No seller found', 'dokan' ),
                ]
            );
        } catch ( Exception $e ) {
            return new WP_Error(
                'dokan_rest_coupon_error',
                $e->getMessage(),
                [ 'status' => 500 ]
            );
        }
