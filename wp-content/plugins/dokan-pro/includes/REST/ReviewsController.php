...
class ReviewsController extends DokanRESTController {
    ...
    public function get_reviews( $request ) {
        ...
        $comments = $review_class->comment_query( $store_id, 'product', $limit, $status, $offset, $search );

        $count = $this->get_total_count( $status, $search );
     ...
     *
     * @param string $status
     * @param string $search Optional. Filter count by author name/email match.
     *
     * @since 2.8.0
     * @since 5.0.5 Added optional `$search` parameter.
     *
     * @return int
     */
    public function get_total_count( $status, $search = '' ) {
        ...
        if ( '' !== $search ) {
            ...
            $search_clause = " AND ( $wpdb->comments.comment_author LIKE %s OR $wpdb->comments.comment_author_email LIKE %s )";
        ...
        $sql = "SELECT COUNT(*)
            ...
            $wpdb->posts.post_type = 'product'" . $search_clause;

        $params = array_merge( [ $user_id, $status ], $search_params );
