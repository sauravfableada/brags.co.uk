<?php
/**
 * New User Approve Messages
 *
 * Contains functions for default messages used in the New User Approve plugin.
 *
...
function nua_default_approve_user_message() {
	$message  =
		__( 'You have been approved to access {sitename}', 'new-user-approve' ) .
		"\r\n\r\n";
	$message .= "{username}\r\n\r\n";
	$message .= "{login_url}\r\n\r\n";
	$message .=
	...
	$message = apply_filters(
		'new_user_approve_approve_user_message_default',
		$message
	);
...
function nua_default_deny_user_message() {
	$message = __(
		'You have been denied access to {sitename}.',
		'new-user-approve'
	);

	$message = apply_filters(
		'new_user_approve_deny_user_message_default',
		$message
	);
...
/**
 * The default message that will be shown to users who are auto-approved with invitation codes.
 *
 * @return string
 */
...
function nua_default_welcome_message() {
	$welcome = sprintf(
		__(
			'Welcome to {sitename}. This site is accessible to approved users only. To be approved, you must first register.',
			'new-user-approve'
		),
		get_option( 'blogname' )
	);
	...
	$welcome = apply_filters(
		'new_user_approve_welcome_message_default',
		$welcome
	);
...
function nua_default_notification_message() {
	$message  =
		__(
			'{username} ({user_email}) has requested a username at {sitename}',
			'new-user-approve'
		) . "\n\n";
	$message .= "{site_url}\n\n";
	$message .=
	...
	$message = apply_filters(
		'new_user_approve_notification_message_default',
		$message
	);
...
function nua_default_registration_message() {
	...
	$message = apply_filters(
		'new_user_approve_registration_message_default',
		$message
	);
...
/**
 * The default welcome email message sent to users after registration.
 *
 * @return string
 */
